UNIX BUILD NOTES
====================
Some notes on how to build ATC in Unix.

Note
---------------------
Always use absolute paths to configure and compile atc and the dependencies,
...
memory available when compiling ATC. On systems with less, gcc can be
...
See the section "Disable-wallet mode" to build ATC without wallet.
...
Notes
-----
The release is built with GCC and then "strip aitcoind" to strip the debug
...
If you see memory usage blow up over time from your aitcoind node (particularly on the STN) you
...
Security
--------
To help make your atc installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, atc should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, atc may be compiled in
...
This example lists the steps necessary to setup and build a command line only, non-wallet distribution of the latest changes on Arch Linux:
    ...
    git clone https://github.com/atc-labs/aitcoin
    cd aitcoin/
    ./autogen.sh
...
Note 2: When running aitcoind, it expects boost libreries in /usr/lib64 folder. Make sure that they are located there. See libbost instalation step for details.
...
with 4.8-built atc Core is needed follow the steps under "Berkeley DB" above.
