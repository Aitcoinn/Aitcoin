...
The ATC daemon can be configured to act as a trusted "border
router", implementing the atc wire protocol and relay, making
...
The ZeroMQ feature in ATC requires ZeroMQ API version 4.3.2 or
...
during the *configure* step of building aitcoind:
...
For instance:
    ...
    $ aitcoind -zmqpubhashtx=tcp://127.0.0.1:29882 \
               -zmqpubrawtx=ipc:///tmp/aitcoind.tx.raw

These options can also be provided in atc.conf.
...
From the perspective of aitcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into ATC directly. Furthermore, no information is
...
using. ATC appends an up-counting sequence number to each
