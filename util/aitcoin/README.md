# New chain creation
This document describes how to create a new chain from ATC.
...
3. `nDefaultPort` - Hardcoded as 9883,which can be configured in atc.conf
4. `nMinimumChainWork` - Change the hardcode value or configured in atc.conf
5. `checkpointData` 
6. `ChainTxData`
...
2. `asertActivationTime` and `asertAnchorParams`
    ATC mainnet use chainwork-144 as difficulty adjustment algorithm (DAA) before height 21256, then the DAA changed to 
    aserti3-2d .
