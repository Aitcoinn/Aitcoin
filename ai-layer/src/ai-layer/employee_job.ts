...
export class EmployeeJob {
  private states: Map<string, EmployeeJobState> = new Map();

  constructor() {
    blockchainBridge.registerModule('employee_job');
  }

  private getOrCreate(entityId: string): EmployeeJobState {
  ...
  activate(entityId: string, intensity = 50): EmployeeJobState {
    ...
    blockchainBridge.validateAIOutput('employee_job', { entityId, level: state.level });
    logger.info({ entityId, module: 'employee_job', intensity }, '[EmployeeJob] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'employee_job' }, '[EmployeeJob] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EmployeeJobState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): EmployeeJobState | null { return this.states.get(entityId) ?? null; }
...
export const employeeJob = new EmployeeJob();
