...
export class MagneticField {
  private states: Map<string, MagneticFieldState> = new Map();

  constructor() {
    blockchainBridge.registerModule('magnetic_field');
  }

  private getOrCreate(entityId: string): MagneticFieldState {
  ...
  activate(entityId: string, intensity = 50): MagneticFieldState {
    ...
    blockchainBridge.validateAIOutput('magnetic_field', { entityId, level: state.level });
    logger.info({ entityId, module: 'magnetic_field', intensity }, '[MagneticField] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'magnetic_field' }, '[MagneticField] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MagneticFieldState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MagneticFieldState | null { return this.states.get(entityId) ?? null; }
