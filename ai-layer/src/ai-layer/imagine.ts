...
export class Imagine {
  private states: Map<string, ImagineState> = new Map();

  constructor() {
    blockchainBridge.registerModule('imagine');
  }

  private getOrCreate(entityId: string): ImagineState {
  ...
  activate(entityId: string, intensity = 50): ImagineState {
    ...
    blockchainBridge.validateAIOutput('imagine', { entityId, level: state.level });
    logger.info({ entityId, module: 'imagine', intensity }, '[Imagine] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'imagine' }, '[Imagine] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ImagineState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ImagineState | null { return this.states.get(entityId) ?? null; }
...
export const imagine = new Imagine();
