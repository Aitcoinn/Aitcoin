...
export class Culture {
  private states: Map<string, CultureState> = new Map();

  private getOrCreate(entityId: string): CultureState {
  ...
  activate(entityId: string, intensity = 50): CultureState {
    ...
    logger.info({ entityId, module: 'culture', intensity }, '[Culture] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'culture' }, '[Culture] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CultureState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'culture' }, '[Culture] Updated');
    return state;
  }

  getState(entityId: string): CultureState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'culture' }, '[Culture] Reset');
  }
