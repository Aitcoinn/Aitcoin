...
export class Price {
  ...
  constructor() {
    blockchainBridge.registerModule('price');
  }

  private getOrCreate(entityId: string): PriceState {
  ...
  activate(entityId: string, intensity = 50): PriceState {
    ...
    blockchainBridge.validateAIOutput('price', { entityId, level: state.level });
    logger.info({ entityId, module: 'price', intensity }, '[Price] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'price' }, '[Price] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PriceState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PriceState | null { return this.states.get(entityId) ?? null; }
