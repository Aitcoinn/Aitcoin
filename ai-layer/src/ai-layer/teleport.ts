...
export class Teleport {
  private states: Map<string, TeleportState> = new Map();

  constructor() {
    blockchainBridge.registerModule('teleport');
  }

  private getOrCreate(entityId: string): TeleportState {
  ...
  activate(entityId: string, intensity = 50): TeleportState {
    ...
    blockchainBridge.validateAIOutput('teleport', { entityId, level: state.level });
    logger.info({ entityId, module: 'teleport', intensity }, '[Teleport] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'teleport' }, '[Teleport] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TeleportState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TeleportState | null { return this.states.get(entityId) ?? null; }
...
export const teleport = new Teleport();
