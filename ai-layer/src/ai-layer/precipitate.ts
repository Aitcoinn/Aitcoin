...
export class Precipitate {
  private states: Map<string, PrecipitateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('precipitate');
  }

  private getOrCreate(entityId: string): PrecipitateState {
  ...
  activate(entityId: string, intensity = 50): PrecipitateState {
    ...
    blockchainBridge.validateAIOutput('precipitate', { entityId, level: state.level });
    logger.info({ entityId, module: 'precipitate', intensity }, '[Precipitate] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'precipitate' }, '[Precipitate] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PrecipitateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PrecipitateState | null { return this.states.get(entityId) ?? null; }
...
export const precipitate = new Precipitate();
