...
export class Mop {
  private states: Map<string, MopState> = new Map();

  constructor() {
    blockchainBridge.registerModule('mop');
  }

  private getOrCreate(entityId: string): MopState {
  ...
  activate(entityId: string, intensity = 50): MopState {
    ...
    blockchainBridge.validateAIOutput('mop', { entityId, level: state.level });
    logger.info({ entityId, module: 'mop', intensity }, '[Mop] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'mop' }, '[Mop] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MopState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MopState | null { return this.states.get(entityId) ?? null; }
