...
export class Borealis {
  private states: Map<string, BorealisState> = new Map();

  constructor() {
    blockchainBridge.registerModule('borealis');
  }

  private getOrCreate(entityId: string): BorealisState {
  ...
  activate(entityId: string, intensity = 50): BorealisState {
    ...
    blockchainBridge.validateAIOutput('borealis', { entityId, level: state.level });
    logger.info({ entityId, module: 'borealis', intensity }, '[Borealis] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'borealis' }, '[Borealis] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BorealisState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BorealisState | null { return this.states.get(entityId) ?? null; }
...
export const borealis = new Borealis();
