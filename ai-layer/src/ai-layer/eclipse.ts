...
export class Eclipse {
  private states: Map<string, EclipseState> = new Map();

  constructor() {
    blockchainBridge.registerModule('eclipse');
  }

  private getOrCreate(entityId: string): EclipseState {
  ...
  activate(entityId: string, intensity = 50): EclipseState {
    ...
    blockchainBridge.validateAIOutput('eclipse', { entityId, level: state.level });
    logger.info({ entityId, module: 'eclipse', intensity }, '[Eclipse] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'eclipse' }, '[Eclipse] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EclipseState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): EclipseState | null { return this.states.get(entityId) ?? null; }
...
export const eclipse = new Eclipse();
