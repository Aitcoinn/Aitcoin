...
export class SkyColor {
  private states: Map<string, SkyColorState> = new Map();

  constructor() {
    blockchainBridge.registerModule('sky_color');
  }

  private getOrCreate(entityId: string): SkyColorState {
  ...
  activate(entityId: string, intensity = 50): SkyColorState {
    ...
    blockchainBridge.validateAIOutput('sky_color', { entityId, level: state.level });
    logger.info({ entityId, module: 'sky_color', intensity }, '[SkyColor] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'sky_color' }, '[SkyColor] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SkyColorState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SkyColorState | null { return this.states.get(entityId) ?? null; }
...
export const skyColor = new SkyColor();
