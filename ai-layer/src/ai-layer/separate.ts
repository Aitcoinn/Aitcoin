...
export class Separate {
  private states: Map<string, SeparateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('separate');
  }

  private getOrCreate(entityId: string): SeparateState {
  ...
  activate(entityId: string, intensity = 50): SeparateState {
    ...
    blockchainBridge.validateAIOutput('separate', { entityId, level: state.level });
    logger.info({ entityId, module: 'separate', intensity }, '[Separate] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'separate' }, '[Separate] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SeparateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SeparateState | null { return this.states.get(entityId) ?? null; }
...
export const separate = new Separate();
