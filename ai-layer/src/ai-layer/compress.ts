...
export class Compress {
  private states: Map<string, CompressState> = new Map();

  constructor() {
    blockchainBridge.registerModule('compress');
  }

  private getOrCreate(entityId: string): CompressState {
  ...
  activate(entityId: string, intensity = 50): CompressState {
    ...
    blockchainBridge.validateAIOutput('compress', { entityId, level: state.level });
    logger.info({ entityId, module: 'compress', intensity }, '[Compress] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'compress' }, '[Compress] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CompressState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CompressState | null { return this.states.get(entityId) ?? null; }
