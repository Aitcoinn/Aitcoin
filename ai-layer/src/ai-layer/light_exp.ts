...
export class LightExp {
  private states: Map<string, LightExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('light_exp');
  }

  private getOrCreate(entityId: string): LightExpState {
  ...
  activate(entityId: string, intensity = 50): LightExpState {
    ...
    blockchainBridge.validateAIOutput('light_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'light_exp', intensity }, '[LightExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'light_exp' }, '[LightExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LightExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LightExpState | null { return this.states.get(entityId) ?? null; }
...
export const lightExp = new LightExp();
export default lightExp;
