...
export class Database {
  ...
  constructor() {
    blockchainBridge.registerModule('database');
  }

  private getOrCreate(entityId: string): DatabaseState {
  ...
  activate(entityId: string, intensity = 50): DatabaseState {
    ...
    blockchainBridge.validateAIOutput('database', { entityId, level: state.level });
    logger.info({ entityId, module: 'database', intensity }, '[Database] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'database' }, '[Database] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DatabaseState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DatabaseState | null { return this.states.get(entityId) ?? null; }
