...
export class Trust {
  ...
  private getOrCreate(entityId: string): TrustState {
  ...
  activate(entityId: string, intensity = 50): TrustState {
    ...
    logger.info({ entityId, module: 'trust', intensity }, '[Trust] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'trust' }, '[Trust] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TrustState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'trust' }, '[Trust] Updated');
    return state;
  }

  getState(entityId: string): TrustState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'trust' }, '[Trust] Reset');
  }
