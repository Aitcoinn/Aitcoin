...
export class Sleet {
  private states: Map<string, SleetState> = new Map();

  constructor() {
    blockchainBridge.registerModule('sleet');
  }

  private getOrCreate(entityId: string): SleetState {
  ...
  activate(entityId: string, intensity = 50): SleetState {
    ...
    blockchainBridge.validateAIOutput('sleet', { entityId, level: state.level });
    logger.info({ entityId, module: 'sleet', intensity }, '[Sleet] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'sleet' }, '[Sleet] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SleetState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SleetState | null { return this.states.get(entityId) ?? null; }
