...
export class Empathy {
  ...
  private getOrCreate(entityId: string): EmpathyState {
  ...
  activate(entityId: string, intensity = 50): EmpathyState {
    ...
    logger.info({ entityId, module: 'empathy', intensity }, '[Empathy] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'empathy' }, '[Empathy] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EmpathyState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'empathy' }, '[Empathy] Updated');
    return state;
  }

  getState(entityId: string): EmpathyState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'empathy' }, '[Empathy] Reset');
  }
...
export const empathy = new Empathy();
