...
export class DryWeather {
  private states: Map<string, DryWeatherState> = new Map();

  constructor() {
    blockchainBridge.registerModule('dry_weather');
  }

  private getOrCreate(entityId: string): DryWeatherState {
  ...
  activate(entityId: string, intensity = 50): DryWeatherState {
    ...
    blockchainBridge.validateAIOutput('dry_weather', { entityId, level: state.level });
    logger.info({ entityId, module: 'dry_weather', intensity }, '[DryWeather] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'dry_weather' }, '[DryWeather] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DryWeatherState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DryWeatherState | null { return this.states.get(entityId) ?? null; }
...
export const dryWeather = new DryWeather();
export default dryWeather;
