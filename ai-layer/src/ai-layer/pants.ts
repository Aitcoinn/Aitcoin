...
export class Pants {
  private states: Map<string, PantsState> = new Map();

  constructor() {
    blockchainBridge.registerModule('pants');
  }

  private getOrCreate(entityId: string): PantsState {
  ...
  activate(entityId: string, intensity = 50): PantsState {
    ...
    blockchainBridge.validateAIOutput('pants', { entityId, level: state.level });
    logger.info({ entityId, module: 'pants', intensity }, '[Pants] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'pants' }, '[Pants] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PantsState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PantsState | null { return this.states.get(entityId) ?? null; }
