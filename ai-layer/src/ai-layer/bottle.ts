...
export class Bottle {
  private states: Map<string, BottleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('bottle');
  }

  private getOrCreate(entityId: string): BottleState {
  ...
  activate(entityId: string, intensity = 50): BottleState {
    ...
    blockchainBridge.validateAIOutput('bottle', { entityId, level: state.level });
    logger.info({ entityId, module: 'bottle', intensity }, '[Bottle] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'bottle' }, '[Bottle] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BottleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BottleState | null { return this.states.get(entityId) ?? null; }
