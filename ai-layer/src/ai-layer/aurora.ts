...
export class Aurora {
  private states: Map<string, AuroraState> = new Map();

  constructor() {
    blockchainBridge.registerModule('aurora');
  }

  private getOrCreate(entityId: string): AuroraState {
  ...
  activate(entityId: string, intensity = 50): AuroraState {
    ...
    blockchainBridge.validateAIOutput('aurora', { entityId, level: state.level });
    logger.info({ entityId, module: 'aurora', intensity }, '[Aurora] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'aurora' }, '[Aurora] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AuroraState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AuroraState | null { return this.states.get(entityId) ?? null; }
