...
export class Axe {
  private states: Map<string, AxeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('axe');
  }

  private getOrCreate(entityId: string): AxeState {
  ...
  activate(entityId: string, intensity = 50): AxeState {
    ...
    blockchainBridge.validateAIOutput('axe', { entityId, level: state.level });
    logger.info({ entityId, module: 'axe', intensity }, '[Axe] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'axe' }, '[Axe] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AxeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AxeState | null { return this.states.get(entityId) ?? null; }
