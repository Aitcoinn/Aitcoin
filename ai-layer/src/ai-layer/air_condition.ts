...
export class AirCondition {
  private states: Map<string, AirConditionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('air_condition');
  }

  private getOrCreate(entityId: string): AirConditionState {
  ...
  activate(entityId: string, intensity = 50): AirConditionState {
    ...
    blockchainBridge.validateAIOutput('air_condition', { entityId, level: state.level });
    logger.info({ entityId, module: 'air_condition', intensity }, '[AirCondition] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'air_condition' }, '[AirCondition] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AirConditionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AirConditionState | null { return this.states.get(entityId) ?? null; }
...
export const airCondition = new AirCondition();
