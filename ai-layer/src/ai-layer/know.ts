...
export class Know {
  private states: Map<string, KnowState> = new Map();

  constructor() {
    blockchainBridge.registerModule('know');
  }

  private getOrCreate(entityId: string): KnowState {
  ...
  activate(entityId: string, intensity = 50): KnowState {
    ...
    blockchainBridge.validateAIOutput('know', { entityId, level: state.level });
    logger.info({ entityId, module: 'know', intensity }, '[Know] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'know' }, '[Know] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): KnowState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): KnowState | null { return this.states.get(entityId) ?? null; }
