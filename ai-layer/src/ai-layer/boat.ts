...
export class Boat {
  private states: Map<string, BoatState> = new Map();

  constructor() {
    blockchainBridge.registerModule('boat');
  }

  private getOrCreate(entityId: string): BoatState {
  ...
  activate(entityId: string, intensity = 50): BoatState {
    ...
    blockchainBridge.validateAIOutput('boat', { entityId, level: state.level });
    logger.info({ entityId, module: 'boat', intensity }, '[Boat] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'boat' }, '[Boat] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BoatState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BoatState | null { return this.states.get(entityId) ?? null; }
