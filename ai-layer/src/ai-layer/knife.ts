...
export class Knife {
  private states: Map<string, KnifeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('knife');
  }

  private getOrCreate(entityId: string): KnifeState {
  ...
  activate(entityId: string, intensity = 50): KnifeState {
    ...
    blockchainBridge.validateAIOutput('knife', { entityId, level: state.level });
    logger.info({ entityId, module: 'knife', intensity }, '[Knife] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'knife' }, '[Knife] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): KnifeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): KnifeState | null { return this.states.get(entityId) ?? null; }
