...
export class DepositionGeo {
  private states: Map<string, DepositionGeoState> = new Map();

  constructor() {
    blockchainBridge.registerModule('deposition_geo');
  }

  private getOrCreate(entityId: string): DepositionGeoState {
  ...
  activate(entityId: string, intensity = 50): DepositionGeoState {
    ...
    blockchainBridge.validateAIOutput('deposition_geo', { entityId, level: state.level });
    logger.info({ entityId, module: 'deposition_geo', intensity }, '[DepositionGeo] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'deposition_geo' }, '[DepositionGeo] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DepositionGeoState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DepositionGeoState | null { return this.states.get(entityId) ?? null; }
...
export const depositionGeo = new DepositionGeo();
export default depositionGeo;
