...
export class CoreEarth {
  private states: Map<string, CoreEarthState> = new Map();

  constructor() {
    blockchainBridge.registerModule('core_earth');
  }

  private getOrCreate(entityId: string): CoreEarthState {
  ...
  activate(entityId: string, intensity = 50): CoreEarthState {
    ...
    blockchainBridge.validateAIOutput('core_earth', { entityId, level: state.level });
    logger.info({ entityId, module: 'core_earth', intensity }, '[CoreEarth] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'core_earth' }, '[CoreEarth] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CoreEarthState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CoreEarthState | null { return this.states.get(entityId) ?? null; }
...
export const coreEarth = new CoreEarth();
export default coreEarth;
