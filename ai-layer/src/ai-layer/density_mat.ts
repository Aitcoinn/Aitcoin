...
export class DensityMat {
  private states: Map<string, DensityMatState> = new Map();

  constructor() {
    blockchainBridge.registerModule('density_mat');
  }

  private getOrCreate(entityId: string): DensityMatState {
  ...
  activate(entityId: string, intensity = 50): DensityMatState {
    ...
    blockchainBridge.validateAIOutput('density_mat', { entityId, level: state.level });
    logger.info({ entityId, module: 'density_mat', intensity }, '[DensityMat] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'density_mat' }, '[DensityMat] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DensityMatState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DensityMatState | null { return this.states.get(entityId) ?? null; }
...
export const densityMat = new DensityMat();
export default densityMat;
