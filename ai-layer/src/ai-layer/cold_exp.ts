...
export class ColdExp {
  private states: Map<string, ColdExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cold_exp');
  }

  private getOrCreate(entityId: string): ColdExpState {
  ...
  activate(entityId: string, intensity = 50): ColdExpState {
    ...
    blockchainBridge.validateAIOutput('cold_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'cold_exp', intensity }, '[ColdExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cold_exp' }, '[ColdExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ColdExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ColdExpState | null { return this.states.get(entityId) ?? null; }
...
export const coldExp = new ColdExp();
export default coldExp;
