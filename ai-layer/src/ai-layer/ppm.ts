...
export class Ppm {
  private states: Map<string, PpmState> = new Map();

  constructor() {
    blockchainBridge.registerModule('ppm');
  }

  private getOrCreate(entityId: string): PpmState {
  ...
  activate(entityId: string, intensity = 50): PpmState {
    ...
    blockchainBridge.validateAIOutput('ppm', { entityId, level: state.level });
    logger.info({ entityId, module: 'ppm', intensity }, '[Ppm] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ppm' }, '[Ppm] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PpmState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PpmState | null { return this.states.get(entityId) ?? null; }
