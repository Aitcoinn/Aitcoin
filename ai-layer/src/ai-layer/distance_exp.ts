...
export class DistanceExp {
  private states: Map<string, DistanceExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('distance_exp');
  }

  private getOrCreate(entityId: string): DistanceExpState {
  ...
  activate(entityId: string, intensity = 50): DistanceExpState {
    ...
    blockchainBridge.validateAIOutput('distance_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'distance_exp', intensity }, '[DistanceExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'distance_exp' }, '[DistanceExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DistanceExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DistanceExpState | null { return this.states.get(entityId) ?? null; }
...
export const distanceExp = new DistanceExp();
export default distanceExp;
