...
export class ToiletSys {
  private states: Map<string, ToiletSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('toilet_sys');
  }

  private getOrCreate(entityId: string): ToiletSysState {
  ...
  activate(entityId: string, intensity = 50): ToiletSysState {
    ...
    blockchainBridge.validateAIOutput('toilet_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'toilet_sys', intensity }, '[ToiletSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'toilet_sys' }, '[ToiletSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ToiletSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ToiletSysState | null { return this.states.get(entityId) ?? null; }
...
export const toiletSys = new ToiletSys();
export default toiletSys;
