...
export class LockData {
  private states: Map<string, LockDataState> = new Map();

  constructor() {
    blockchainBridge.registerModule('lock_data');
  }

  private getOrCreate(entityId: string): LockDataState {
  ...
  activate(entityId: string, intensity = 50): LockDataState {
    ...
    blockchainBridge.validateAIOutput('lock_data', { entityId, level: state.level });
    logger.info({ entityId, module: 'lock_data', intensity }, '[LockData] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'lock_data' }, '[LockData] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LockDataState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LockDataState | null { return this.states.get(entityId) ?? null; }
...
export const lockData = new LockData();
