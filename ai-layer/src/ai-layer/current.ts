...
export class Current {
  private states: Map<string, CurrentState> = new Map();

  constructor() {
    blockchainBridge.registerModule('current');
  }

  private getOrCreate(entityId: string): CurrentState {
  ...
  activate(entityId: string, intensity = 50): CurrentState {
    ...
    blockchainBridge.validateAIOutput('current', { entityId, level: state.level });
    logger.info({ entityId, module: 'current', intensity }, '[Current] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'current' }, '[Current] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CurrentState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CurrentState | null { return this.states.get(entityId) ?? null; }
