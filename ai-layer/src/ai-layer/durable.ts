...
export class Durable {
  ...
  private getOrCreate(entityId: string): DurableState {
  ...
  activate(entityId: string, intensity = 50): DurableState {
    ...
    logger.info({ entityId, module: 'durable', intensity }, '[Durable] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'durable' }, '[Durable] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DurableState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'durable' }, '[Durable] Updated');
    return state;
  }

  getState(entityId: string): DurableState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'durable' }, '[Durable] Reset');
  }
...
export const durable = new Durable();
