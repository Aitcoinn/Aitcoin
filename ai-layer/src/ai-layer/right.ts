...
export class Right {
  private states: Map<string, RightState> = new Map();

  constructor() {
    blockchainBridge.registerModule('right');
  }

  private getOrCreate(entityId: string): RightState {
  ...
  activate(entityId: string, intensity = 50): RightState {
    ...
    blockchainBridge.validateAIOutput('right', { entityId, level: state.level });
    logger.info({ entityId, module: 'right', intensity }, '[Right] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'right' }, '[Right] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RightState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RightState | null { return this.states.get(entityId) ?? null; }
