...
export class WetWeather {
  private states: Map<string, WetWeatherState> = new Map();

  constructor() {
    blockchainBridge.registerModule('wet_weather');
  }

  private getOrCreate(entityId: string): WetWeatherState {
  ...
  activate(entityId: string, intensity = 50): WetWeatherState {
    ...
    blockchainBridge.validateAIOutput('wet_weather', { entityId, level: state.level });
    logger.info({ entityId, module: 'wet_weather', intensity }, '[WetWeather] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'wet_weather' }, '[WetWeather] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WetWeatherState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WetWeatherState | null { return this.states.get(entityId) ?? null; }
...
export const wetWeather = new WetWeather();
export default wetWeather;
