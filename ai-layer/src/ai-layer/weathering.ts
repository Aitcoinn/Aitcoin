...
export class Weathering {
  private states: Map<string, WeatheringState> = new Map();

  constructor() {
    blockchainBridge.registerModule('weathering');
  }

  private getOrCreate(entityId: string): WeatheringState {
  ...
  activate(entityId: string, intensity = 50): WeatheringState {
    ...
    blockchainBridge.validateAIOutput('weathering', { entityId, level: state.level });
    logger.info({ entityId, module: 'weathering', intensity }, '[Weathering] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'weathering' }, '[Weathering] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WeatheringState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WeatheringState | null { return this.states.get(entityId) ?? null; }
...
export const weathering = new Weathering();
