...
export class ThrowError {
  private states: Map<string, ThrowErrorState> = new Map();

  constructor() {
    blockchainBridge.registerModule('throw_error');
  }

  private getOrCreate(entityId: string): ThrowErrorState {
  ...
  activate(entityId: string, intensity = 50): ThrowErrorState {
    ...
    blockchainBridge.validateAIOutput('throw_error', { entityId, level: state.level });
    logger.info({ entityId, module: 'throw_error', intensity }, '[ThrowError] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'throw_error' }, '[ThrowError] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ThrowErrorState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ThrowErrorState | null { return this.states.get(entityId) ?? null; }
