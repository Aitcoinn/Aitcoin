...
export class HighPressure {
  private states: Map<string, HighPressureState> = new Map();

  constructor() {
    blockchainBridge.registerModule('high_pressure');
  }

  private getOrCreate(entityId: string): HighPressureState {
  ...
  activate(entityId: string, intensity = 50): HighPressureState {
    ...
    blockchainBridge.validateAIOutput('high_pressure', { entityId, level: state.level });
    logger.info({ entityId, module: 'high_pressure', intensity }, '[HighPressure] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'high_pressure' }, '[HighPressure] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HighPressureState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HighPressureState | null { return this.states.get(entityId) ?? null; }
...
export const highPressure = new HighPressure();
