...
export class Socket {
  private states: Map<string, SocketState> = new Map();

  constructor() {
    blockchainBridge.registerModule('socket');
  }

  private getOrCreate(entityId: string): SocketState {
  ...
  activate(entityId: string, intensity = 50): SocketState {
    ...
    blockchainBridge.validateAIOutput('socket', { entityId, level: state.level });
    logger.info({ entityId, module: 'socket', intensity }, '[Socket] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'socket' }, '[Socket] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SocketState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SocketState | null { return this.states.get(entityId) ?? null; }
