...
export class Cost {
  private states: Map<string, CostState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cost');
  }

  private getOrCreate(entityId: string): CostState {
  ...
  activate(entityId: string, intensity = 50): CostState {
    ...
    blockchainBridge.validateAIOutput('cost', { entityId, level: state.level });
    logger.info({ entityId, module: 'cost', intensity }, '[Cost] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cost' }, '[Cost] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CostState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CostState | null { return this.states.get(entityId) ?? null; }
