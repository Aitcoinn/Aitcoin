...
export class RainbowSky {
  private states: Map<string, RainbowSkyState> = new Map();

  constructor() {
    blockchainBridge.registerModule('rainbow_sky');
  }

  private getOrCreate(entityId: string): RainbowSkyState {
  ...
  activate(entityId: string, intensity = 50): RainbowSkyState {
    ...
    blockchainBridge.validateAIOutput('rainbow_sky', { entityId, level: state.level });
    logger.info({ entityId, module: 'rainbow_sky', intensity }, '[RainbowSky] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'rainbow_sky' }, '[RainbowSky] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RainbowSkyState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RainbowSkyState | null { return this.states.get(entityId) ?? null; }
...
export const rainbowSky = new RainbowSky();
