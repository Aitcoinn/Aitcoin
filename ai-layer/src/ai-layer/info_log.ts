...
export class InfoLog {
  private states: Map<string, InfoLogState> = new Map();

  constructor() {
    blockchainBridge.registerModule('info_log');
  }

  private getOrCreate(entityId: string): InfoLogState {
  ...
  activate(entityId: string, intensity = 50): InfoLogState {
    ...
    blockchainBridge.validateAIOutput('info_log', { entityId, level: state.level });
    logger.info({ entityId, module: 'info_log', intensity }, '[InfoLog] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'info_log' }, '[InfoLog] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): InfoLogState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): InfoLogState | null { return this.states.get(entityId) ?? null; }
