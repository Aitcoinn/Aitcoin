...
export class Car {
  private states: Map<string, CarState> = new Map();

  constructor() {
    blockchainBridge.registerModule('car');
  }

  private getOrCreate(entityId: string): CarState {
  ...
  activate(entityId: string, intensity = 50): CarState {
    ...
    blockchainBridge.validateAIOutput('car', { entityId, level: state.level });
    logger.info({ entityId, module: 'car', intensity }, '[Car] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'car' }, '[Car] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CarState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CarState | null { return this.states.get(entityId) ?? null; }
