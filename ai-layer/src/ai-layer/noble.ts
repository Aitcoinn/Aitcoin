...
export class Noble {
  private states: Map<string, NobleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('noble');
  }

  private getOrCreate(entityId: string): NobleState {
  ...
  activate(entityId: string, intensity = 50): NobleState {
    ...
    blockchainBridge.validateAIOutput('noble', { entityId, level: state.level });
    logger.info({ entityId, module: 'noble', intensity }, '[Noble] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'noble' }, '[Noble] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): NobleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): NobleState | null { return this.states.get(entityId) ?? null; }
