...
export class Gas {
  private states: Map<string, GasState> = new Map();

  constructor() {
    blockchainBridge.registerModule('gas');
  }

  private getOrCreate(entityId: string): GasState {
  ...
  activate(entityId: string, intensity = 50): GasState {
    ...
    blockchainBridge.validateAIOutput('gas', { entityId, level: state.level });
    logger.info({ entityId, module: 'gas', intensity }, '[Gas] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'gas' }, '[Gas] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): GasState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): GasState | null { return this.states.get(entityId) ?? null; }
