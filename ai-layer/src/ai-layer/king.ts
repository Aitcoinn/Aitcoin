...
export class King {
  private states: Map<string, KingState> = new Map();

  private getOrCreate(entityId: string): KingState {
  ...
  activate(entityId: string, intensity = 50): KingState {
    ...
    logger.info({ entityId, module: 'king', intensity }, '[King] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'king' }, '[King] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): KingState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'king' }, '[King] Updated');
    return state;
  }

  getState(entityId: string): KingState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'king' }, '[King] Reset');
  }
