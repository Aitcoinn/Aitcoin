...
export class LibraryRoom {
  private states: Map<string, LibraryRoomState> = new Map();

  constructor() {
    blockchainBridge.registerModule('library_room');
  }

  private getOrCreate(entityId: string): LibraryRoomState {
  ...
  activate(entityId: string, intensity = 50): LibraryRoomState {
    ...
    blockchainBridge.validateAIOutput('library_room', { entityId, level: state.level });
    logger.info({ entityId, module: 'library_room', intensity }, '[LibraryRoom] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'library_room' }, '[LibraryRoom] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LibraryRoomState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LibraryRoomState | null { return this.states.get(entityId) ?? null; }
...
export const libraryRoom = new LibraryRoom();
