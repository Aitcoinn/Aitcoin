...
export class AiEngine {
  private states: Map<string, AiEngineState> = new Map();

  constructor() {
    blockchainBridge.registerModule('ai_engine');
  }

  private getOrCreate(entityId: string): AiEngineState {
  ...
  activate(entityId: string, intensity = 50): AiEngineState {
    ...
    blockchainBridge.validateAIOutput('ai_engine', { entityId, level: state.level });
    logger.info({ entityId, module: 'ai_engine', intensity }, '[AiEngine] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ai_engine' }, '[AiEngine] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AiEngineState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AiEngineState | null { return this.states.get(entityId) ?? null; }
