...
export class LogicExp {
  private states: Map<string, LogicExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('logic_exp');
  }

  private getOrCreate(entityId: string): LogicExpState {
  ...
  activate(entityId: string, intensity = 50): LogicExpState {
    ...
    blockchainBridge.validateAIOutput('logic_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'logic_exp', intensity }, '[LogicExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'logic_exp' }, '[LogicExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LogicExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LogicExpState | null { return this.states.get(entityId) ?? null; }
...
export const logicExp = new LogicExp();
export default logicExp;
