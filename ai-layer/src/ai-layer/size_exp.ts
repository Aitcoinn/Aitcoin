...
export class SizeExp {
  private states: Map<string, SizeExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('size_exp');
  }

  private getOrCreate(entityId: string): SizeExpState {
  ...
  activate(entityId: string, intensity = 50): SizeExpState {
    ...
    blockchainBridge.validateAIOutput('size_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'size_exp', intensity }, '[SizeExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'size_exp' }, '[SizeExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SizeExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SizeExpState | null { return this.states.get(entityId) ?? null; }
...
export const sizeExp = new SizeExp();
export default sizeExp;
