...
export class Reward {
  private states: Map<string, RewardState> = new Map();

  constructor() {
    blockchainBridge.registerModule('reward');
  }

  private getOrCreate(entityId: string): RewardState {
  ...
  activate(entityId: string, intensity = 50): RewardState {
    ...
    blockchainBridge.validateAIOutput('reward', { entityId, level: state.level });
    logger.info({ entityId, module: 'reward', intensity }, '[Reward] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'reward' }, '[Reward] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RewardState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RewardState | null { return this.states.get(entityId) ?? null; }
