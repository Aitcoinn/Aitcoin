...
export class Loud {
  private states: Map<string, LoudState> = new Map();

  constructor() {
    blockchainBridge.registerModule('loud');
  }

  private getOrCreate(entityId: string): LoudState {
  ...
  activate(entityId: string, intensity = 50): LoudState {
    ...
    blockchainBridge.validateAIOutput('loud', { entityId, level: state.level });
    logger.info({ entityId, module: 'loud', intensity }, '[Loud] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'loud' }, '[Loud] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LoudState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LoudState | null { return this.states.get(entityId) ?? null; }
