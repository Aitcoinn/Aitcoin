...
export class Condense {
  private states: Map<string, CondenseState> = new Map();

  constructor() {
    blockchainBridge.registerModule('condense');
  }

  private getOrCreate(entityId: string): CondenseState {
  ...
  activate(entityId: string, intensity = 50): CondenseState {
    ...
    blockchainBridge.validateAIOutput('condense', { entityId, level: state.level });
    logger.info({ entityId, module: 'condense', intensity }, '[Condense] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'condense' }, '[Condense] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CondenseState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CondenseState | null { return this.states.get(entityId) ?? null; }
...
export const condense = new Condense();
