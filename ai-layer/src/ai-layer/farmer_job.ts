...
export class FarmerJob {
  private states: Map<string, FarmerJobState> = new Map();

  constructor() {
    blockchainBridge.registerModule('farmer_job');
  }

  private getOrCreate(entityId: string): FarmerJobState {
  ...
  activate(entityId: string, intensity = 50): FarmerJobState {
    ...
    blockchainBridge.validateAIOutput('farmer_job', { entityId, level: state.level });
    logger.info({ entityId, module: 'farmer_job', intensity }, '[FarmerJob] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'farmer_job' }, '[FarmerJob] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FarmerJobState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FarmerJobState | null { return this.states.get(entityId) ?? null; }
...
export const farmerJob = new FarmerJob();
