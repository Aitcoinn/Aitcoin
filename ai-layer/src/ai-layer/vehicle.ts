...
export class Vehicle {
  private states: Map<string, VehicleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('vehicle');
  }

  private getOrCreate(entityId: string): VehicleState {
  ...
  activate(entityId: string, intensity = 50): VehicleState {
    ...
    blockchainBridge.validateAIOutput('vehicle', { entityId, level: state.level });
    logger.info({ entityId, module: 'vehicle', intensity }, '[Vehicle] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'vehicle' }, '[Vehicle] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): VehicleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): VehicleState | null { return this.states.get(entityId) ?? null; }
