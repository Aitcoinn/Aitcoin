...
export class Earthquake {
  private states: Map<string, EarthquakeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('earthquake');
  }

  private getOrCreate(entityId: string): EarthquakeState {
  ...
  activate(entityId: string, intensity = 50): EarthquakeState {
    ...
    blockchainBridge.validateAIOutput('earthquake', { entityId, level: state.level });
    logger.info({ entityId, module: 'earthquake', intensity }, '[Earthquake] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'earthquake' }, '[Earthquake] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EarthquakeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): EarthquakeState | null { return this.states.get(entityId) ?? null; }
...
export const earthquake = new Earthquake();
