...
export class GymRoom {
  private states: Map<string, GymRoomState> = new Map();

  constructor() {
    blockchainBridge.registerModule('gym_room');
  }

  private getOrCreate(entityId: string): GymRoomState {
  ...
  activate(entityId: string, intensity = 50): GymRoomState {
    ...
    blockchainBridge.validateAIOutput('gym_room', { entityId, level: state.level });
    logger.info({ entityId, module: 'gym_room', intensity }, '[GymRoom] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'gym_room' }, '[GymRoom] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): GymRoomState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): GymRoomState | null { return this.states.get(entityId) ?? null; }
...
export const gymRoom = new GymRoom();
