...
export class CrystalForm {
  private states: Map<string, CrystalFormState> = new Map();

  constructor() {
    blockchainBridge.registerModule('crystal_form');
  }

  private getOrCreate(entityId: string): CrystalFormState {
  ...
  activate(entityId: string, intensity = 50): CrystalFormState {
    ...
    blockchainBridge.validateAIOutput('crystal_form', { entityId, level: state.level });
    logger.info({ entityId, module: 'crystal_form', intensity }, '[CrystalForm] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'crystal_form' }, '[CrystalForm] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CrystalFormState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CrystalFormState | null { return this.states.get(entityId) ?? null; }
...
export const crystalForm = new CrystalForm();
