...
export class Boiling {
  private states: Map<string, BoilingState> = new Map();

  constructor() {
    blockchainBridge.registerModule('boiling');
  }

  private getOrCreate(entityId: string): BoilingState {
  ...
  activate(entityId: string, intensity = 50): BoilingState {
    ...
    blockchainBridge.validateAIOutput('boiling', { entityId, level: state.level });
    logger.info({ entityId, module: 'boiling', intensity }, '[Boiling] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'boiling' }, '[Boiling] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BoilingState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BoilingState | null { return this.states.get(entityId) ?? null; }
...
export const boiling = new Boiling();
