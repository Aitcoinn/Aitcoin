...
export class Tsunami {
  private states: Map<string, TsunamiState> = new Map();

  constructor() {
    blockchainBridge.registerModule('tsunami');
  }

  private getOrCreate(entityId: string): TsunamiState {
  ...
  activate(entityId: string, intensity = 50): TsunamiState {
    ...
    blockchainBridge.validateAIOutput('tsunami', { entityId, level: state.level });
    logger.info({ entityId, module: 'tsunami', intensity }, '[Tsunami] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'tsunami' }, '[Tsunami] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TsunamiState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TsunamiState | null { return this.states.get(entityId) ?? null; }
...
export const tsunami = new Tsunami();
