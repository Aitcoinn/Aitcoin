...
export class Inert {
  private states: Map<string, InertState> = new Map();

  constructor() {
    blockchainBridge.registerModule('inert');
  }

  private getOrCreate(entityId: string): InertState {
  ...
  activate(entityId: string, intensity = 50): InertState {
    ...
    blockchainBridge.validateAIOutput('inert', { entityId, level: state.level });
    logger.info({ entityId, module: 'inert', intensity }, '[Inert] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'inert' }, '[Inert] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): InertState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): InertState | null { return this.states.get(entityId) ?? null; }
