...
export class Select {
  private states: Map<string, SelectState> = new Map();

  constructor() {
    blockchainBridge.registerModule('select');
  }

  private getOrCreate(entityId: string): SelectState {
  ...
  activate(entityId: string, intensity = 50): SelectState {
    ...
    blockchainBridge.validateAIOutput('select', { entityId, level: state.level });
    logger.info({ entityId, module: 'select', intensity }, '[Select] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'select' }, '[Select] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SelectState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SelectState | null { return this.states.get(entityId) ?? null; }
