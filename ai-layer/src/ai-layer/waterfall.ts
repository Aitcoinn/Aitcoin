...
export class Waterfall {
  private states: Map<string, WaterfallState> = new Map();

  constructor() {
    blockchainBridge.registerModule('waterfall');
  }

  private getOrCreate(entityId: string): WaterfallState {
  ...
  activate(entityId: string, intensity = 50): WaterfallState {
    ...
    blockchainBridge.validateAIOutput('waterfall', { entityId, level: state.level });
    logger.info({ entityId, module: 'waterfall', intensity }, '[Waterfall] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'waterfall' }, '[Waterfall] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WaterfallState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WaterfallState | null { return this.states.get(entityId) ?? null; }
