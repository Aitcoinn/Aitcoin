...
export class Ship {
  private states: Map<string, ShipState> = new Map();

  constructor() {
    blockchainBridge.registerModule('ship');
  }

  private getOrCreate(entityId: string): ShipState {
  ...
  activate(entityId: string, intensity = 50): ShipState {
    ...
    blockchainBridge.validateAIOutput('ship', { entityId, level: state.level });
    logger.info({ entityId, module: 'ship', intensity }, '[Ship] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ship' }, '[Ship] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ShipState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ShipState | null { return this.states.get(entityId) ?? null; }
