...
export class Worker {
  ...
  constructor() {
    blockchainBridge.registerModule('worker');
  }

  private getOrCreate(entityId: string): WorkerState {
  ...
  activate(entityId: string, intensity = 50): WorkerState {
    ...
    blockchainBridge.validateAIOutput('worker', { entityId, level: state.level });
    logger.info({ entityId, module: 'worker', intensity }, '[Worker] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'worker' }, '[Worker] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WorkerState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WorkerState | null { return this.states.get(entityId) ?? null; }
