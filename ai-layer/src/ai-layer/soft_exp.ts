...
export class SoftExp {
  private states: Map<string, SoftExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('soft_exp');
  }

  private getOrCreate(entityId: string): SoftExpState {
  ...
  activate(entityId: string, intensity = 50): SoftExpState {
    ...
    blockchainBridge.validateAIOutput('soft_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'soft_exp', intensity }, '[SoftExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'soft_exp' }, '[SoftExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SoftExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SoftExpState | null { return this.states.get(entityId) ?? null; }
...
export const softExp = new SoftExp();
export default softExp;
