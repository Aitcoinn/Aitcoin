...
export class Wire {
  private states: Map<string, WireState> = new Map();

  constructor() {
    blockchainBridge.registerModule('wire');
  }

  private getOrCreate(entityId: string): WireState {
  ...
  activate(entityId: string, intensity = 50): WireState {
    ...
    blockchainBridge.validateAIOutput('wire', { entityId, level: state.level });
    logger.info({ entityId, module: 'wire', intensity }, '[Wire] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'wire' }, '[Wire] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WireState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WireState | null { return this.states.get(entityId) ?? null; }
