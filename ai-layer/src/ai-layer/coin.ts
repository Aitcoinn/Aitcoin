...
export class Coin {
  private states: Map<string, CoinState> = new Map();

  constructor() {
    blockchainBridge.registerModule('coin');
  }

  private getOrCreate(entityId: string): CoinState {
  ...
  activate(entityId: string, intensity = 50): CoinState {
    ...
    blockchainBridge.validateAIOutput('coin', { entityId, level: state.level });
    logger.info({ entityId, module: 'coin', intensity }, '[Coin] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'coin' }, '[Coin] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CoinState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CoinState | null { return this.states.get(entityId) ?? null; }
