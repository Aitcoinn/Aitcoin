...
export class Shoes {
  private states: Map<string, ShoesState> = new Map();

  constructor() {
    blockchainBridge.registerModule('shoes');
  }

  private getOrCreate(entityId: string): ShoesState {
  ...
  activate(entityId: string, intensity = 50): ShoesState {
    ...
    blockchainBridge.validateAIOutput('shoes', { entityId, level: state.level });
    logger.info({ entityId, module: 'shoes', intensity }, '[Shoes] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'shoes' }, '[Shoes] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ShoesState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ShoesState | null { return this.states.get(entityId) ?? null; }
