...
export class WhileLoop {
  private states: Map<string, WhileLoopState> = new Map();

  constructor() {
    blockchainBridge.registerModule('while_loop');
  }

  private getOrCreate(entityId: string): WhileLoopState {
  ...
  activate(entityId: string, intensity = 50): WhileLoopState {
    ...
    blockchainBridge.validateAIOutput('while_loop', { entityId, level: state.level });
    logger.info({ entityId, module: 'while_loop', intensity }, '[WhileLoop] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'while_loop' }, '[WhileLoop] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WhileLoopState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WhileLoopState | null { return this.states.get(entityId) ?? null; }
...
export const whileLoop = new WhileLoop();
