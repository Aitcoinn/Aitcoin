...
export class Saw {
  private states: Map<string, SawState> = new Map();

  constructor() {
    blockchainBridge.registerModule('saw');
  }

  private getOrCreate(entityId: string): SawState {
  ...
  activate(entityId: string, intensity = 50): SawState {
    ...
    blockchainBridge.validateAIOutput('saw', { entityId, level: state.level });
    logger.info({ entityId, module: 'saw', intensity }, '[Saw] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'saw' }, '[Saw] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SawState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SawState | null { return this.states.get(entityId) ?? null; }
