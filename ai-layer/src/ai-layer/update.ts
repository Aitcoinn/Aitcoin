...
export class Update {
  private states: Map<string, UpdateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('update');
  }

  private getOrCreate(entityId: string): UpdateState {
  ...
  activate(entityId: string, intensity = 50): UpdateState {
    ...
    blockchainBridge.validateAIOutput('update', { entityId, level: state.level });
    logger.info({ entityId, module: 'update', intensity }, '[Update] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'update' }, '[Update] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): UpdateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): UpdateState | null { return this.states.get(entityId) ?? null; }
