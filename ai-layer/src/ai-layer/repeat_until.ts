...
export class RepeatUntil {
  private states: Map<string, RepeatUntilState> = new Map();

  constructor() {
    blockchainBridge.registerModule('repeat_until');
  }

  private getOrCreate(entityId: string): RepeatUntilState {
  ...
  activate(entityId: string, intensity = 50): RepeatUntilState {
    ...
    blockchainBridge.validateAIOutput('repeat_until', { entityId, level: state.level });
    logger.info({ entityId, module: 'repeat_until', intensity }, '[RepeatUntil] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'repeat_until' }, '[RepeatUntil] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RepeatUntilState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RepeatUntilState | null { return this.states.get(entityId) ?? null; }
...
export const repeatUntil = new RepeatUntil();
