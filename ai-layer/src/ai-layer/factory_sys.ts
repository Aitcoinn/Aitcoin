...
export class FactorySys {
  private states: Map<string, FactorySysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('factory_sys');
  }

  private getOrCreate(entityId: string): FactorySysState {
  ...
  activate(entityId: string, intensity = 50): FactorySysState {
    ...
    blockchainBridge.validateAIOutput('factory_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'factory_sys', intensity }, '[FactorySys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'factory_sys' }, '[FactorySys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FactorySysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FactorySysState | null { return this.states.get(entityId) ?? null; }
...
export const factorySys = new FactorySys();
export default factorySys;
