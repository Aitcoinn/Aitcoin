...
export class Coordinate {
  private states: Map<string, CoordinateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('coordinate');
  }

  private getOrCreate(entityId: string): CoordinateState {
  ...
  activate(entityId: string, intensity = 50): CoordinateState {
    ...
    blockchainBridge.validateAIOutput('coordinate', { entityId, level: state.level });
    logger.info({ entityId, module: 'coordinate', intensity }, '[Coordinate] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'coordinate' }, '[Coordinate] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CoordinateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CoordinateState | null { return this.states.get(entityId) ?? null; }
...
export const coordinate = new Coordinate();
