...
export class ObjectData {
  private states: Map<string, ObjectDataState> = new Map();

  constructor() {
    blockchainBridge.registerModule('object_data');
  }

  private getOrCreate(entityId: string): ObjectDataState {
  ...
  activate(entityId: string, intensity = 50): ObjectDataState {
    ...
    blockchainBridge.validateAIOutput('object_data', { entityId, level: state.level });
    logger.info({ entityId, module: 'object_data', intensity }, '[ObjectData] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'object_data' }, '[ObjectData] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ObjectDataState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ObjectDataState | null { return this.states.get(entityId) ?? null; }
...
export const objectData = new ObjectData();
