...
export class Valley {
  private states: Map<string, ValleyState> = new Map();

  constructor() {
    blockchainBridge.registerModule('valley');
  }

  private getOrCreate(entityId: string): ValleyState {
  ...
  activate(entityId: string, intensity = 50): ValleyState {
    ...
    blockchainBridge.validateAIOutput('valley', { entityId, level: state.level });
    logger.info({ entityId, module: 'valley', intensity }, '[Valley] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'valley' }, '[Valley] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ValleyState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ValleyState | null { return this.states.get(entityId) ?? null; }
...
export const valley = new Valley();
