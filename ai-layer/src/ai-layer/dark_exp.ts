...
export class DarkExp {
  private states: Map<string, DarkExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('dark_exp');
  }

  private getOrCreate(entityId: string): DarkExpState {
  ...
  activate(entityId: string, intensity = 50): DarkExpState {
    ...
    blockchainBridge.validateAIOutput('dark_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'dark_exp', intensity }, '[DarkExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'dark_exp' }, '[DarkExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DarkExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DarkExpState | null { return this.states.get(entityId) ?? null; }
...
export const darkExp = new DarkExp();
export default darkExp;
