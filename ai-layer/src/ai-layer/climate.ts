...
export class Climate {
  private states: Map<string, ClimateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('climate');
  }

  private getOrCreate(entityId: string): ClimateState {
  ...
  activate(entityId: string, intensity = 50): ClimateState {
    ...
    blockchainBridge.validateAIOutput('climate', { entityId, level: state.level });
    logger.info({ entityId, module: 'climate', intensity }, '[Climate] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'climate' }, '[Climate] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ClimateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ClimateState | null { return this.states.get(entityId) ?? null; }
...
export const climate = new Climate();
