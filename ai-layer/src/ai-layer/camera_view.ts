...
export class CameraView {
  private states: Map<string, CameraViewState> = new Map();

  constructor() {
    blockchainBridge.registerModule('camera_view');
  }

  private getOrCreate(entityId: string): CameraViewState {
  ...
  activate(entityId: string, intensity = 50): CameraViewState {
    ...
    blockchainBridge.validateAIOutput('camera_view', { entityId, level: state.level });
    logger.info({ entityId, module: 'camera_view', intensity }, '[CameraView] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'camera_view' }, '[CameraView] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CameraViewState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CameraViewState | null { return this.states.get(entityId) ?? null; }
...
export const cameraView = new CameraView();
