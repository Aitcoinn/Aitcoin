...
export class StreetLight {
  private states: Map<string, StreetLightState> = new Map();

  constructor() {
    blockchainBridge.registerModule('street_light');
  }

  private getOrCreate(entityId: string): StreetLightState {
  ...
  activate(entityId: string, intensity = 50): StreetLightState {
    ...
    blockchainBridge.validateAIOutput('street_light', { entityId, level: state.level });
    logger.info({ entityId, module: 'street_light', intensity }, '[StreetLight] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'street_light' }, '[StreetLight] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): StreetLightState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): StreetLightState | null { return this.states.get(entityId) ?? null; }
...
export const streetLight = new StreetLight();
