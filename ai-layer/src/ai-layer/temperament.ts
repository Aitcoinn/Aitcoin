...
export class Temperament {
  ...
  private getOrCreate(entityId: string): TemperamentState {
  ...
  activate(entityId: string, intensity = 50): TemperamentState {
    ...
    logger.info({ entityId, module: 'temperament', intensity }, '[Temperament] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'temperament' }, '[Temperament] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TemperamentState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'temperament' }, '[Temperament] Updated');
    return state;
  }

  getState(entityId: string): TemperamentState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'temperament' }, '[Temperament] Reset');
  }
...
export const temperament = new Temperament();
