...
export class KitchenSys {
  private states: Map<string, KitchenSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('kitchen_sys');
  }

  private getOrCreate(entityId: string): KitchenSysState {
  ...
  activate(entityId: string, intensity = 50): KitchenSysState {
    ...
    blockchainBridge.validateAIOutput('kitchen_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'kitchen_sys', intensity }, '[KitchenSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'kitchen_sys' }, '[KitchenSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): KitchenSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): KitchenSysState | null { return this.states.get(entityId) ?? null; }
...
export const kitchenSys = new KitchenSys();
export default kitchenSys;
