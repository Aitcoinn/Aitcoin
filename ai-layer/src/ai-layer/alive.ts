...
export class Alive {
  private states: Map<string, AliveState> = new Map();

  constructor() {
    blockchainBridge.registerModule('alive');
  }

  private getOrCreate(entityId: string): AliveState {
  ...
  activate(entityId: string, intensity = 50): AliveState {
    ...
    blockchainBridge.validateAIOutput('alive', { entityId, level: state.level });
    logger.info({ entityId, module: 'alive', intensity }, '[Alive] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'alive' }, '[Alive] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AliveState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AliveState | null { return this.states.get(entityId) ?? null; }
