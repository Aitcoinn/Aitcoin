...
export class ParkingSys {
  private states: Map<string, ParkingSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('parking_sys');
  }

  private getOrCreate(entityId: string): ParkingSysState {
  ...
  activate(entityId: string, intensity = 50): ParkingSysState {
    ...
    blockchainBridge.validateAIOutput('parking_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'parking_sys', intensity }, '[ParkingSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'parking_sys' }, '[ParkingSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ParkingSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ParkingSysState | null { return this.states.get(entityId) ?? null; }
...
export const parkingSys = new ParkingSys();
export default parkingSys;
