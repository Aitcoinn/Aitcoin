...
export class Hazard {
  private states: Map<string, HazardState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hazard');
  }

  private getOrCreate(entityId: string): HazardState {
  ...
  activate(entityId: string, intensity = 50): HazardState {
    ...
    blockchainBridge.validateAIOutput('hazard', { entityId, level: state.level });
    logger.info({ entityId, module: 'hazard', intensity }, '[Hazard] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hazard' }, '[Hazard] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HazardState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HazardState | null { return this.states.get(entityId) ?? null; }
...
export const hazard = new Hazard();
