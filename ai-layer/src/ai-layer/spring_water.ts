...
export class SpringWater {
  private states: Map<string, SpringWaterState> = new Map();

  constructor() {
    blockchainBridge.registerModule('spring_water');
  }

  private getOrCreate(entityId: string): SpringWaterState {
  ...
  activate(entityId: string, intensity = 50): SpringWaterState {
    ...
    blockchainBridge.validateAIOutput('spring_water', { entityId, level: state.level });
    logger.info({ entityId, module: 'spring_water', intensity }, '[SpringWater] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'spring_water' }, '[SpringWater] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SpringWaterState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SpringWaterState | null { return this.states.get(entityId) ?? null; }
...
export const springWater = new SpringWater();
