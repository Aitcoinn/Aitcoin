...
export class Bathroom {
  private states: Map<string, BathroomState> = new Map();

  constructor() {
    blockchainBridge.registerModule('bathroom');
  }

  private getOrCreate(entityId: string): BathroomState {
  ...
  activate(entityId: string, intensity = 50): BathroomState {
    ...
    blockchainBridge.validateAIOutput('bathroom', { entityId, level: state.level });
    logger.info({ entityId, module: 'bathroom', intensity }, '[Bathroom] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'bathroom' }, '[Bathroom] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BathroomState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BathroomState | null { return this.states.get(entityId) ?? null; }
...
export const bathroom = new Bathroom();
