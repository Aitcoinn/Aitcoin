...
export class Lightning {
  private states: Map<string, LightningState> = new Map();

  constructor() {
    blockchainBridge.registerModule('lightning');
  }

  private getOrCreate(entityId: string): LightningState {
  ...
  activate(entityId: string, intensity = 50): LightningState {
    ...
    blockchainBridge.validateAIOutput('lightning', { entityId, level: state.level });
    logger.info({ entityId, module: 'lightning', intensity }, '[Lightning] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'lightning' }, '[Lightning] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LightningState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LightningState | null { return this.states.get(entityId) ?? null; }
