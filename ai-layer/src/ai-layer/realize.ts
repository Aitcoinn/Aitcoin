...
export class Realize {
  private states: Map<string, RealizeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('realize');
  }

  private getOrCreate(entityId: string): RealizeState {
  ...
  activate(entityId: string, intensity = 50): RealizeState {
    ...
    blockchainBridge.validateAIOutput('realize', { entityId, level: state.level });
    logger.info({ entityId, module: 'realize', intensity }, '[Realize] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'realize' }, '[Realize] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RealizeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RealizeState | null { return this.states.get(entityId) ?? null; }
...
export const realize = new Realize();
