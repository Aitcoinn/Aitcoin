...
export class Hash {
  private states: Map<string, HashState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hash');
  }

  private getOrCreate(entityId: string): HashState {
  ...
  activate(entityId: string, intensity = 50): HashState {
    ...
    blockchainBridge.validateAIOutput('hash', { entityId, level: state.level });
    logger.info({ entityId, module: 'hash', intensity }, '[Hash] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hash' }, '[Hash] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HashState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HashState | null { return this.states.get(entityId) ?? null; }
