...
export class Portal {
  private states: Map<string, PortalState> = new Map();

  constructor() {
    blockchainBridge.registerModule('portal');
  }

  private getOrCreate(entityId: string): PortalState {
  ...
  activate(entityId: string, intensity = 50): PortalState {
    ...
    blockchainBridge.validateAIOutput('portal', { entityId, level: state.level });
    logger.info({ entityId, module: 'portal', intensity }, '[Portal] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'portal' }, '[Portal] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PortalState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PortalState | null { return this.states.get(entityId) ?? null; }
