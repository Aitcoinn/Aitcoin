...
export class BlueSky {
  private states: Map<string, BlueSkyState> = new Map();

  constructor() {
    blockchainBridge.registerModule('blue_sky');
  }

  private getOrCreate(entityId: string): BlueSkyState {
  ...
  activate(entityId: string, intensity = 50): BlueSkyState {
    ...
    blockchainBridge.validateAIOutput('blue_sky', { entityId, level: state.level });
    logger.info({ entityId, module: 'blue_sky', intensity }, '[BlueSky] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'blue_sky' }, '[BlueSky] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BlueSkyState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BlueSkyState | null { return this.states.get(entityId) ?? null; }
