...
export class Plier {
  private states: Map<string, PlierState> = new Map();

  constructor() {
    blockchainBridge.registerModule('plier');
  }

  private getOrCreate(entityId: string): PlierState {
  ...
  activate(entityId: string, intensity = 50): PlierState {
    ...
    blockchainBridge.validateAIOutput('plier', { entityId, level: state.level });
    logger.info({ entityId, module: 'plier', intensity }, '[Plier] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'plier' }, '[Plier] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PlierState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PlierState | null { return this.states.get(entityId) ?? null; }
