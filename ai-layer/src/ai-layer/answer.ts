...
export class Answer {
  private states: Map<string, AnswerState> = new Map();

  private getOrCreate(entityId: string): AnswerState {
  ...
  activate(entityId: string, intensity = 50): AnswerState {
    ...
    logger.info({ entityId, module: 'answer', intensity }, '[Answer] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'answer' }, '[Answer] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AnswerState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'answer' }, '[Answer] Updated');
    return state;
  }

  getState(entityId: string): AnswerState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'answer' }, '[Answer] Reset');
  }
