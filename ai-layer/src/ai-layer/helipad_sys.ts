...
export class HelipadSys {
  private states: Map<string, HelipadSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('helipad_sys');
  }

  private getOrCreate(entityId: string): HelipadSysState {
  ...
  activate(entityId: string, intensity = 50): HelipadSysState {
    ...
    blockchainBridge.validateAIOutput('helipad_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'helipad_sys', intensity }, '[HelipadSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'helipad_sys' }, '[HelipadSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HelipadSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HelipadSysState | null { return this.states.get(entityId) ?? null; }
...
export const helipadSys = new HelipadSys();
export default helipadSys;
