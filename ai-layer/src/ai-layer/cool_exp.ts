...
export class CoolExp {
  private states: Map<string, CoolExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cool_exp');
  }

  private getOrCreate(entityId: string): CoolExpState {
  ...
  activate(entityId: string, intensity = 50): CoolExpState {
    ...
    blockchainBridge.validateAIOutput('cool_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'cool_exp', intensity }, '[CoolExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cool_exp' }, '[CoolExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CoolExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CoolExpState | null { return this.states.get(entityId) ?? null; }
...
export const coolExp = new CoolExp();
export default coolExp;
