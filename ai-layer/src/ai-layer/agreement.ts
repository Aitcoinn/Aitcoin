...
export class Agreement {
  private states: Map<string, AgreementState> = new Map();

  constructor() {
    blockchainBridge.registerModule('agreement');
  }

  private getOrCreate(entityId: string): AgreementState {
  ...
  activate(entityId: string, intensity = 50): AgreementState {
    ...
    blockchainBridge.validateAIOutput('agreement', { entityId, level: state.level });
    logger.info({ entityId, module: 'agreement', intensity }, '[Agreement] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'agreement' }, '[Agreement] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AgreementState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AgreementState | null { return this.states.get(entityId) ?? null; }
