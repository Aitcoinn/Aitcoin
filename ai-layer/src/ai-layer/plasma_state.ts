...
export class PlasmaState {
  private states: Map<string, PlasmaStateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('plasma_state');
  }

  private getOrCreate(entityId: string): PlasmaStateState {
  ...
  activate(entityId: string, intensity = 50): PlasmaStateState {
    ...
    blockchainBridge.validateAIOutput('plasma_state', { entityId, level: state.level });
    logger.info({ entityId, module: 'plasma_state', intensity }, '[PlasmaState] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'plasma_state' }, '[PlasmaState] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PlasmaStateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PlasmaStateState | null { return this.states.get(entityId) ?? null; }
