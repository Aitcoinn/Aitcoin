...
export class Feel {
  private states: Map<string, FeelState> = new Map();

  constructor() {
    blockchainBridge.registerModule('feel');
  }

  private getOrCreate(entityId: string): FeelState {
  ...
  activate(entityId: string, intensity = 50): FeelState {
    ...
    blockchainBridge.validateAIOutput('feel', { entityId, level: state.level });
    logger.info({ entityId, module: 'feel', intensity }, '[Feel] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'feel' }, '[Feel] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FeelState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FeelState | null { return this.states.get(entityId) ?? null; }
