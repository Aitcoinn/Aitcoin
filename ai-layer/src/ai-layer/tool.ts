...
export class Tool {
  ...
  constructor() {
    blockchainBridge.registerModule('tool');
  }

  private getOrCreate(entityId: string): ToolState {
  ...
  activate(entityId: string, intensity = 50): ToolState {
    ...
    blockchainBridge.validateAIOutput('tool', { entityId, level: state.level });
    logger.info({ entityId, module: 'tool', intensity }, '[Tool] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'tool' }, '[Tool] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ToolState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ToolState | null { return this.states.get(entityId) ?? null; }
