...
export class Humidity {
  private states: Map<string, HumidityState> = new Map();

  constructor() {
    blockchainBridge.registerModule('humidity');
  }

  private getOrCreate(entityId: string): HumidityState {
  ...
  activate(entityId: string, intensity = 50): HumidityState {
    ...
    blockchainBridge.validateAIOutput('humidity', { entityId, level: state.level });
    logger.info({ entityId, module: 'humidity', intensity }, '[Humidity] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'humidity' }, '[Humidity] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HumidityState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HumidityState | null { return this.states.get(entityId) ?? null; }
...
export const humidity = new Humidity();
