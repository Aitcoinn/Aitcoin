...
export class StudyRoom {
  private states: Map<string, StudyRoomState> = new Map();

  constructor() {
    blockchainBridge.registerModule('study_room');
  }

  private getOrCreate(entityId: string): StudyRoomState {
  ...
  activate(entityId: string, intensity = 50): StudyRoomState {
    ...
    blockchainBridge.validateAIOutput('study_room', { entityId, level: state.level });
    logger.info({ entityId, module: 'study_room', intensity }, '[StudyRoom] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'study_room' }, '[StudyRoom] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): StudyRoomState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): StudyRoomState | null { return this.states.get(entityId) ?? null; }
...
export const studyRoom = new StudyRoom();
