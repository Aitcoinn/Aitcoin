...
export class CrustEarth {
  private states: Map<string, CrustEarthState> = new Map();

  constructor() {
    blockchainBridge.registerModule('crust_earth');
  }

  private getOrCreate(entityId: string): CrustEarthState {
  ...
  activate(entityId: string, intensity = 50): CrustEarthState {
    ...
    blockchainBridge.validateAIOutput('crust_earth', { entityId, level: state.level });
    logger.info({ entityId, module: 'crust_earth', intensity }, '[CrustEarth] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'crust_earth' }, '[CrustEarth] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CrustEarthState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CrustEarthState | null { return this.states.get(entityId) ?? null; }
...
export const crustEarth = new CrustEarth();
export default crustEarth;
