...
export class Charger {
  private states: Map<string, ChargerState> = new Map();

  constructor() {
    blockchainBridge.registerModule('charger');
  }

  private getOrCreate(entityId: string): ChargerState {
  ...
  activate(entityId: string, intensity = 50): ChargerState {
    ...
    blockchainBridge.validateAIOutput('charger', { entityId, level: state.level });
    logger.info({ entityId, module: 'charger', intensity }, '[Charger] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'charger' }, '[Charger] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ChargerState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ChargerState | null { return this.states.get(entityId) ?? null; }
