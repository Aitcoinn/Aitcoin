...
export class Cup {
  private states: Map<string, CupState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cup');
  }

  private getOrCreate(entityId: string): CupState {
  ...
  activate(entityId: string, intensity = 50): CupState {
    ...
    blockchainBridge.validateAIOutput('cup', { entityId, level: state.level });
    logger.info({ entityId, module: 'cup', intensity }, '[Cup] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cup' }, '[Cup] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CupState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CupState | null { return this.states.get(entityId) ?? null; }
