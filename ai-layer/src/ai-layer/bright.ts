...
export class Bright {
  private states: Map<string, BrightState> = new Map();

  constructor() {
    blockchainBridge.registerModule('bright');
  }

  private getOrCreate(entityId: string): BrightState {
  ...
  activate(entityId: string, intensity = 50): BrightState {
    ...
    blockchainBridge.validateAIOutput('bright', { entityId, level: state.level });
    logger.info({ entityId, module: 'bright', intensity }, '[Bright] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'bright' }, '[Bright] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BrightState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BrightState | null { return this.states.get(entityId) ?? null; }
...
export const bright = new Bright();
