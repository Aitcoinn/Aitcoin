...
export class Delight {
  ...
  private getOrCreate(entityId: string): DelightState {
  ...
  activate(entityId: string, intensity = 50): DelightState {
    ...
    logger.info({ entityId, module: 'delight', intensity }, '[Delight] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'delight' }, '[Delight] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DelightState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'delight' }, '[Delight] Updated');
    return state;
  }

  getState(entityId: string): DelightState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'delight' }, '[Delight] Reset');
  }
