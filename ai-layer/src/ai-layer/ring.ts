...
export class Ring {
  private states: Map<string, RingState> = new Map();

  constructor() {
    blockchainBridge.registerModule('ring');
  }

  private getOrCreate(entityId: string): RingState {
  ...
  activate(entityId: string, intensity = 50): RingState {
    ...
    blockchainBridge.validateAIOutput('ring', { entityId, level: state.level });
    logger.info({ entityId, module: 'ring', intensity }, '[Ring] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ring' }, '[Ring] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RingState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RingState | null { return this.states.get(entityId) ?? null; }
