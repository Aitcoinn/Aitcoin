...
export class UnderstandExp {
  private states: Map<string, UnderstandExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('understand_exp');
  }

  private getOrCreate(entityId: string): UnderstandExpState {
  ...
  activate(entityId: string, intensity = 50): UnderstandExpState {
    ...
    blockchainBridge.validateAIOutput('understand_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'understand_exp', intensity }, '[UnderstandExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'understand_exp' }, '[UnderstandExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): UnderstandExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): UnderstandExpState | null { return this.states.get(entityId) ?? null; }
...
export const understandExp = new UnderstandExp();
export default understandExp;
