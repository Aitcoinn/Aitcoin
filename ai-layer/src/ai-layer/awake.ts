...
export class Awake {
  private states: Map<string, AwakeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('awake');
  }

  private getOrCreate(entityId: string): AwakeState {
  ...
  activate(entityId: string, intensity = 50): AwakeState {
    ...
    blockchainBridge.validateAIOutput('awake', { entityId, level: state.level });
    logger.info({ entityId, module: 'awake', intensity }, '[Awake] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'awake' }, '[Awake] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AwakeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AwakeState | null { return this.states.get(entityId) ?? null; }
