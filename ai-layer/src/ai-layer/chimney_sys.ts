...
export class ChimneySys {
  private states: Map<string, ChimneySysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('chimney_sys');
  }

  private getOrCreate(entityId: string): ChimneySysState {
  ...
  activate(entityId: string, intensity = 50): ChimneySysState {
    ...
    blockchainBridge.validateAIOutput('chimney_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'chimney_sys', intensity }, '[ChimneySys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'chimney_sys' }, '[ChimneySys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ChimneySysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ChimneySysState | null { return this.states.get(entityId) ?? null; }
...
export const chimneySys = new ChimneySys();
export default chimneySys;
