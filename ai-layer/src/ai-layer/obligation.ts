...
export class Obligation {
  private states: Map<string, ObligationState> = new Map();

  constructor() {
    blockchainBridge.registerModule('obligation');
  }

  private getOrCreate(entityId: string): ObligationState {
  ...
  activate(entityId: string, intensity = 50): ObligationState {
    ...
    blockchainBridge.validateAIOutput('obligation', { entityId, level: state.level });
    logger.info({ entityId, module: 'obligation', intensity }, '[Obligation] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'obligation' }, '[Obligation] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ObligationState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ObligationState | null { return this.states.get(entityId) ?? null; }
...
export const obligation = new Obligation();
