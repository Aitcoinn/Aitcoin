...
export class RealExp {
  private states: Map<string, RealExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('real_exp');
  }

  private getOrCreate(entityId: string): RealExpState {
  ...
  activate(entityId: string, intensity = 50): RealExpState {
    ...
    blockchainBridge.validateAIOutput('real_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'real_exp', intensity }, '[RealExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'real_exp' }, '[RealExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RealExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RealExpState | null { return this.states.get(entityId) ?? null; }
...
export const realExp = new RealExp();
export default realExp;
