...
export class Hammer {
  private states: Map<string, HammerState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hammer');
  }

  private getOrCreate(entityId: string): HammerState {
  ...
  activate(entityId: string, intensity = 50): HammerState {
    ...
    blockchainBridge.validateAIOutput('hammer', { entityId, level: state.level });
    logger.info({ entityId, module: 'hammer', intensity }, '[Hammer] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hammer' }, '[Hammer] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HammerState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HammerState | null { return this.states.get(entityId) ?? null; }
