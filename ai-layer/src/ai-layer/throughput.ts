...
export class Throughput {
  private states: Map<string, ThroughputState> = new Map();

  constructor() {
    blockchainBridge.registerModule('throughput');
  }

  private getOrCreate(entityId: string): ThroughputState {
  ...
  activate(entityId: string, intensity = 50): ThroughputState {
    ...
    blockchainBridge.validateAIOutput('throughput', { entityId, level: state.level });
    logger.info({ entityId, module: 'throughput', intensity }, '[Throughput] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'throughput' }, '[Throughput] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ThroughputState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ThroughputState | null { return this.states.get(entityId) ?? null; }
