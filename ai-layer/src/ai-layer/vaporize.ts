...
export class Vaporize {
  private states: Map<string, VaporizeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('vaporize');
  }

  private getOrCreate(entityId: string): VaporizeState {
  ...
  activate(entityId: string, intensity = 50): VaporizeState {
    ...
    blockchainBridge.validateAIOutput('vaporize', { entityId, level: state.level });
    logger.info({ entityId, module: 'vaporize', intensity }, '[Vaporize] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'vaporize' }, '[Vaporize] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): VaporizeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): VaporizeState | null { return this.states.get(entityId) ?? null; }
...
export const vaporize = new Vaporize();
