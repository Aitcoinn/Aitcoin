...
export class Cable {
  private states: Map<string, CableState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cable');
  }

  private getOrCreate(entityId: string): CableState {
  ...
  activate(entityId: string, intensity = 50): CableState {
    ...
    blockchainBridge.validateAIOutput('cable', { entityId, level: state.level });
    logger.info({ entityId, module: 'cable', intensity }, '[Cable] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cable' }, '[Cable] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CableState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CableState | null { return this.states.get(entityId) ?? null; }
