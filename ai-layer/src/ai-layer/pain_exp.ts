...
export class PainExp {
  private states: Map<string, PainExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('pain_exp');
  }

  private getOrCreate(entityId: string): PainExpState {
  ...
  activate(entityId: string, intensity = 50): PainExpState {
    ...
    blockchainBridge.validateAIOutput('pain_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'pain_exp', intensity }, '[PainExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'pain_exp' }, '[PainExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PainExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PainExpState | null { return this.states.get(entityId) ?? null; }
...
export const painExp = new PainExp();
export default painExp;
