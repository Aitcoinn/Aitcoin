...
export class SlowExp {
  private states: Map<string, SlowExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('slow_exp');
  }

  private getOrCreate(entityId: string): SlowExpState {
  ...
  activate(entityId: string, intensity = 50): SlowExpState {
    ...
    blockchainBridge.validateAIOutput('slow_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'slow_exp', intensity }, '[SlowExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'slow_exp' }, '[SlowExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SlowExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SlowExpState | null { return this.states.get(entityId) ?? null; }
...
export const slowExp = new SlowExp();
export default slowExp;
