...
export class Law {
  private states: Map<string, LawState> = new Map();

  constructor() {
    blockchainBridge.registerModule('law');
  }

  private getOrCreate(entityId: string): LawState {
  ...
  activate(entityId: string, intensity = 50): LawState {
    ...
    blockchainBridge.validateAIOutput('law', { entityId, level: state.level });
    logger.info({ entityId, module: 'law', intensity }, '[Law] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'law' }, '[Law] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LawState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LawState | null { return this.states.get(entityId) ?? null; }
