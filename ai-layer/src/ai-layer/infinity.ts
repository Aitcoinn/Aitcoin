...
export class Infinity {
  private states: Map<string, InfinityState> = new Map();

  constructor() {
    blockchainBridge.registerModule('infinity');
  }

  private getOrCreate(entityId: string): InfinityState {
  ...
  activate(entityId: string, intensity = 50): InfinityState {
    ...
    blockchainBridge.validateAIOutput('infinity', { entityId, level: state.level });
    logger.info({ entityId, module: 'infinity', intensity }, '[Infinity] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'infinity' }, '[Infinity] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): InfinityState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): InfinityState | null { return this.states.get(entityId) ?? null; }
...
export const infinity = new Infinity();
