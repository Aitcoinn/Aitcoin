...
export class LivingRoom {
  private states: Map<string, LivingRoomState> = new Map();

  constructor() {
    blockchainBridge.registerModule('living_room');
  }

  private getOrCreate(entityId: string): LivingRoomState {
  ...
  activate(entityId: string, intensity = 50): LivingRoomState {
    ...
    blockchainBridge.validateAIOutput('living_room', { entityId, level: state.level });
    logger.info({ entityId, module: 'living_room', intensity }, '[LivingRoom] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'living_room' }, '[LivingRoom] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LivingRoomState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LivingRoomState | null { return this.states.get(entityId) ?? null; }
