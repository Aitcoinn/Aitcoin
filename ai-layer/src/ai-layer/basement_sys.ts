...
export class BasementSys {
  private states: Map<string, BasementSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('basement_sys');
  }

  private getOrCreate(entityId: string): BasementSysState {
  ...
  activate(entityId: string, intensity = 50): BasementSysState {
    ...
    blockchainBridge.validateAIOutput('basement_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'basement_sys', intensity }, '[BasementSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'basement_sys' }, '[BasementSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BasementSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BasementSysState | null { return this.states.get(entityId) ?? null; }
...
export const basementSys = new BasementSys();
export default basementSys;
