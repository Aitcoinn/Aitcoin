...
export class Typhoon {
  private states: Map<string, TyphoonState> = new Map();

  constructor() {
    blockchainBridge.registerModule('typhoon');
  }

  private getOrCreate(entityId: string): TyphoonState {
  ...
  activate(entityId: string, intensity = 50): TyphoonState {
    ...
    blockchainBridge.validateAIOutput('typhoon', { entityId, level: state.level });
    logger.info({ entityId, module: 'typhoon', intensity }, '[Typhoon] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'typhoon' }, '[Typhoon] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TyphoonState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TyphoonState | null { return this.states.get(entityId) ?? null; }
...
export const typhoon = new Typhoon();
