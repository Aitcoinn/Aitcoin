...
export class Geyser {
  private states: Map<string, GeyserState> = new Map();

  constructor() {
    blockchainBridge.registerModule('geyser');
  }

  private getOrCreate(entityId: string): GeyserState {
  ...
  activate(entityId: string, intensity = 50): GeyserState {
    ...
    blockchainBridge.validateAIOutput('geyser', { entityId, level: state.level });
    logger.info({ entityId, module: 'geyser', intensity }, '[Geyser] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'geyser' }, '[Geyser] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): GeyserState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): GeyserState | null { return this.states.get(entityId) ?? null; }
...
export const geyser = new Geyser();
