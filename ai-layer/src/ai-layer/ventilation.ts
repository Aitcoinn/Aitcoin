...
export class Ventilation {
  private states: Map<string, VentilationState> = new Map();

  constructor() {
    blockchainBridge.registerModule('ventilation');
  }

  private getOrCreate(entityId: string): VentilationState {
  ...
  activate(entityId: string, intensity = 50): VentilationState {
    ...
    blockchainBridge.validateAIOutput('ventilation', { entityId, level: state.level });
    logger.info({ entityId, module: 'ventilation', intensity }, '[Ventilation] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ventilation' }, '[Ventilation] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): VentilationState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): VentilationState | null { return this.states.get(entityId) ?? null; }
