...
export class Island {
  private states: Map<string, IslandState> = new Map();

  constructor() {
    blockchainBridge.registerModule('island');
  }

  private getOrCreate(entityId: string): IslandState {
  ...
  activate(entityId: string, intensity = 50): IslandState {
    ...
    blockchainBridge.validateAIOutput('island', { entityId, level: state.level });
    logger.info({ entityId, module: 'island', intensity }, '[Island] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'island' }, '[Island] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): IslandState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): IslandState | null { return this.states.get(entityId) ?? null; }
