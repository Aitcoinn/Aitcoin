...
export class Latency {
  private states: Map<string, LatencyState> = new Map();

  constructor() {
    blockchainBridge.registerModule('latency');
  }

  private getOrCreate(entityId: string): LatencyState {
  ...
  activate(entityId: string, intensity = 50): LatencyState {
    ...
    blockchainBridge.validateAIOutput('latency', { entityId, level: state.level });
    logger.info({ entityId, module: 'latency', intensity }, '[Latency] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'latency' }, '[Latency] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LatencyState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LatencyState | null { return this.states.get(entityId) ?? null; }
