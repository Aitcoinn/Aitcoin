...
export class DecorationSys {
  private states: Map<string, DecorationSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('decoration_sys');
  }

  private getOrCreate(entityId: string): DecorationSysState {
  ...
  activate(entityId: string, intensity = 50): DecorationSysState {
    ...
    blockchainBridge.validateAIOutput('decoration_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'decoration_sys', intensity }, '[DecorationSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'decoration_sys' }, '[DecorationSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DecorationSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DecorationSysState | null { return this.states.get(entityId) ?? null; }
...
export const decorationSys = new DecorationSys();
export default decorationSys;
