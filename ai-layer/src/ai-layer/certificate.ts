...
export class Certificate {
  private states: Map<string, CertificateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('certificate');
  }

  private getOrCreate(entityId: string): CertificateState {
  ...
  activate(entityId: string, intensity = 50): CertificateState {
    ...
    blockchainBridge.validateAIOutput('certificate', { entityId, level: state.level });
    logger.info({ entityId, module: 'certificate', intensity }, '[Certificate] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'certificate' }, '[Certificate] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CertificateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CertificateState | null { return this.states.get(entityId) ?? null; }
