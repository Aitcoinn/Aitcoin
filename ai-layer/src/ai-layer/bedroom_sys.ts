...
export class BedroomSys {
  private states: Map<string, BedroomSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('bedroom_sys');
  }

  private getOrCreate(entityId: string): BedroomSysState {
  ...
  activate(entityId: string, intensity = 50): BedroomSysState {
    ...
    blockchainBridge.validateAIOutput('bedroom_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'bedroom_sys', intensity }, '[BedroomSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'bedroom_sys' }, '[BedroomSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BedroomSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BedroomSysState | null { return this.states.get(entityId) ?? null; }
...
export const bedroomSys = new BedroomSys();
export default bedroomSys;
