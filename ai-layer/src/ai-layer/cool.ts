...
export class Cool {
  private states: Map<string, CoolState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cool');
  }

  private getOrCreate(entityId: string): CoolState {
  ...
  activate(entityId: string, intensity = 50): CoolState {
    ...
    blockchainBridge.validateAIOutput('cool', { entityId, level: state.level });
    logger.info({ entityId, module: 'cool', intensity }, '[Cool] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cool' }, '[Cool] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CoolState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CoolState | null { return this.states.get(entityId) ?? null; }
