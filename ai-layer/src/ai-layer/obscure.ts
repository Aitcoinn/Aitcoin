...
export class Obscure {
  private states: Map<string, ObscureState> = new Map();

  constructor() {
    blockchainBridge.registerModule('obscure');
  }

  private getOrCreate(entityId: string): ObscureState {
  ...
  activate(entityId: string, intensity = 50): ObscureState {
    ...
    blockchainBridge.validateAIOutput('obscure', { entityId, level: state.level });
    logger.info({ entityId, module: 'obscure', intensity }, '[Obscure] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'obscure' }, '[Obscure] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ObscureState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ObscureState | null { return this.states.get(entityId) ?? null; }
...
export const obscure = new Obscure();
