...
export class Trigger {
  ...
  constructor() {
    blockchainBridge.registerModule('trigger');
  }

  private getOrCreate(entityId: string): TriggerState {
  ...
  activate(entityId: string, intensity = 50): TriggerState {
    ...
    blockchainBridge.validateAIOutput('trigger', { entityId, level: state.level });
    logger.info({ entityId, module: 'trigger', intensity }, '[Trigger] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'trigger' }, '[Trigger] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TriggerState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TriggerState | null { return this.states.get(entityId) ?? null; }
