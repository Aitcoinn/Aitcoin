...
export class Cloud {
  private states: Map<string, CloudState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cloud');
  }

  private getOrCreate(entityId: string): CloudState {
  ...
  activate(entityId: string, intensity = 50): CloudState {
    ...
    blockchainBridge.validateAIOutput('cloud', { entityId, level: state.level });
    logger.info({ entityId, module: 'cloud', intensity }, '[Cloud] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cloud' }, '[Cloud] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CloudState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CloudState | null { return this.states.get(entityId) ?? null; }
