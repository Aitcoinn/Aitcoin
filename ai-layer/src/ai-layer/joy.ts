...
export class Joy {
  ...
  private getOrCreate(entityId: string): JoyState {
  ...
  activate(entityId: string, intensity = 50): JoyState {
    ...
    logger.info({ entityId, module: 'joy', intensity }, '[Joy] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'joy' }, '[Joy] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): JoyState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'joy' }, '[Joy] Updated');
    return state;
  }

  getState(entityId: string): JoyState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'joy' }, '[Joy] Reset');
  }
