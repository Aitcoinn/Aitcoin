...
export class See {
  private states: Map<string, SeeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('see');
  }

  private getOrCreate(entityId: string): SeeState {
  ...
  activate(entityId: string, intensity = 50): SeeState {
    ...
    blockchainBridge.validateAIOutput('see', { entityId, level: state.level });
    logger.info({ entityId, module: 'see', intensity }, '[See] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'see' }, '[See] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SeeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SeeState | null { return this.states.get(entityId) ?? null; }
