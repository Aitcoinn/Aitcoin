...
export class DoctorJob {
  private states: Map<string, DoctorJobState> = new Map();

  constructor() {
    blockchainBridge.registerModule('doctor_job');
  }

  private getOrCreate(entityId: string): DoctorJobState {
  ...
  activate(entityId: string, intensity = 50): DoctorJobState {
    ...
    blockchainBridge.validateAIOutput('doctor_job', { entityId, level: state.level });
    logger.info({ entityId, module: 'doctor_job', intensity }, '[DoctorJob] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'doctor_job' }, '[DoctorJob] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DoctorJobState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DoctorJobState | null { return this.states.get(entityId) ?? null; }
...
export const doctorJob = new DoctorJob();
export default doctorJob;
