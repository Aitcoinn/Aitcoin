...
export class Bus {
  private states: Map<string, BusState> = new Map();

  constructor() {
    blockchainBridge.registerModule('bus');
  }

  private getOrCreate(entityId: string): BusState {
  ...
  activate(entityId: string, intensity = 50): BusState {
    ...
    blockchainBridge.validateAIOutput('bus', { entityId, level: state.level });
    logger.info({ entityId, module: 'bus', intensity }, '[Bus] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'bus' }, '[Bus] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BusState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BusState | null { return this.states.get(entityId) ?? null; }
