...
export class IntuitionExp {
  private states: Map<string, IntuitionExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('intuition_exp');
  }

  private getOrCreate(entityId: string): IntuitionExpState {
  ...
  activate(entityId: string, intensity = 50): IntuitionExpState {
    ...
    blockchainBridge.validateAIOutput('intuition_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'intuition_exp', intensity }, '[IntuitionExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'intuition_exp' }, '[IntuitionExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): IntuitionExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): IntuitionExpState | null { return this.states.get(entityId) ?? null; }
...
export const intuitionExp = new IntuitionExp();
export default intuitionExp;
