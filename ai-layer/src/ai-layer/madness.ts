...
export class Madness {
  private states: Map<string, MadnessState> = new Map();

  constructor() {
    blockchainBridge.registerModule('madness');
  }

  private getOrCreate(entityId: string): MadnessState {
  ...
  activate(entityId: string, intensity = 50): MadnessState {
    ...
    blockchainBridge.validateAIOutput('madness', { entityId, level: state.level });
    logger.info({ entityId, module: 'madness', intensity }, '[Madness] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'madness' }, '[Madness] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MadnessState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MadnessState | null { return this.states.get(entityId) ?? null; }
...
export const madness = new Madness();
