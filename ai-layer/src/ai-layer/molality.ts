...
export class Molality {
  private states: Map<string, MolalityState> = new Map();

  constructor() {
    blockchainBridge.registerModule('molality');
  }

  private getOrCreate(entityId: string): MolalityState {
  ...
  activate(entityId: string, intensity = 50): MolalityState {
    ...
    blockchainBridge.validateAIOutput('molality', { entityId, level: state.level });
    logger.info({ entityId, module: 'molality', intensity }, '[Molality] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'molality' }, '[Molality] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MolalityState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MolalityState | null { return this.states.get(entityId) ?? null; }
...
export const molality = new Molality();
