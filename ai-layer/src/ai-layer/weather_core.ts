...
export class WeatherCore {
  private states: Map<string, WeatherCoreState> = new Map();

  constructor() {
    blockchainBridge.registerModule('weather_core');
  }

  private getOrCreate(entityId: string): WeatherCoreState {
  ...
  activate(entityId: string, intensity = 50): WeatherCoreState {
    ...
    blockchainBridge.validateAIOutput('weather_core', { entityId, level: state.level });
    logger.info({ entityId, module: 'weather_core', intensity }, '[WeatherCore] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'weather_core' }, '[WeatherCore] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WeatherCoreState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WeatherCoreState | null { return this.states.get(entityId) ?? null; }
...
export const weatherCore = new WeatherCore();
export default weatherCore;
