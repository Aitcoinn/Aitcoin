...
export class BossJob {
  private states: Map<string, BossJobState> = new Map();

  constructor() {
    blockchainBridge.registerModule('boss_job');
  }

  private getOrCreate(entityId: string): BossJobState {
  ...
  activate(entityId: string, intensity = 50): BossJobState {
    ...
    blockchainBridge.validateAIOutput('boss_job', { entityId, level: state.level });
    logger.info({ entityId, module: 'boss_job', intensity }, '[BossJob] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'boss_job' }, '[BossJob] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BossJobState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BossJobState | null { return this.states.get(entityId) ?? null; }
...
export const bossJob = new BossJob();
