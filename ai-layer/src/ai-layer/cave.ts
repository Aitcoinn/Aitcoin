...
export class Cave {
  private states: Map<string, CaveState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cave');
  }

  private getOrCreate(entityId: string): CaveState {
  ...
  activate(entityId: string, intensity = 50): CaveState {
    ...
    blockchainBridge.validateAIOutput('cave', { entityId, level: state.level });
    logger.info({ entityId, module: 'cave', intensity }, '[Cave] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cave' }, '[Cave] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CaveState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CaveState | null { return this.states.get(entityId) ?? null; }
