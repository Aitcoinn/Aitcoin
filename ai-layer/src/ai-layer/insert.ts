...
export class Insert {
  private states: Map<string, InsertState> = new Map();

  constructor() {
    blockchainBridge.registerModule('insert');
  }

  private getOrCreate(entityId: string): InsertState {
  ...
  activate(entityId: string, intensity = 50): InsertState {
    ...
    blockchainBridge.validateAIOutput('insert', { entityId, level: state.level });
    logger.info({ entityId, module: 'insert', intensity }, '[Insert] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'insert' }, '[Insert] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): InsertState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): InsertState | null { return this.states.get(entityId) ?? null; }
...
export const insert = new Insert();
