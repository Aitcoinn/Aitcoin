...
export class Temperature {
  private states: Map<string, TemperatureState> = new Map();

  constructor() {
    blockchainBridge.registerModule('temperature');
  }

  private getOrCreate(entityId: string): TemperatureState {
  ...
  activate(entityId: string, intensity = 50): TemperatureState {
    ...
    blockchainBridge.validateAIOutput('temperature', { entityId, level: state.level });
    logger.info({ entityId, module: 'temperature', intensity }, '[Temperature] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'temperature' }, '[Temperature] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TemperatureState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TemperatureState | null { return this.states.get(entityId) ?? null; }
