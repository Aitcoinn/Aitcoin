...
export class FaultLine {
  private states: Map<string, FaultLineState> = new Map();

  constructor() {
    blockchainBridge.registerModule('fault_line');
  }

  private getOrCreate(entityId: string): FaultLineState {
  ...
  activate(entityId: string, intensity = 50): FaultLineState {
    ...
    blockchainBridge.validateAIOutput('fault_line', { entityId, level: state.level });
    logger.info({ entityId, module: 'fault_line', intensity }, '[FaultLine] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fault_line' }, '[FaultLine] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FaultLineState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FaultLineState | null { return this.states.get(entityId) ?? null; }
...
export const faultLine = new FaultLine();
