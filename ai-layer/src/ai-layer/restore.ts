...
export class Restore {
  private states: Map<string, RestoreState> = new Map();

  constructor() {
    blockchainBridge.registerModule('restore');
  }

  private getOrCreate(entityId: string): RestoreState {
  ...
  activate(entityId: string, intensity = 50): RestoreState {
    ...
    blockchainBridge.validateAIOutput('restore', { entityId, level: state.level });
    logger.info({ entityId, module: 'restore', intensity }, '[Restore] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'restore' }, '[Restore] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RestoreState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RestoreState | null { return this.states.get(entityId) ?? null; }
