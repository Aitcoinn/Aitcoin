...
export class Array {
  private states: Map<string, ArrayState> = new Map();

  constructor() {
    blockchainBridge.registerModule('array');
  }

  private getOrCreate(entityId: string): ArrayState {
  ...
  activate(entityId: string, intensity = 50): ArrayState {
    ...
    blockchainBridge.validateAIOutput('array', { entityId, level: state.level });
    logger.info({ entityId, module: 'array', intensity }, '[Array] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'array' }, '[Array] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ArrayState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ArrayState | null { return this.states.get(entityId) ?? null; }
