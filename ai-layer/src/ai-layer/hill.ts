...
export class Hill {
  private states: Map<string, HillState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hill');
  }

  private getOrCreate(entityId: string): HillState {
  ...
  activate(entityId: string, intensity = 50): HillState {
    ...
    blockchainBridge.validateAIOutput('hill', { entityId, level: state.level });
    logger.info({ entityId, module: 'hill', intensity }, '[Hill] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hill' }, '[Hill] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HillState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HillState | null { return this.states.get(entityId) ?? null; }
