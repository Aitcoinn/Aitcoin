...
export class Abstraction {
  private states: Map<string, AbstractionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('abstraction');
  }

  private getOrCreate(entityId: string): AbstractionState {
  ...
  activate(entityId: string, intensity = 50): AbstractionState {
    ...
    blockchainBridge.validateAIOutput('abstraction', { entityId, level: state.level });
    logger.info({ entityId, module: 'abstraction', intensity }, '[Abstraction] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'abstraction' }, '[Abstraction] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AbstractionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AbstractionState | null { return this.states.get(entityId) ?? null; }
...
export const abstraction = new Abstraction();
