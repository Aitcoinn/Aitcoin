...
export class ErrorHandle {
  private states: Map<string, ErrorHandleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('error_handle');
  }

  private getOrCreate(entityId: string): ErrorHandleState {
  ...
  activate(entityId: string, intensity = 50): ErrorHandleState {
    ...
    blockchainBridge.validateAIOutput('error_handle', { entityId, level: state.level });
    logger.info({ entityId, module: 'error_handle', intensity }, '[ErrorHandle] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'error_handle' }, '[ErrorHandle] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ErrorHandleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ErrorHandleState | null { return this.states.get(entityId) ?? null; }
