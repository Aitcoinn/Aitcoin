...
export class Reactive {
  ...
  constructor() {
    blockchainBridge.registerModule('reactive');
  }

  private getOrCreate(entityId: string): ReactiveState {
  ...
  activate(entityId: string, intensity = 50): ReactiveState {
    ...
    blockchainBridge.validateAIOutput('reactive', { entityId, level: state.level });
    logger.info({ entityId, module: 'reactive', intensity }, '[Reactive] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'reactive' }, '[Reactive] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ReactiveState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ReactiveState | null { return this.states.get(entityId) ?? null; }
