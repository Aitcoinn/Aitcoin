...
export class NorthPole {
  private states: Map<string, NorthPoleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('north_pole');
  }

  private getOrCreate(entityId: string): NorthPoleState {
  ...
  activate(entityId: string, intensity = 50): NorthPoleState {
    ...
    blockchainBridge.validateAIOutput('north_pole', { entityId, level: state.level });
    logger.info({ entityId, module: 'north_pole', intensity }, '[NorthPole] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'north_pole' }, '[NorthPole] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): NorthPoleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): NorthPoleState | null { return this.states.get(entityId) ?? null; }
...
export const northPole = new NorthPole();
