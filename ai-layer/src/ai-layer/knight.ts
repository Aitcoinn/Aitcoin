...
export class Knight {
  private states: Map<string, KnightState> = new Map();

  private getOrCreate(entityId: string): KnightState {
  ...
  activate(entityId: string, intensity = 50): KnightState {
    ...
    logger.info({ entityId, module: 'knight', intensity }, '[Knight] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'knight' }, '[Knight] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): KnightState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'knight' }, '[Knight] Updated');
    return state;
  }

  getState(entityId: string): KnightState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'knight' }, '[Knight] Reset');
  }
