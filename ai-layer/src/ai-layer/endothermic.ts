...
export class Endothermic {
  private states: Map<string, EndothermicState> = new Map();

  constructor() {
    blockchainBridge.registerModule('endothermic');
  }

  private getOrCreate(entityId: string): EndothermicState {
  ...
  activate(entityId: string, intensity = 50): EndothermicState {
    ...
    blockchainBridge.validateAIOutput('endothermic', { entityId, level: state.level });
    logger.info({ entityId, module: 'endothermic', intensity }, '[Endothermic] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'endothermic' }, '[Endothermic] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EndothermicState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): EndothermicState | null { return this.states.get(entityId) ?? null; }
...
export const endothermic = new Endothermic();
export default endothermic;
