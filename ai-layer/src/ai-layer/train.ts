...
export class Train {
  private states: Map<string, TrainState> = new Map();

  constructor() {
    blockchainBridge.registerModule('train');
  }

  private getOrCreate(entityId: string): TrainState {
  ...
  activate(entityId: string, intensity = 50): TrainState {
    ...
    blockchainBridge.validateAIOutput('train', { entityId, level: state.level });
    logger.info({ entityId, module: 'train', intensity }, '[Train] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'train' }, '[Train] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TrainState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TrainState | null { return this.states.get(entityId) ?? null; }
