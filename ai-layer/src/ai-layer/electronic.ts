...
export class Electronic {
  private states: Map<string, ElectronicState> = new Map();

  constructor() {
    blockchainBridge.registerModule('electronic');
  }

  private getOrCreate(entityId: string): ElectronicState {
  ...
  activate(entityId: string, intensity = 50): ElectronicState {
    ...
    blockchainBridge.validateAIOutput('electronic', { entityId, level: state.level });
    logger.info({ entityId, module: 'electronic', intensity }, '[Electronic] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'electronic' }, '[Electronic] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ElectronicState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ElectronicState | null { return this.states.get(entityId) ?? null; }
...
export const electronic = new Electronic();
