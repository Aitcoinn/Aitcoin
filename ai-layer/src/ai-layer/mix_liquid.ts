...
export class MixLiquid {
  private states: Map<string, MixLiquidState> = new Map();

  constructor() {
    blockchainBridge.registerModule('mix_liquid');
  }

  private getOrCreate(entityId: string): MixLiquidState {
  ...
  activate(entityId: string, intensity = 50): MixLiquidState {
    ...
    blockchainBridge.validateAIOutput('mix_liquid', { entityId, level: state.level });
    logger.info({ entityId, module: 'mix_liquid', intensity }, '[MixLiquid] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'mix_liquid' }, '[MixLiquid] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MixLiquidState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MixLiquidState | null { return this.states.get(entityId) ?? null; }
...
export const mixLiquid = new MixLiquid();
export default mixLiquid;
