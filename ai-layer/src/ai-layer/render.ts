...
export class Render {
  ...
  constructor() {
    blockchainBridge.registerModule('render');
  }

  private getOrCreate(entityId: string): RenderState {
  ...
  activate(entityId: string, intensity = 50): RenderState {
    ...
    blockchainBridge.validateAIOutput('render', { entityId, level: state.level });
    logger.info({ entityId, module: 'render', intensity }, '[Render] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'render' }, '[Render] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RenderState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RenderState | null { return this.states.get(entityId) ?? null; }
