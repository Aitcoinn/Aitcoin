...
export class SystemCore {
  private states: Map<string, SystemCoreState> = new Map();

  constructor() {
    blockchainBridge.registerModule('system_core');
  }

  private getOrCreate(entityId: string): SystemCoreState {
  ...
  activate(entityId: string, intensity = 50): SystemCoreState {
    ...
    blockchainBridge.validateAIOutput('system_core', { entityId, level: state.level });
    logger.info({ entityId, module: 'system_core', intensity }, '[SystemCore] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'system_core' }, '[SystemCore] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SystemCoreState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SystemCoreState | null { return this.states.get(entityId) ?? null; }
