...
export class Lake {
  private states: Map<string, LakeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('lake');
  }

  private getOrCreate(entityId: string): LakeState {
  ...
  activate(entityId: string, intensity = 50): LakeState {
    ...
    blockchainBridge.validateAIOutput('lake', { entityId, level: state.level });
    logger.info({ entityId, module: 'lake', intensity }, '[Lake] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'lake' }, '[Lake] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LakeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LakeState | null { return this.states.get(entityId) ?? null; }
