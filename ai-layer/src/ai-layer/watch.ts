...
export class Watch {
  private states: Map<string, WatchState> = new Map();

  constructor() {
    blockchainBridge.registerModule('watch');
  }

  private getOrCreate(entityId: string): WatchState {
  ...
  activate(entityId: string, intensity = 50): WatchState {
    ...
    blockchainBridge.validateAIOutput('watch', { entityId, level: state.level });
    logger.info({ entityId, module: 'watch', intensity }, '[Watch] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'watch' }, '[Watch] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WatchState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WatchState | null { return this.states.get(entityId) ?? null; }
