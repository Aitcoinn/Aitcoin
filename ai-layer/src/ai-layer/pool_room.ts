...
export class PoolRoom {
  private states: Map<string, PoolRoomState> = new Map();

  constructor() {
    blockchainBridge.registerModule('pool_room');
  }

  private getOrCreate(entityId: string): PoolRoomState {
  ...
  activate(entityId: string, intensity = 50): PoolRoomState {
    ...
    blockchainBridge.validateAIOutput('pool_room', { entityId, level: state.level });
    logger.info({ entityId, module: 'pool_room', intensity }, '[PoolRoom] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'pool_room' }, '[PoolRoom] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PoolRoomState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PoolRoomState | null { return this.states.get(entityId) ?? null; }
...
export const poolRoom = new PoolRoom();
