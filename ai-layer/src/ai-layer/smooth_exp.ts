...
export class SmoothExp {
  private states: Map<string, SmoothExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('smooth_exp');
  }

  private getOrCreate(entityId: string): SmoothExpState {
  ...
  activate(entityId: string, intensity = 50): SmoothExpState {
    ...
    blockchainBridge.validateAIOutput('smooth_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'smooth_exp', intensity }, '[SmoothExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'smooth_exp' }, '[SmoothExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SmoothExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SmoothExpState | null { return this.states.get(entityId) ?? null; }
...
export const smoothExp = new SmoothExp();
export default smoothExp;
