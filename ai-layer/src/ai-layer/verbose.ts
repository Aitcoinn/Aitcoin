...
export class Verbose {
  private states: Map<string, VerboseState> = new Map();

  constructor() {
    blockchainBridge.registerModule('verbose');
  }

  private getOrCreate(entityId: string): VerboseState {
  ...
  activate(entityId: string, intensity = 50): VerboseState {
    ...
    blockchainBridge.validateAIOutput('verbose', { entityId, level: state.level });
    logger.info({ entityId, module: 'verbose', intensity }, '[Verbose] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'verbose' }, '[Verbose] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): VerboseState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): VerboseState | null { return this.states.get(entityId) ?? null; }
...
export const verbose = new Verbose();
