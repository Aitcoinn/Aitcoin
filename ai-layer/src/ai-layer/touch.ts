...
export class Touch {
  private states: Map<string, TouchState> = new Map();

  constructor() {
    blockchainBridge.registerModule('touch');
  }

  private getOrCreate(entityId: string): TouchState {
  ...
  activate(entityId: string, intensity = 50): TouchState {
    ...
    blockchainBridge.validateAIOutput('touch', { entityId, level: state.level });
    logger.info({ entityId, module: 'touch', intensity }, '[Touch] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'touch' }, '[Touch] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TouchState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TouchState | null { return this.states.get(entityId) ?? null; }
