...
export class SurfaceTension {
  private states: Map<string, SurfaceTensionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('surface_tension');
  }

  private getOrCreate(entityId: string): SurfaceTensionState {
  ...
  activate(entityId: string, intensity = 50): SurfaceTensionState {
    ...
    blockchainBridge.validateAIOutput('surface_tension', { entityId, level: state.level });
    logger.info({ entityId, module: 'surface_tension', intensity }, '[SurfaceTension] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'surface_tension' }, '[SurfaceTension] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SurfaceTensionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SurfaceTensionState | null { return this.states.get(entityId) ?? null; }
...
export const surfaceTension = new SurfaceTension();
