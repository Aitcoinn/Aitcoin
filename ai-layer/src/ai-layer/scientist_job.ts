...
export class ScientistJob {
  private states: Map<string, ScientistJobState> = new Map();

  constructor() {
    blockchainBridge.registerModule('scientist_job');
  }

  private getOrCreate(entityId: string): ScientistJobState {
  ...
  activate(entityId: string, intensity = 50): ScientistJobState {
    ...
    blockchainBridge.validateAIOutput('scientist_job', { entityId, level: state.level });
    logger.info({ entityId, module: 'scientist_job', intensity }, '[ScientistJob] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'scientist_job' }, '[ScientistJob] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ScientistJobState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ScientistJobState | null { return this.states.get(entityId) ?? null; }
...
export const scientistJob = new ScientistJob();
export default scientistJob;
