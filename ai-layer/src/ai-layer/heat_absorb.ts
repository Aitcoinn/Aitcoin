...
export class HeatAbsorb {
  private states: Map<string, HeatAbsorbState> = new Map();

  constructor() {
    blockchainBridge.registerModule('heat_absorb');
  }

  private getOrCreate(entityId: string): HeatAbsorbState {
  ...
  activate(entityId: string, intensity = 50): HeatAbsorbState {
    ...
    blockchainBridge.validateAIOutput('heat_absorb', { entityId, level: state.level });
    logger.info({ entityId, module: 'heat_absorb', intensity }, '[HeatAbsorb] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'heat_absorb' }, '[HeatAbsorb] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HeatAbsorbState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HeatAbsorbState | null { return this.states.get(entityId) ?? null; }
...
export const heatAbsorb = new HeatAbsorb();
export default heatAbsorb;
