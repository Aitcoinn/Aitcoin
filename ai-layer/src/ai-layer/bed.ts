...
export class Bed {
  private states: Map<string, BedState> = new Map();

  constructor() {
    blockchainBridge.registerModule('bed');
  }

  private getOrCreate(entityId: string): BedState {
  ...
  activate(entityId: string, intensity = 50): BedState {
    ...
    blockchainBridge.validateAIOutput('bed', { entityId, level: state.level });
    logger.info({ entityId, module: 'bed', intensity }, '[Bed] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'bed' }, '[Bed] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BedState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BedState | null { return this.states.get(entityId) ?? null; }
