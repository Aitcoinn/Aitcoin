...
export class Buoyancy {
  private states: Map<string, BuoyancyState> = new Map();

  constructor() {
    blockchainBridge.registerModule('buoyancy');
  }

  private getOrCreate(entityId: string): BuoyancyState {
  ...
  activate(entityId: string, intensity = 50): BuoyancyState {
    ...
    blockchainBridge.validateAIOutput('buoyancy', { entityId, level: state.level });
    logger.info({ entityId, module: 'buoyancy', intensity }, '[Buoyancy] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'buoyancy' }, '[Buoyancy] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BuoyancyState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BuoyancyState | null { return this.states.get(entityId) ?? null; }
...
export const buoyancy = new Buoyancy();
