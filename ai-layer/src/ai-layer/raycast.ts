...
export class Raycast {
  private states: Map<string, RaycastState> = new Map();

  constructor() {
    blockchainBridge.registerModule('raycast');
  }

  private getOrCreate(entityId: string): RaycastState {
  ...
  activate(entityId: string, intensity = 50): RaycastState {
    ...
    blockchainBridge.validateAIOutput('raycast', { entityId, level: state.level });
    logger.info({ entityId, module: 'raycast', intensity }, '[Raycast] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'raycast' }, '[Raycast] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RaycastState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RaycastState | null { return this.states.get(entityId) ?? null; }
