...
export class SunlightAtm {
  private states: Map<string, SunlightAtmState> = new Map();

  constructor() {
    blockchainBridge.registerModule('sunlight_atm');
  }

  private getOrCreate(entityId: string): SunlightAtmState {
  ...
  activate(entityId: string, intensity = 50): SunlightAtmState {
    ...
    blockchainBridge.validateAIOutput('sunlight_atm', { entityId, level: state.level });
    logger.info({ entityId, module: 'sunlight_atm', intensity }, '[SunlightAtm] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'sunlight_atm' }, '[SunlightAtm] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SunlightAtmState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SunlightAtmState | null { return this.states.get(entityId) ?? null; }
...
export const sunlightAtm = new SunlightAtm();
export default sunlightAtm;
