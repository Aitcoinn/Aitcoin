...
export class Cart {
  private states: Map<string, CartState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cart');
  }

  private getOrCreate(entityId: string): CartState {
  ...
  activate(entityId: string, intensity = 50): CartState {
    ...
    blockchainBridge.validateAIOutput('cart', { entityId, level: state.level });
    logger.info({ entityId, module: 'cart', intensity }, '[Cart] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cart' }, '[Cart] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CartState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CartState | null { return this.states.get(entityId) ?? null; }
