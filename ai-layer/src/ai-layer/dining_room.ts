...
export class DiningRoom {
  private states: Map<string, DiningRoomState> = new Map();

  constructor() {
    blockchainBridge.registerModule('dining_room');
  }

  private getOrCreate(entityId: string): DiningRoomState {
  ...
  activate(entityId: string, intensity = 50): DiningRoomState {
    ...
    blockchainBridge.validateAIOutput('dining_room', { entityId, level: state.level });
    logger.info({ entityId, module: 'dining_room', intensity }, '[DiningRoom] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'dining_room' }, '[DiningRoom] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DiningRoomState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DiningRoomState | null { return this.states.get(entityId) ?? null; }
...
export const diningRoom = new DiningRoom();
