...
export class Fps {
  private states: Map<string, FpsState> = new Map();

  constructor() {
    blockchainBridge.registerModule('fps');
  }

  private getOrCreate(entityId: string): FpsState {
  ...
  activate(entityId: string, intensity = 50): FpsState {
    ...
    blockchainBridge.validateAIOutput('fps', { entityId, level: state.level });
    logger.info({ entityId, module: 'fps', intensity }, '[Fps] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fps' }, '[Fps] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FpsState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FpsState | null { return this.states.get(entityId) ?? null; }
