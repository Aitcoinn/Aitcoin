...
export class FullMoon {
  private states: Map<string, FullMoonState> = new Map();

  constructor() {
    blockchainBridge.registerModule('full_moon');
  }

  private getOrCreate(entityId: string): FullMoonState {
  ...
  activate(entityId: string, intensity = 50): FullMoonState {
    ...
    blockchainBridge.validateAIOutput('full_moon', { entityId, level: state.level });
    logger.info({ entityId, module: 'full_moon', intensity }, '[FullMoon] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'full_moon' }, '[FullMoon] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FullMoonState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FullMoonState | null { return this.states.get(entityId) ?? null; }
...
export const fullMoon = new FullMoon();
