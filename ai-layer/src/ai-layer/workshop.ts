...
export class Workshop {
  private states: Map<string, WorkshopState> = new Map();

  constructor() {
    blockchainBridge.registerModule('workshop');
  }

  private getOrCreate(entityId: string): WorkshopState {
  ...
  activate(entityId: string, intensity = 50): WorkshopState {
    ...
    blockchainBridge.validateAIOutput('workshop', { entityId, level: state.level });
    logger.info({ entityId, module: 'workshop', intensity }, '[Workshop] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'workshop' }, '[Workshop] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WorkshopState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WorkshopState | null { return this.states.get(entityId) ?? null; }
