...
export class EventEngine {
  private states: Map<string, EventEngineState> = new Map();

  constructor() {
    blockchainBridge.registerModule('event_engine');
  }

  private getOrCreate(entityId: string): EventEngineState {
  ...
  activate(entityId: string, intensity = 50): EventEngineState {
    ...
    blockchainBridge.validateAIOutput('event_engine', { entityId, level: state.level });
    logger.info({ entityId, module: 'event_engine', intensity }, '[EventEngine] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'event_engine' }, '[EventEngine] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EventEngineState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): EventEngineState | null { return this.states.get(entityId) ?? null; }
