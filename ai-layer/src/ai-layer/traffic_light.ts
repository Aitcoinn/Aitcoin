...
export class TrafficLight {
  private states: Map<string, TrafficLightState> = new Map();

  constructor() {
    blockchainBridge.registerModule('traffic_light');
  }

  private getOrCreate(entityId: string): TrafficLightState {
  ...
  activate(entityId: string, intensity = 50): TrafficLightState {
    ...
    blockchainBridge.validateAIOutput('traffic_light', { entityId, level: state.level });
    logger.info({ entityId, module: 'traffic_light', intensity }, '[TrafficLight] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'traffic_light' }, '[TrafficLight] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TrafficLightState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TrafficLightState | null { return this.states.get(entityId) ?? null; }
...
export const trafficLight = new TrafficLight();
