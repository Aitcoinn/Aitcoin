...
export class Fan {
  private states: Map<string, FanState> = new Map();

  constructor() {
    blockchainBridge.registerModule('fan');
  }

  private getOrCreate(entityId: string): FanState {
  ...
  activate(entityId: string, intensity = 50): FanState {
    ...
    blockchainBridge.validateAIOutput('fan', { entityId, level: state.level });
    logger.info({ entityId, module: 'fan', intensity }, '[Fan] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fan' }, '[Fan] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FanState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FanState | null { return this.states.get(entityId) ?? null; }
