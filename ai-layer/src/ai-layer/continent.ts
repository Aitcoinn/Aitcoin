...
export class Continent {
  private states: Map<string, ContinentState> = new Map();

  constructor() {
    blockchainBridge.registerModule('continent');
  }

  private getOrCreate(entityId: string): ContinentState {
  ...
  activate(entityId: string, intensity = 50): ContinentState {
    ...
    blockchainBridge.validateAIOutput('continent', { entityId, level: state.level });
    logger.info({ entityId, module: 'continent', intensity }, '[Continent] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'continent' }, '[Continent] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ContinentState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ContinentState | null { return this.states.get(entityId) ?? null; }
...
export const continent = new Continent();
