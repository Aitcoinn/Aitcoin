...
export class Norm {
  private states: Map<string, NormState> = new Map();

  private getOrCreate(entityId: string): NormState {
  ...
  activate(entityId: string, intensity = 50): NormState {
    ...
    logger.info({ entityId, module: 'norm', intensity }, '[Norm] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'norm' }, '[Norm] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): NormState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'norm' }, '[Norm] Updated');
    return state;
  }

  getState(entityId: string): NormState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'norm' }, '[Norm] Reset');
  }
