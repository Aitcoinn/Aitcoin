...
export class Existence {
  private states: Map<string, ExistenceState> = new Map();

  constructor() {
    blockchainBridge.registerModule('existence');
  }

  private getOrCreate(entityId: string): ExistenceState {
  ...
  activate(entityId: string, intensity = 50): ExistenceState {
    ...
    blockchainBridge.validateAIOutput('existence', { entityId, level: state.level });
    logger.info({ entityId, module: 'existence', intensity }, '[Existence] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'existence' }, '[Existence] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ExistenceState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ExistenceState | null { return this.states.get(entityId) ?? null; }
...
export const existence = new Existence();
