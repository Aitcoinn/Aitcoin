...
export class PoliceJob {
  private states: Map<string, PoliceJobState> = new Map();

  constructor() {
    blockchainBridge.registerModule('police_job');
  }

  private getOrCreate(entityId: string): PoliceJobState {
  ...
  activate(entityId: string, intensity = 50): PoliceJobState {
    ...
    blockchainBridge.validateAIOutput('police_job', { entityId, level: state.level });
    logger.info({ entityId, module: 'police_job', intensity }, '[PoliceJob] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'police_job' }, '[PoliceJob] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PoliceJobState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PoliceJobState | null { return this.states.get(entityId) ?? null; }
...
export const policeJob = new PoliceJob();
export default policeJob;
