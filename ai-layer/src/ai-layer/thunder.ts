...
export class Thunder {
  private states: Map<string, ThunderState> = new Map();

  constructor() {
    blockchainBridge.registerModule('thunder');
  }

  private getOrCreate(entityId: string): ThunderState {
  ...
  activate(entityId: string, intensity = 50): ThunderState {
    ...
    blockchainBridge.validateAIOutput('thunder', { entityId, level: state.level });
    logger.info({ entityId, module: 'thunder', intensity }, '[Thunder] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'thunder' }, '[Thunder] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ThunderState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ThunderState | null { return this.states.get(entityId) ?? null; }
...
export const thunder = new Thunder();
