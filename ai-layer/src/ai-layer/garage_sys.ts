...
export class GarageSys {
  private states: Map<string, GarageSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('garage_sys');
  }

  private getOrCreate(entityId: string): GarageSysState {
  ...
  activate(entityId: string, intensity = 50): GarageSysState {
    ...
    blockchainBridge.validateAIOutput('garage_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'garage_sys', intensity }, '[GarageSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'garage_sys' }, '[GarageSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): GarageSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): GarageSysState | null { return this.states.get(entityId) ?? null; }
...
export const garageSys = new GarageSys();
export default garageSys;
