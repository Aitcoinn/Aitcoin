...
export class SeasonChange {
  private states: Map<string, SeasonChangeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('season_change');
  }

  private getOrCreate(entityId: string): SeasonChangeState {
  ...
  activate(entityId: string, intensity = 50): SeasonChangeState {
    ...
    blockchainBridge.validateAIOutput('season_change', { entityId, level: state.level });
    logger.info({ entityId, module: 'season_change', intensity }, '[SeasonChange] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'season_change' }, '[SeasonChange] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SeasonChangeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SeasonChangeState | null { return this.states.get(entityId) ?? null; }
...
export const seasonChange = new SeasonChange();
