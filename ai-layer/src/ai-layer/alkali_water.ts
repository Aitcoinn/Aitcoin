...
export class AlkaliWater {
  private states: Map<string, AlkaliWaterState> = new Map();

  constructor() {
    blockchainBridge.registerModule('alkali_water');
  }

  private getOrCreate(entityId: string): AlkaliWaterState {
  ...
  activate(entityId: string, intensity = 50): AlkaliWaterState {
    ...
    blockchainBridge.validateAIOutput('alkali_water', { entityId, level: state.level });
    logger.info({ entityId, module: 'alkali_water', intensity }, '[AlkaliWater] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'alkali_water' }, '[AlkaliWater] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AlkaliWaterState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AlkaliWaterState | null { return this.states.get(entityId) ?? null; }
...
export const alkaliWater = new AlkaliWater();
export default alkaliWater;
