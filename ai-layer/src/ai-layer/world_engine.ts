...
export class WorldEngine {
  private states: Map<string, WorldEngineState> = new Map();

  constructor() {
    blockchainBridge.registerModule('world_engine');
  }

  private getOrCreate(entityId: string): WorldEngineState {
  ...
  activate(entityId: string, intensity = 50): WorldEngineState {
    ...
    blockchainBridge.validateAIOutput('world_engine', { entityId, level: state.level });
    logger.info({ entityId, module: 'world_engine', intensity }, '[WorldEngine] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'world_engine' }, '[WorldEngine] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WorldEngineState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WorldEngineState | null { return this.states.get(entityId) ?? null; }
