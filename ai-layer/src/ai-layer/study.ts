...
export class Study {
  private states: Map<string, StudyState> = new Map();

  private getOrCreate(entityId: string): StudyState {
  ...
  activate(entityId: string, intensity = 50): StudyState {
    ...
    logger.info({ entityId, module: 'study', intensity }, '[Study] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'study' }, '[Study] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): StudyState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'study' }, '[Study] Updated');
    return state;
  }

  getState(entityId: string): StudyState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'study' }, '[Study] Reset');
  }
