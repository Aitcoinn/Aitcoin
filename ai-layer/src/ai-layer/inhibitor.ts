...
export class Inhibitor {
  private states: Map<string, InhibitorState> = new Map();

  constructor() {
    blockchainBridge.registerModule('inhibitor');
  }

  private getOrCreate(entityId: string): InhibitorState {
  ...
  activate(entityId: string, intensity = 50): InhibitorState {
    ...
    blockchainBridge.validateAIOutput('inhibitor', { entityId, level: state.level });
    logger.info({ entityId, module: 'inhibitor', intensity }, '[Inhibitor] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'inhibitor' }, '[Inhibitor] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): InhibitorState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): InhibitorState | null { return this.states.get(entityId) ?? null; }
...
export const inhibitor = new Inhibitor();
