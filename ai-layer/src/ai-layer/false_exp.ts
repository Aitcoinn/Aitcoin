...
export class FalseExp {
  private states: Map<string, FalseExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('false_exp');
  }

  private getOrCreate(entityId: string): FalseExpState {
  ...
  activate(entityId: string, intensity = 50): FalseExpState {
    ...
    blockchainBridge.validateAIOutput('false_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'false_exp', intensity }, '[FalseExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'false_exp' }, '[FalseExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FalseExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FalseExpState | null { return this.states.get(entityId) ?? null; }
...
export const falseExp = new FalseExp();
export default falseExp;
