...
export class Deposition {
  private states: Map<string, DepositionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('deposition');
  }

  private getOrCreate(entityId: string): DepositionState {
  ...
  activate(entityId: string, intensity = 50): DepositionState {
    ...
    blockchainBridge.validateAIOutput('deposition', { entityId, level: state.level });
    logger.info({ entityId, module: 'deposition', intensity }, '[Deposition] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'deposition' }, '[Deposition] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DepositionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DepositionState | null { return this.states.get(entityId) ?? null; }
...
export const deposition = new Deposition();
