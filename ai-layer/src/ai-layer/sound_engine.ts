...
export class SoundEngine {
  private states: Map<string, SoundEngineState> = new Map();

  constructor() {
    blockchainBridge.registerModule('sound_engine');
  }

  private getOrCreate(entityId: string): SoundEngineState {
  ...
  activate(entityId: string, intensity = 50): SoundEngineState {
    ...
    blockchainBridge.validateAIOutput('sound_engine', { entityId, level: state.level });
    logger.info({ entityId, module: 'sound_engine', intensity }, '[SoundEngine] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'sound_engine' }, '[SoundEngine] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SoundEngineState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SoundEngineState | null { return this.states.get(entityId) ?? null; }
