...
export class Memory {
  ...
  private getOrCreate(entityId: string): MemoryState {
  ...
  activate(entityId: string, intensity = 50): MemoryState {
    ...
    logger.info({ entityId, module: 'memory', intensity }, '[Memory] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'memory' }, '[Memory] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MemoryState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'memory' }, '[Memory] Updated');
    return state;
  }

  getState(entityId: string): MemoryState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'memory' }, '[Memory] Reset');
  }
