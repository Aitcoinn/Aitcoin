...
export class Optimize {
  private states: Map<string, OptimizeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('optimize');
  }

  private getOrCreate(entityId: string): OptimizeState {
  ...
  activate(entityId: string, intensity = 50): OptimizeState {
    ...
    blockchainBridge.validateAIOutput('optimize', { entityId, level: state.level });
    logger.info({ entityId, module: 'optimize', intensity }, '[Optimize] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'optimize' }, '[Optimize] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): OptimizeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): OptimizeState | null { return this.states.get(entityId) ?? null; }
...
export const optimize = new Optimize();
