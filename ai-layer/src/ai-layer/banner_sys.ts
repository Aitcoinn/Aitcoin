...
export class BannerSys {
  private states: Map<string, BannerSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('banner_sys');
  }

  private getOrCreate(entityId: string): BannerSysState {
  ...
  activate(entityId: string, intensity = 50): BannerSysState {
    ...
    blockchainBridge.validateAIOutput('banner_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'banner_sys', intensity }, '[BannerSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'banner_sys' }, '[BannerSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BannerSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BannerSysState | null { return this.states.get(entityId) ?? null; }
...
export const bannerSys = new BannerSys();
export default bannerSys;
