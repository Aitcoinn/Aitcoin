...
export class Tropical {
  private states: Map<string, TropicalState> = new Map();

  constructor() {
    blockchainBridge.registerModule('tropical');
  }

  private getOrCreate(entityId: string): TropicalState {
  ...
  activate(entityId: string, intensity = 50): TropicalState {
    ...
    blockchainBridge.validateAIOutput('tropical', { entityId, level: state.level });
    logger.info({ entityId, module: 'tropical', intensity }, '[Tropical] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'tropical' }, '[Tropical] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TropicalState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TropicalState | null { return this.states.get(entityId) ?? null; }
...
export const tropical = new Tropical();
