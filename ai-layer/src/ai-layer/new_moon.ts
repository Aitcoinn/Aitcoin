...
export class NewMoon {
  private states: Map<string, NewMoonState> = new Map();

  constructor() {
    blockchainBridge.registerModule('new_moon');
  }

  private getOrCreate(entityId: string): NewMoonState {
  ...
  activate(entityId: string, intensity = 50): NewMoonState {
    ...
    blockchainBridge.validateAIOutput('new_moon', { entityId, level: state.level });
    logger.info({ entityId, module: 'new_moon', intensity }, '[NewMoon] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'new_moon' }, '[NewMoon] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): NewMoonState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): NewMoonState | null { return this.states.get(entityId) ?? null; }
...
export const newMoon = new NewMoon();
