...
export class Shower {
  private states: Map<string, ShowerState> = new Map();

  constructor() {
    blockchainBridge.registerModule('shower');
  }

  private getOrCreate(entityId: string): ShowerState {
  ...
  activate(entityId: string, intensity = 50): ShowerState {
    ...
    blockchainBridge.validateAIOutput('shower', { entityId, level: state.level });
    logger.info({ entityId, module: 'shower', intensity }, '[Shower] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'shower' }, '[Shower] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ShowerState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ShowerState | null { return this.states.get(entityId) ?? null; }
...
export const shower = new Shower();
