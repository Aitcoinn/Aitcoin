...
export class GlassObj {
  private states: Map<string, GlassObjState> = new Map();

  constructor() {
    blockchainBridge.registerModule('glass_obj');
  }

  private getOrCreate(entityId: string): GlassObjState {
  ...
  activate(entityId: string, intensity = 50): GlassObjState {
    ...
    blockchainBridge.validateAIOutput('glass_obj', { entityId, level: state.level });
    logger.info({ entityId, module: 'glass_obj', intensity }, '[GlassObj] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'glass_obj' }, '[GlassObj] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): GlassObjState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): GlassObjState | null { return this.states.get(entityId) ?? null; }
...
export const glassObj = new GlassObj();
export default glassObj;
