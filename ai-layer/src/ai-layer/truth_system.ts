...
export class TruthSystem {
  private states: Map<string, TruthSystemState> = new Map();

  constructor() {
    blockchainBridge.registerModule('truth_system');
  }

  private getOrCreate(entityId: string): TruthSystemState {
  ...
  activate(entityId: string, intensity = 50): TruthSystemState {
    ...
    blockchainBridge.validateAIOutput('truth_system', { entityId, level: state.level });
    logger.info({ entityId, module: 'truth_system', intensity }, '[TruthSystem] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'truth_system' }, '[TruthSystem] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TruthSystemState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TruthSystemState | null { return this.states.get(entityId) ?? null; }
...
export const truthSystem = new TruthSystem();
export default truthSystem;
