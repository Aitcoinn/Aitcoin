...
export class ExperienceCore {
  private states: Map<string, ExperienceCoreState> = new Map();

  constructor() {
    blockchainBridge.registerModule('experience_core');
  }

  private getOrCreate(entityId: string): ExperienceCoreState {
  ...
  activate(entityId: string, intensity = 50): ExperienceCoreState {
    ...
    blockchainBridge.validateAIOutput('experience_core', { entityId, level: state.level });
    logger.info({ entityId, module: 'experience_core', intensity }, '[ExperienceCore] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'experience_core' }, '[ExperienceCore] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ExperienceCoreState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ExperienceCoreState | null { return this.states.get(entityId) ?? null; }
...
export const experienceCore = new ExperienceCore();
export default experienceCore;
