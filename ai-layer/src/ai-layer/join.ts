...
export class Join {
  private states: Map<string, JoinState> = new Map();

  constructor() {
    blockchainBridge.registerModule('join');
  }

  private getOrCreate(entityId: string): JoinState {
  ...
  activate(entityId: string, intensity = 50): JoinState {
    ...
    blockchainBridge.validateAIOutput('join', { entityId, level: state.level });
    logger.info({ entityId, module: 'join', intensity }, '[Join] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'join' }, '[Join] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): JoinState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): JoinState | null { return this.states.get(entityId) ?? null; }
