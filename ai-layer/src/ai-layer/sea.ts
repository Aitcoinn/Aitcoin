...
export class Sea {
  private states: Map<string, SeaState> = new Map();

  constructor() {
    blockchainBridge.registerModule('sea');
  }

  private getOrCreate(entityId: string): SeaState {
  ...
  activate(entityId: string, intensity = 50): SeaState {
    ...
    blockchainBridge.validateAIOutput('sea', { entityId, level: state.level });
    logger.info({ entityId, module: 'sea', intensity }, '[Sea] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'sea' }, '[Sea] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SeaState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SeaState | null { return this.states.get(entityId) ?? null; }
