...
export class Role {
  private states: Map<string, RoleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('role');
  }

  private getOrCreate(entityId: string): RoleState {
  ...
  activate(entityId: string, intensity = 50): RoleState {
    ...
    blockchainBridge.validateAIOutput('role', { entityId, level: state.level });
    logger.info({ entityId, module: 'role', intensity }, '[Role] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'role' }, '[Role] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RoleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RoleState | null { return this.states.get(entityId) ?? null; }
