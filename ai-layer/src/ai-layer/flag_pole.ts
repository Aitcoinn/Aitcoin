...
export class FlagPole {
  private states: Map<string, FlagPoleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('flag_pole');
  }

  private getOrCreate(entityId: string): FlagPoleState {
  ...
  activate(entityId: string, intensity = 50): FlagPoleState {
    ...
    blockchainBridge.validateAIOutput('flag_pole', { entityId, level: state.level });
    logger.info({ entityId, module: 'flag_pole', intensity }, '[FlagPole] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'flag_pole' }, '[FlagPole] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FlagPoleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FlagPoleState | null { return this.states.get(entityId) ?? null; }
...
export const flagPole = new FlagPole();
