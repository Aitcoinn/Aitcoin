...
export class Dead {
  private states: Map<string, DeadState> = new Map();

  constructor() {
    blockchainBridge.registerModule('dead');
  }

  private getOrCreate(entityId: string): DeadState {
  ...
  activate(entityId: string, intensity = 50): DeadState {
    ...
    blockchainBridge.validateAIOutput('dead', { entityId, level: state.level });
    logger.info({ entityId, module: 'dead', intensity }, '[Dead] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'dead' }, '[Dead] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DeadState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DeadState | null { return this.states.get(entityId) ?? null; }
