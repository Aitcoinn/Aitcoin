...
export class Gale {
  private states: Map<string, GaleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('gale');
  }

  private getOrCreate(entityId: string): GaleState {
  ...
  activate(entityId: string, intensity = 50): GaleState {
    ...
    blockchainBridge.validateAIOutput('gale', { entityId, level: state.level });
    logger.info({ entityId, module: 'gale', intensity }, '[Gale] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'gale' }, '[Gale] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): GaleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): GaleState | null { return this.states.get(entityId) ?? null; }
