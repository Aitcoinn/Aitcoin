...
export class Hail {
  private states: Map<string, HailState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hail');
  }

  private getOrCreate(entityId: string): HailState {
  ...
  activate(entityId: string, intensity = 50): HailState {
    ...
    blockchainBridge.validateAIOutput('hail', { entityId, level: state.level });
    logger.info({ entityId, module: 'hail', intensity }, '[Hail] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hail' }, '[Hail] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HailState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HailState | null { return this.states.get(entityId) ?? null; }
