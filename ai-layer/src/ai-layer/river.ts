...
export class River {
  private states: Map<string, RiverState> = new Map();

  constructor() {
    blockchainBridge.registerModule('river');
  }

  private getOrCreate(entityId: string): RiverState {
  ...
  activate(entityId: string, intensity = 50): RiverState {
    ...
    blockchainBridge.validateAIOutput('river', { entityId, level: state.level });
    logger.info({ entityId, module: 'river', intensity }, '[River] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'river' }, '[River] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RiverState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RiverState | null { return this.states.get(entityId) ?? null; }
