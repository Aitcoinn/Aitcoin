...
export class Mantle {
  private states: Map<string, MantleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('mantle');
  }

  private getOrCreate(entityId: string): MantleState {
  ...
  activate(entityId: string, intensity = 50): MantleState {
    ...
    blockchainBridge.validateAIOutput('mantle', { entityId, level: state.level });
    logger.info({ entityId, module: 'mantle', intensity }, '[Mantle] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'mantle' }, '[Mantle] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MantleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MantleState | null { return this.states.get(entityId) ?? null; }
