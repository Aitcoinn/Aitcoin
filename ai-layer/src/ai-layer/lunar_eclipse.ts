...
export class LunarEclipse {
  private states: Map<string, LunarEclipseState> = new Map();

  constructor() {
    blockchainBridge.registerModule('lunar_eclipse');
  }

  private getOrCreate(entityId: string): LunarEclipseState {
  ...
  activate(entityId: string, intensity = 50): LunarEclipseState {
    ...
    blockchainBridge.validateAIOutput('lunar_eclipse', { entityId, level: state.level });
    logger.info({ entityId, module: 'lunar_eclipse', intensity }, '[LunarEclipse] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'lunar_eclipse' }, '[LunarEclipse] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LunarEclipseState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LunarEclipseState | null { return this.states.get(entityId) ?? null; }
...
export const lunarEclipse = new LunarEclipse();
