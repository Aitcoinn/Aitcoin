...
export class Smell {
  private states: Map<string, SmellState> = new Map();

  constructor() {
    blockchainBridge.registerModule('smell');
  }

  private getOrCreate(entityId: string): SmellState {
  ...
  activate(entityId: string, intensity = 50): SmellState {
    ...
    blockchainBridge.validateAIOutput('smell', { entityId, level: state.level });
    logger.info({ entityId, module: 'smell', intensity }, '[Smell] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'smell' }, '[Smell] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SmellState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SmellState | null { return this.states.get(entityId) ?? null; }
