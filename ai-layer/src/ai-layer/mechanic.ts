...
export class Mechanic {
  private states: Map<string, MechanicState> = new Map();

  constructor() {
    blockchainBridge.registerModule('mechanic');
  }

  private getOrCreate(entityId: string): MechanicState {
  ...
  activate(entityId: string, intensity = 50): MechanicState {
    ...
    blockchainBridge.validateAIOutput('mechanic', { entityId, level: state.level });
    logger.info({ entityId, module: 'mechanic', intensity }, '[Mechanic] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'mechanic' }, '[Mechanic] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MechanicState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MechanicState | null { return this.states.get(entityId) ?? null; }
