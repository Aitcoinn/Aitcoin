...
export class PositionSys {
  private states: Map<string, PositionSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('position_sys');
  }

  private getOrCreate(entityId: string): PositionSysState {
  ...
  activate(entityId: string, intensity = 50): PositionSysState {
    ...
    blockchainBridge.validateAIOutput('position_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'position_sys', intensity }, '[PositionSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'position_sys' }, '[PositionSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PositionSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PositionSysState | null { return this.states.get(entityId) ?? null; }
...
export const positionSys = new PositionSys();
export default positionSys;
