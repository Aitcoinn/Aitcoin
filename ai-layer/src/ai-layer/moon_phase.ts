...
export class MoonPhase {
  private states: Map<string, MoonPhaseState> = new Map();

  constructor() {
    blockchainBridge.registerModule('moon_phase');
  }

  private getOrCreate(entityId: string): MoonPhaseState {
  ...
  activate(entityId: string, intensity = 50): MoonPhaseState {
    ...
    blockchainBridge.validateAIOutput('moon_phase', { entityId, level: state.level });
    logger.info({ entityId, module: 'moon_phase', intensity }, '[MoonPhase] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'moon_phase' }, '[MoonPhase] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MoonPhaseState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MoonPhaseState | null { return this.states.get(entityId) ?? null; }
...
export const moonPhase = new MoonPhase();
