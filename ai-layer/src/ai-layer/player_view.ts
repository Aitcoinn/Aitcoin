...
export class PlayerView {
  private states: Map<string, PlayerViewState> = new Map();

  constructor() {
    blockchainBridge.registerModule('player_view');
  }

  private getOrCreate(entityId: string): PlayerViewState {
  ...
  activate(entityId: string, intensity = 50): PlayerViewState {
    ...
    blockchainBridge.validateAIOutput('player_view', { entityId, level: state.level });
    logger.info({ entityId, module: 'player_view', intensity }, '[PlayerView] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'player_view' }, '[PlayerView] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PlayerViewState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PlayerViewState | null { return this.states.get(entityId) ?? null; }
