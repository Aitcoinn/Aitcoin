...
export class OrangeSky {
  private states: Map<string, OrangeSkyState> = new Map();

  constructor() {
    blockchainBridge.registerModule('orange_sky');
  }

  private getOrCreate(entityId: string): OrangeSkyState {
  ...
  activate(entityId: string, intensity = 50): OrangeSkyState {
    ...
    blockchainBridge.validateAIOutput('orange_sky', { entityId, level: state.level });
    logger.info({ entityId, module: 'orange_sky', intensity }, '[OrangeSky] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'orange_sky' }, '[OrangeSky] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): OrangeSkyState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): OrangeSkyState | null { return this.states.get(entityId) ?? null; }
...
export const orangeSky = new OrangeSky();
