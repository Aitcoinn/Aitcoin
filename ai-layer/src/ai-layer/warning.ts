...
export class Warning {
  private states: Map<string, WarningState> = new Map();

  constructor() {
    blockchainBridge.registerModule('warning');
  }

  private getOrCreate(entityId: string): WarningState {
  ...
  activate(entityId: string, intensity = 50): WarningState {
    ...
    blockchainBridge.validateAIOutput('warning', { entityId, level: state.level });
    logger.info({ entityId, module: 'warning', intensity }, '[Warning] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'warning' }, '[Warning] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WarningState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WarningState | null { return this.states.get(entityId) ?? null; }
...
export const warning = new Warning();
