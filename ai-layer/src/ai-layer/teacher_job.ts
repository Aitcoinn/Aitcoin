...
export class TeacherJob {
  private states: Map<string, TeacherJobState> = new Map();

  constructor() {
    blockchainBridge.registerModule('teacher_job');
  }

  private getOrCreate(entityId: string): TeacherJobState {
  ...
  activate(entityId: string, intensity = 50): TeacherJobState {
    ...
    blockchainBridge.validateAIOutput('teacher_job', { entityId, level: state.level });
    logger.info({ entityId, module: 'teacher_job', intensity }, '[TeacherJob] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'teacher_job' }, '[TeacherJob] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TeacherJobState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TeacherJobState | null { return this.states.get(entityId) ?? null; }
...
export const teacherJob = new TeacherJob();
