...
export class PressureAir {
  private states: Map<string, PressureAirState> = new Map();

  constructor() {
    blockchainBridge.registerModule('pressure_air');
  }

  private getOrCreate(entityId: string): PressureAirState {
  ...
  activate(entityId: string, intensity = 50): PressureAirState {
    ...
    blockchainBridge.validateAIOutput('pressure_air', { entityId, level: state.level });
    logger.info({ entityId, module: 'pressure_air', intensity }, '[PressureAir] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'pressure_air' }, '[PressureAir] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PressureAirState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PressureAirState | null { return this.states.get(entityId) ?? null; }
...
export const pressureAir = new PressureAir();
export default pressureAir;
