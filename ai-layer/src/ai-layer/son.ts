...
export class Son {
  private states: Map<string, SonState> = new Map();

  private getOrCreate(entityId: string): SonState {
  ...
  activate(entityId: string, intensity = 50): SonState {
    ...
    logger.info({ entityId, module: 'son', intensity }, '[Son] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'son' }, '[Son] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SonState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'son' }, '[Son] Updated');
    return state;
  }

  getState(entityId: string): SonState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'son' }, '[Son] Reset');
  }
