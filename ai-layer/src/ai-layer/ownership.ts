...
export class Ownership {
  private states: Map<string, OwnershipState> = new Map();

  constructor() {
    blockchainBridge.registerModule('ownership');
  }

  private getOrCreate(entityId: string): OwnershipState {
  ...
  activate(entityId: string, intensity = 50): OwnershipState {
    ...
    blockchainBridge.validateAIOutput('ownership', { entityId, level: state.level });
    logger.info({ entityId, module: 'ownership', intensity }, '[Ownership] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ownership' }, '[Ownership] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): OwnershipState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): OwnershipState | null { return this.states.get(entityId) ?? null; }
