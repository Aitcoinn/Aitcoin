...
export class Wind {
  private states: Map<string, WindState> = new Map();

  constructor() {
    blockchainBridge.registerModule('wind');
  }

  private getOrCreate(entityId: string): WindState {
  ...
  activate(entityId: string, intensity = 50): WindState {
    ...
    blockchainBridge.validateAIOutput('wind', { entityId, level: state.level });
    logger.info({ entityId, module: 'wind', intensity }, '[Wind] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'wind' }, '[Wind] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WindState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WindState | null { return this.states.get(entityId) ?? null; }
