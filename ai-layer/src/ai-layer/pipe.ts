...
export class Pipe {
  private states: Map<string, PipeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('pipe');
  }

  private getOrCreate(entityId: string): PipeState {
  ...
  activate(entityId: string, intensity = 50): PipeState {
    ...
    blockchainBridge.validateAIOutput('pipe', { entityId, level: state.level });
    logger.info({ entityId, module: 'pipe', intensity }, '[Pipe] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'pipe' }, '[Pipe] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PipeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PipeState | null { return this.states.get(entityId) ?? null; }
