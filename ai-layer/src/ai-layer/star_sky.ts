...
export class StarSky {
  private states: Map<string, StarSkyState> = new Map();

  constructor() {
    blockchainBridge.registerModule('star_sky');
  }

  private getOrCreate(entityId: string): StarSkyState {
  ...
  activate(entityId: string, intensity = 50): StarSkyState {
    ...
    blockchainBridge.validateAIOutput('star_sky', { entityId, level: state.level });
    logger.info({ entityId, module: 'star_sky', intensity }, '[StarSky] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'star_sky' }, '[StarSky] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): StarSkyState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): StarSkyState | null { return this.states.get(entityId) ?? null; }
...
export const starSky = new StarSky();
