...
export class ScaleSys {
  private states: Map<string, ScaleSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('scale_sys');
  }

  private getOrCreate(entityId: string): ScaleSysState {
  ...
  activate(entityId: string, intensity = 50): ScaleSysState {
    ...
    blockchainBridge.validateAIOutput('scale_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'scale_sys', intensity }, '[ScaleSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'scale_sys' }, '[ScaleSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ScaleSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ScaleSysState | null { return this.states.get(entityId) ?? null; }
...
export const scaleSys = new ScaleSys();
export default scaleSys;
