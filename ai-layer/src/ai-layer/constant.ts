...
export class Constant {
  private states: Map<string, ConstantState> = new Map();

  constructor() {
    blockchainBridge.registerModule('constant');
  }

  private getOrCreate(entityId: string): ConstantState {
  ...
  activate(entityId: string, intensity = 50): ConstantState {
    ...
    blockchainBridge.validateAIOutput('constant', { entityId, level: state.level });
    logger.info({ entityId, module: 'constant', intensity }, '[Constant] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'constant' }, '[Constant] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ConstantState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ConstantState | null { return this.states.get(entityId) ?? null; }
