...
export class Tide {
  private states: Map<string, TideState> = new Map();

  constructor() {
    blockchainBridge.registerModule('tide');
  }

  private getOrCreate(entityId: string): TideState {
  ...
  activate(entityId: string, intensity = 50): TideState {
    ...
    blockchainBridge.validateAIOutput('tide', { entityId, level: state.level });
    logger.info({ entityId, module: 'tide', intensity }, '[Tide] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'tide' }, '[Tide] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TideState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TideState | null { return this.states.get(entityId) ?? null; }
