...
export class Trade {
  private states: Map<string, TradeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('trade');
  }

  private getOrCreate(entityId: string): TradeState {
  ...
  activate(entityId: string, intensity = 50): TradeState {
    ...
    blockchainBridge.validateAIOutput('trade', { entityId, level: state.level });
    logger.info({ entityId, module: 'trade', intensity }, '[Trade] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'trade' }, '[Trade] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TradeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TradeState | null { return this.states.get(entityId) ?? null; }
