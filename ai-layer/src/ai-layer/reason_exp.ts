...
export class ReasonExp {
  private states: Map<string, ReasonExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('reason_exp');
  }

  private getOrCreate(entityId: string): ReasonExpState {
  ...
  activate(entityId: string, intensity = 50): ReasonExpState {
    ...
    blockchainBridge.validateAIOutput('reason_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'reason_exp', intensity }, '[ReasonExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'reason_exp' }, '[ReasonExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ReasonExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ReasonExpState | null { return this.states.get(entityId) ?? null; }
...
export const reasonExp = new ReasonExp();
export default reasonExp;
