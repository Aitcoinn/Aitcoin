...
export class Phone {
  private states: Map<string, PhoneState> = new Map();

  constructor() {
    blockchainBridge.registerModule('phone');
  }

  private getOrCreate(entityId: string): PhoneState {
  ...
  activate(entityId: string, intensity = 50): PhoneState {
    ...
    blockchainBridge.validateAIOutput('phone', { entityId, level: state.level });
    logger.info({ entityId, module: 'phone', intensity }, '[Phone] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'phone' }, '[Phone] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PhoneState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PhoneState | null { return this.states.get(entityId) ?? null; }
