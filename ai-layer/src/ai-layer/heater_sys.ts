...
export class HeaterSys {
  private states: Map<string, HeaterSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('heater_sys');
  }

  private getOrCreate(entityId: string): HeaterSysState {
  ...
  activate(entityId: string, intensity = 50): HeaterSysState {
    ...
    blockchainBridge.validateAIOutput('heater_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'heater_sys', intensity }, '[HeaterSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'heater_sys' }, '[HeaterSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HeaterSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HeaterSysState | null { return this.states.get(entityId) ?? null; }
...
export const heaterSys = new HeaterSys();
export default heaterSys;
