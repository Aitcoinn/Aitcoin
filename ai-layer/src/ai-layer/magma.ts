...
export class Magma {
  private states: Map<string, MagmaState> = new Map();

  constructor() {
    blockchainBridge.registerModule('magma');
  }

  private getOrCreate(entityId: string): MagmaState {
  ...
  activate(entityId: string, intensity = 50): MagmaState {
    ...
    blockchainBridge.validateAIOutput('magma', { entityId, level: state.level });
    logger.info({ entityId, module: 'magma', intensity }, '[Magma] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'magma' }, '[Magma] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MagmaState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MagmaState | null { return this.states.get(entityId) ?? null; }
