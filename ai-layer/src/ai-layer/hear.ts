...
export class Hear {
  private states: Map<string, HearState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hear');
  }

  private getOrCreate(entityId: string): HearState {
  ...
  activate(entityId: string, intensity = 50): HearState {
    ...
    blockchainBridge.validateAIOutput('hear', { entityId, level: state.level });
    logger.info({ entityId, module: 'hear', intensity }, '[Hear] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hear' }, '[Hear] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HearState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HearState | null { return this.states.get(entityId) ?? null; }
