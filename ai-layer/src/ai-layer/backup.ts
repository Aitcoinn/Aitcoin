...
export class Backup {
  private states: Map<string, BackupState> = new Map();

  constructor() {
    blockchainBridge.registerModule('backup');
  }

  private getOrCreate(entityId: string): BackupState {
  ...
  activate(entityId: string, intensity = 50): BackupState {
    ...
    blockchainBridge.validateAIOutput('backup', { entityId, level: state.level });
    logger.info({ entityId, module: 'backup', intensity }, '[Backup] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'backup' }, '[Backup] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BackupState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BackupState | null { return this.states.get(entityId) ?? null; }
