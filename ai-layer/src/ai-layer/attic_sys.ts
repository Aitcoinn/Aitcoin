...
export class AtticSys {
  private states: Map<string, AtticSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('attic_sys');
  }

  private getOrCreate(entityId: string): AtticSysState {
  ...
  activate(entityId: string, intensity = 50): AtticSysState {
    ...
    blockchainBridge.validateAIOutput('attic_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'attic_sys', intensity }, '[AtticSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'attic_sys' }, '[AtticSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AtticSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AtticSysState | null { return this.states.get(entityId) ?? null; }
...
export const atticSys = new AtticSys();
export default atticSys;
