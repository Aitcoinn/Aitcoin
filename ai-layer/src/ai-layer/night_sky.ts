...
export class NightSky {
  private states: Map<string, NightSkyState> = new Map();

  constructor() {
    blockchainBridge.registerModule('night_sky');
  }

  private getOrCreate(entityId: string): NightSkyState {
  ...
  activate(entityId: string, intensity = 50): NightSkyState {
    ...
    blockchainBridge.validateAIOutput('night_sky', { entityId, level: state.level });
    logger.info({ entityId, module: 'night_sky', intensity }, '[NightSky] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'night_sky' }, '[NightSky] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): NightSkyState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): NightSkyState | null { return this.states.get(entityId) ?? null; }
...
export const nightSky = new NightSky();
