...
export class Rope {
  private states: Map<string, RopeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('rope');
  }

  private getOrCreate(entityId: string): RopeState {
  ...
  activate(entityId: string, intensity = 50): RopeState {
    ...
    blockchainBridge.validateAIOutput('rope', { entityId, level: state.level });
    logger.info({ entityId, module: 'rope', intensity }, '[Rope] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'rope' }, '[Rope] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RopeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RopeState | null { return this.states.get(entityId) ?? null; }
