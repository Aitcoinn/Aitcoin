...
export class HeavyRain {
  private states: Map<string, HeavyRainState> = new Map();

  constructor() {
    blockchainBridge.registerModule('heavy_rain');
  }

  private getOrCreate(entityId: string): HeavyRainState {
  ...
  activate(entityId: string, intensity = 50): HeavyRainState {
    ...
    blockchainBridge.validateAIOutput('heavy_rain', { entityId, level: state.level });
    logger.info({ entityId, module: 'heavy_rain', intensity }, '[HeavyRain] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'heavy_rain' }, '[HeavyRain] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HeavyRainState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HeavyRainState | null { return this.states.get(entityId) ?? null; }
...
export const heavyRain = new HeavyRain();
