...
export class Solubility {
  private states: Map<string, SolubilityState> = new Map();

  constructor() {
    blockchainBridge.registerModule('solubility');
  }

  private getOrCreate(entityId: string): SolubilityState {
  ...
  activate(entityId: string, intensity = 50): SolubilityState {
    ...
    blockchainBridge.validateAIOutput('solubility', { entityId, level: state.level });
    logger.info({ entityId, module: 'solubility', intensity }, '[Solubility] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'solubility' }, '[Solubility] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SolubilityState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SolubilityState | null { return this.states.get(entityId) ?? null; }
...
export const solubility = new Solubility();
