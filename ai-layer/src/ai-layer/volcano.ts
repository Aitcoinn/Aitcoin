...
export class Volcano {
  private states: Map<string, VolcanoState> = new Map();

  constructor() {
    blockchainBridge.registerModule('volcano');
  }

  private getOrCreate(entityId: string): VolcanoState {
  ...
  activate(entityId: string, intensity = 50): VolcanoState {
    ...
    blockchainBridge.validateAIOutput('volcano', { entityId, level: state.level });
    logger.info({ entityId, module: 'volcano', intensity }, '[Volcano] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'volcano' }, '[Volcano] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): VolcanoState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): VolcanoState | null { return this.states.get(entityId) ?? null; }
