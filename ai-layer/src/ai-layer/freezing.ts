...
export class Freezing {
  private states: Map<string, FreezingState> = new Map();

  constructor() {
    blockchainBridge.registerModule('freezing');
  }

  private getOrCreate(entityId: string): FreezingState {
  ...
  activate(entityId: string, intensity = 50): FreezingState {
    ...
    blockchainBridge.validateAIOutput('freezing', { entityId, level: state.level });
    logger.info({ entityId, module: 'freezing', intensity }, '[Freezing] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'freezing' }, '[Freezing] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FreezingState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FreezingState | null { return this.states.get(entityId) ?? null; }
...
export const freezing = new Freezing();
