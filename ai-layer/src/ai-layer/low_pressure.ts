...
export class LowPressure {
  private states: Map<string, LowPressureState> = new Map();

  constructor() {
    blockchainBridge.registerModule('low_pressure');
  }

  private getOrCreate(entityId: string): LowPressureState {
  ...
  activate(entityId: string, intensity = 50): LowPressureState {
    ...
    blockchainBridge.validateAIOutput('low_pressure', { entityId, level: state.level });
    logger.info({ entityId, module: 'low_pressure', intensity }, '[LowPressure] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'low_pressure' }, '[LowPressure] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LowPressureState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LowPressureState | null { return this.states.get(entityId) ?? null; }
...
export const lowPressure = new LowPressure();
