...
export class Soldier {
  private states: Map<string, SoldierState> = new Map();

  private getOrCreate(entityId: string): SoldierState {
  ...
  activate(entityId: string, intensity = 50): SoldierState {
    ...
    logger.info({ entityId, module: 'soldier', intensity }, '[Soldier] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'soldier' }, '[Soldier] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SoldierState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'soldier' }, '[Soldier] Updated');
    return state;
  }

  getState(entityId: string): SoldierState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'soldier' }, '[Soldier] Reset');
  }
