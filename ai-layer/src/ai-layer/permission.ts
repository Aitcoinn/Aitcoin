...
export class Permission {
  private states: Map<string, PermissionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('permission');
  }

  private getOrCreate(entityId: string): PermissionState {
  ...
  activate(entityId: string, intensity = 50): PermissionState {
    ...
    blockchainBridge.validateAIOutput('permission', { entityId, level: state.level });
    logger.info({ entityId, module: 'permission', intensity }, '[Permission] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'permission' }, '[Permission] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PermissionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PermissionState | null { return this.states.get(entityId) ?? null; }
