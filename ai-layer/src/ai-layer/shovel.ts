...
export class Shovel {
  private states: Map<string, ShovelState> = new Map();

  constructor() {
    blockchainBridge.registerModule('shovel');
  }

  private getOrCreate(entityId: string): ShovelState {
  ...
  activate(entityId: string, intensity = 50): ShovelState {
    ...
    blockchainBridge.validateAIOutput('shovel', { entityId, level: state.level });
    logger.info({ entityId, module: 'shovel', intensity }, '[Shovel] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'shovel' }, '[Shovel] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ShovelState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ShovelState | null { return this.states.get(entityId) ?? null; }
...
export const shovel = new Shovel();
