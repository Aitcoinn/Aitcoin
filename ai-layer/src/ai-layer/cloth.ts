...
export class Cloth {
  private states: Map<string, ClothState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cloth');
  }

  private getOrCreate(entityId: string): ClothState {
  ...
  activate(entityId: string, intensity = 50): ClothState {
    ...
    blockchainBridge.validateAIOutput('cloth', { entityId, level: state.level });
    logger.info({ entityId, module: 'cloth', intensity }, '[Cloth] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cloth' }, '[Cloth] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ClothState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ClothState | null { return this.states.get(entityId) ?? null; }
