...
export class UpdateFrame {
  private states: Map<string, UpdateFrameState> = new Map();

  constructor() {
    blockchainBridge.registerModule('update_frame');
  }

  private getOrCreate(entityId: string): UpdateFrameState {
  ...
  activate(entityId: string, intensity = 50): UpdateFrameState {
    ...
    blockchainBridge.validateAIOutput('update_frame', { entityId, level: state.level });
    logger.info({ entityId, module: 'update_frame', intensity }, '[UpdateFrame] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'update_frame' }, '[UpdateFrame] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): UpdateFrameState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): UpdateFrameState | null { return this.states.get(entityId) ?? null; }
...
export const updateFrame = new UpdateFrame();
