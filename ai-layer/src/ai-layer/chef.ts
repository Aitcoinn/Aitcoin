...
export class Chef {
  private states: Map<string, ChefState> = new Map();

  constructor() {
    blockchainBridge.registerModule('chef');
  }

  private getOrCreate(entityId: string): ChefState {
  ...
  activate(entityId: string, intensity = 50): ChefState {
    ...
    blockchainBridge.validateAIOutput('chef', { entityId, level: state.level });
    logger.info({ entityId, module: 'chef', intensity }, '[Chef] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'chef' }, '[Chef] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ChefState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ChefState | null { return this.states.get(entityId) ?? null; }
