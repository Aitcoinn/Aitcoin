...
export class WorldView {
  private states: Map<string, WorldViewState> = new Map();

  constructor() {
    blockchainBridge.registerModule('world_view');
  }

  private getOrCreate(entityId: string): WorldViewState {
  ...
  activate(entityId: string, intensity = 50): WorldViewState {
    ...
    blockchainBridge.validateAIOutput('world_view', { entityId, level: state.level });
    logger.info({ entityId, module: 'world_view', intensity }, '[WorldView] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'world_view' }, '[WorldView] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WorldViewState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WorldViewState | null { return this.states.get(entityId) ?? null; }
