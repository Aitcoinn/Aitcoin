...
export class Twin {
  ...
  private getOrCreate(entityId: string): TwinState {
  ...
  activate(entityId: string, intensity = 50): TwinState {
    ...
    logger.info({ entityId, module: 'twin', intensity }, '[Twin] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'twin' }, '[Twin] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TwinState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'twin' }, '[Twin] Updated');
    return state;
  }

  getState(entityId: string): TwinState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'twin' }, '[Twin] Reset');
  }
