...
export class Bucket {
  private states: Map<string, BucketState> = new Map();

  constructor() {
    blockchainBridge.registerModule('bucket');
  }

  private getOrCreate(entityId: string): BucketState {
  ...
  activate(entityId: string, intensity = 50): BucketState {
    ...
    blockchainBridge.validateAIOutput('bucket', { entityId, level: state.level });
    logger.info({ entityId, module: 'bucket', intensity }, '[Bucket] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'bucket' }, '[Bucket] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BucketState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BucketState | null { return this.states.get(entityId) ?? null; }
