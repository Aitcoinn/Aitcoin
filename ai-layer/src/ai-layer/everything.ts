...
export class Everything {
  private states: Map<string, EverythingState> = new Map();

  constructor() {
    blockchainBridge.registerModule('everything');
  }

  private getOrCreate(entityId: string): EverythingState {
  ...
  activate(entityId: string, intensity = 50): EverythingState {
    ...
    blockchainBridge.validateAIOutput('everything', { entityId, level: state.level });
    logger.info({ entityId, module: 'everything', intensity }, '[Everything] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'everything' }, '[Everything] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EverythingState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): EverythingState | null { return this.states.get(entityId) ?? null; }
