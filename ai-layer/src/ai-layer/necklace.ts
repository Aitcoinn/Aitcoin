...
export class Necklace {
  private states: Map<string, NecklaceState> = new Map();

  constructor() {
    blockchainBridge.registerModule('necklace');
  }

  private getOrCreate(entityId: string): NecklaceState {
  ...
  activate(entityId: string, intensity = 50): NecklaceState {
    ...
    blockchainBridge.validateAIOutput('necklace', { entityId, level: state.level });
    logger.info({ entityId, module: 'necklace', intensity }, '[Necklace] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'necklace' }, '[Necklace] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): NecklaceState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): NecklaceState | null { return this.states.get(entityId) ?? null; }
...
export const necklace = new Necklace();
