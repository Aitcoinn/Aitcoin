...
export class Absence {
  private states: Map<string, AbsenceState> = new Map();

  constructor() {
    blockchainBridge.registerModule('absence');
  }

  private getOrCreate(entityId: string): AbsenceState {
  ...
  activate(entityId: string, intensity = 50): AbsenceState {
    ...
    blockchainBridge.validateAIOutput('absence', { entityId, level: state.level });
    logger.info({ entityId, module: 'absence', intensity }, '[Absence] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'absence' }, '[Absence] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AbsenceState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AbsenceState | null { return this.states.get(entityId) ?? null; }
...
export const absence = new Absence();
