...
export class Fixed {
  private states: Map<string, FixedState> = new Map();

  private getOrCreate(entityId: string): FixedState {
  ...
  activate(entityId: string, intensity = 50): FixedState {
    ...
    logger.info({ entityId, module: 'fixed', intensity }, '[Fixed] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fixed' }, '[Fixed] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FixedState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fixed' }, '[Fixed] Updated');
    return state;
  }

  getState(entityId: string): FixedState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'fixed' }, '[Fixed] Reset');
  }
