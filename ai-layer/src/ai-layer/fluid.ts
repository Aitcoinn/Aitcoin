...
export class Fluid {
  private states: Map<string, FluidState> = new Map();

  constructor() {
    blockchainBridge.registerModule('fluid');
  }

  private getOrCreate(entityId: string): FluidState {
  ...
  activate(entityId: string, intensity = 50): FluidState {
    ...
    blockchainBridge.validateAIOutput('fluid', { entityId, level: state.level });
    logger.info({ entityId, module: 'fluid', intensity }, '[Fluid] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fluid' }, '[Fluid] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FluidState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FluidState | null { return this.states.get(entityId) ?? null; }
