...
export class Plate {
  private states: Map<string, PlateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('plate');
  }

  private getOrCreate(entityId: string): PlateState {
  ...
  activate(entityId: string, intensity = 50): PlateState {
    ...
    blockchainBridge.validateAIOutput('plate', { entityId, level: state.level });
    logger.info({ entityId, module: 'plate', intensity }, '[Plate] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'plate' }, '[Plate] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PlateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PlateState | null { return this.states.get(entityId) ?? null; }
