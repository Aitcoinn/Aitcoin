...
export class Nimbus {
  private states: Map<string, NimbusState> = new Map();

  constructor() {
    blockchainBridge.registerModule('nimbus');
  }

  private getOrCreate(entityId: string): NimbusState {
  ...
  activate(entityId: string, intensity = 50): NimbusState {
    ...
    blockchainBridge.validateAIOutput('nimbus', { entityId, level: state.level });
    logger.info({ entityId, module: 'nimbus', intensity }, '[Nimbus] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'nimbus' }, '[Nimbus] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): NimbusState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): NimbusState | null { return this.states.get(entityId) ?? null; }
