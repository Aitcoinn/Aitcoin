...
export class Wake {
  private states: Map<string, WakeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('wake');
  }

  private getOrCreate(entityId: string): WakeState {
  ...
  activate(entityId: string, intensity = 50): WakeState {
    ...
    blockchainBridge.validateAIOutput('wake', { entityId, level: state.level });
    logger.info({ entityId, module: 'wake', intensity }, '[Wake] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'wake' }, '[Wake] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WakeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WakeState | null { return this.states.get(entityId) ?? null; }
