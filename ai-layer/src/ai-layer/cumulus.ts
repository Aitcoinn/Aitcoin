...
export class Cumulus {
  private states: Map<string, CumulusState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cumulus');
  }

  private getOrCreate(entityId: string): CumulusState {
  ...
  activate(entityId: string, intensity = 50): CumulusState {
    ...
    blockchainBridge.validateAIOutput('cumulus', { entityId, level: state.level });
    logger.info({ entityId, module: 'cumulus', intensity }, '[Cumulus] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cumulus' }, '[Cumulus] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CumulusState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CumulusState | null { return this.states.get(entityId) ?? null; }
...
export const cumulus = new Cumulus();
