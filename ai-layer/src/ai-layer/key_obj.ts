...
export class KeyObj {
  private states: Map<string, KeyObjState> = new Map();

  constructor() {
    blockchainBridge.registerModule('key_obj');
  }

  private getOrCreate(entityId: string): KeyObjState {
  ...
  activate(entityId: string, intensity = 50): KeyObjState {
    ...
    blockchainBridge.validateAIOutput('key_obj', { entityId, level: state.level });
    logger.info({ entityId, module: 'key_obj', intensity }, '[KeyObj] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'key_obj' }, '[KeyObj] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): KeyObjState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): KeyObjState | null { return this.states.get(entityId) ?? null; }
...
export const keyObj = new KeyObj();
