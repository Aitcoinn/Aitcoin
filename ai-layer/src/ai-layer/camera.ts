...
export class Camera {
  ...
  constructor() {
    blockchainBridge.registerModule('camera');
  }

  private getOrCreate(entityId: string): CameraState {
  ...
  activate(entityId: string, intensity = 50): CameraState {
    ...
    blockchainBridge.validateAIOutput('camera', { entityId, level: state.level });
    logger.info({ entityId, module: 'camera', intensity }, '[Camera] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'camera' }, '[Camera] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CameraState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CameraState | null { return this.states.get(entityId) ?? null; }
