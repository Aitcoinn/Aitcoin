...
export class Banknote {
  private states: Map<string, BanknoteState> = new Map();

  constructor() {
    blockchainBridge.registerModule('banknote');
  }

  private getOrCreate(entityId: string): BanknoteState {
  ...
  activate(entityId: string, intensity = 50): BanknoteState {
    ...
    blockchainBridge.validateAIOutput('banknote', { entityId, level: state.level });
    logger.info({ entityId, module: 'banknote', intensity }, '[Banknote] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'banknote' }, '[Banknote] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BanknoteState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BanknoteState | null { return this.states.get(entityId) ?? null; }
...
export const banknote = new Banknote();
