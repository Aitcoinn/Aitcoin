...
export class AirConditioner {
  private states: Map<string, AirConditionerState> = new Map();

  constructor() {
    blockchainBridge.registerModule('air_conditioner');
  }

  private getOrCreate(entityId: string): AirConditionerState {
  ...
  activate(entityId: string, intensity = 50): AirConditionerState {
    ...
    blockchainBridge.validateAIOutput('air_conditioner', { entityId, level: state.level });
    logger.info({ entityId, module: 'air_conditioner', intensity }, '[AirConditioner] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'air_conditioner' }, '[AirConditioner] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AirConditionerState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AirConditionerState | null { return this.states.get(entityId) ?? null; }
...
export const airConditioner = new AirConditioner();
