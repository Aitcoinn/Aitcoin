...
export class IllusionExp {
  private states: Map<string, IllusionExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('illusion_exp');
  }

  private getOrCreate(entityId: string): IllusionExpState {
  ...
  activate(entityId: string, intensity = 50): IllusionExpState {
    ...
    blockchainBridge.validateAIOutput('illusion_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'illusion_exp', intensity }, '[IllusionExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'illusion_exp' }, '[IllusionExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): IllusionExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): IllusionExpState | null { return this.states.get(entityId) ?? null; }
...
export const illusionExp = new IllusionExp();
export default illusionExp;
