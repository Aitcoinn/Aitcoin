...
export class Solid {
  private states: Map<string, SolidState> = new Map();

  constructor() {
    blockchainBridge.registerModule('solid');
  }

  private getOrCreate(entityId: string): SolidState {
  ...
  activate(entityId: string, intensity = 50): SolidState {
    ...
    blockchainBridge.validateAIOutput('solid', { entityId, level: state.level });
    logger.info({ entityId, module: 'solid', intensity }, '[Solid] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'solid' }, '[Solid] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SolidState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SolidState | null { return this.states.get(entityId) ?? null; }
