...
export class Dreaming {
  private states: Map<string, DreamingState> = new Map();

  constructor() {
    blockchainBridge.registerModule('dreaming');
  }

  private getOrCreate(entityId: string): DreamingState {
  ...
  activate(entityId: string, intensity = 50): DreamingState {
    ...
    blockchainBridge.validateAIOutput('dreaming', { entityId, level: state.level });
    logger.info({ entityId, module: 'dreaming', intensity }, '[Dreaming] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'dreaming' }, '[Dreaming] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DreamingState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DreamingState | null { return this.states.get(entityId) ?? null; }
...
export const dreaming = new Dreaming();
