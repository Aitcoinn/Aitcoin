...
export class Tornado {
  private states: Map<string, TornadoState> = new Map();

  constructor() {
    blockchainBridge.registerModule('tornado');
  }

  private getOrCreate(entityId: string): TornadoState {
  ...
  activate(entityId: string, intensity = 50): TornadoState {
    ...
    blockchainBridge.validateAIOutput('tornado', { entityId, level: state.level });
    logger.info({ entityId, module: 'tornado', intensity }, '[Tornado] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'tornado' }, '[Tornado] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TornadoState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TornadoState | null { return this.states.get(entityId) ?? null; }
...
export const tornado = new Tornado();
