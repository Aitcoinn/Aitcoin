...
export class TriggerArea {
  private states: Map<string, TriggerAreaState> = new Map();

  constructor() {
    blockchainBridge.registerModule('trigger_area');
  }

  private getOrCreate(entityId: string): TriggerAreaState {
  ...
  activate(entityId: string, intensity = 50): TriggerAreaState {
    ...
    blockchainBridge.validateAIOutput('trigger_area', { entityId, level: state.level });
    logger.info({ entityId, module: 'trigger_area', intensity }, '[TriggerArea] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'trigger_area' }, '[TriggerArea] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TriggerAreaState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TriggerAreaState | null { return this.states.get(entityId) ?? null; }
...
export const triggerArea = new TriggerArea();
