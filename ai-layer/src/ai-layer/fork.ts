...
export class Fork {
  private states: Map<string, ForkState> = new Map();

  constructor() {
    blockchainBridge.registerModule('fork');
  }

  private getOrCreate(entityId: string): ForkState {
  ...
  activate(entityId: string, intensity = 50): ForkState {
    ...
    blockchainBridge.validateAIOutput('fork', { entityId, level: state.level });
    logger.info({ entityId, module: 'fork', intensity }, '[Fork] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fork' }, '[Fork] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ForkState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ForkState | null { return this.states.get(entityId) ?? null; }
