...
export class Broom {
  private states: Map<string, BroomState> = new Map();

  constructor() {
    blockchainBridge.registerModule('broom');
  }

  private getOrCreate(entityId: string): BroomState {
  ...
  activate(entityId: string, intensity = 50): BroomState {
    ...
    blockchainBridge.validateAIOutput('broom', { entityId, level: state.level });
    logger.info({ entityId, module: 'broom', intensity }, '[Broom] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'broom' }, '[Broom] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BroomState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BroomState | null { return this.states.get(entityId) ?? null; }
