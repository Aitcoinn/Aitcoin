...
export class WeakExp {
  private states: Map<string, WeakExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('weak_exp');
  }

  private getOrCreate(entityId: string): WeakExpState {
  ...
  activate(entityId: string, intensity = 50): WeakExpState {
    ...
    blockchainBridge.validateAIOutput('weak_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'weak_exp', intensity }, '[WeakExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'weak_exp' }, '[WeakExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WeakExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WeakExpState | null { return this.states.get(entityId) ?? null; }
...
export const weakExp = new WeakExp();
export default weakExp;
