...
export class DesertClimate {
  private states: Map<string, DesertClimateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('desert_climate');
  }

  private getOrCreate(entityId: string): DesertClimateState {
  ...
  activate(entityId: string, intensity = 50): DesertClimateState {
    ...
    blockchainBridge.validateAIOutput('desert_climate', { entityId, level: state.level });
    logger.info({ entityId, module: 'desert_climate', intensity }, '[DesertClimate] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'desert_climate' }, '[DesertClimate] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DesertClimateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DesertClimateState | null { return this.states.get(entityId) ?? null; }
...
export const desertClimate = new DesertClimate();
export default desertClimate;
