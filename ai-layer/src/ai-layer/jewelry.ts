...
export class Jewelry {
  private states: Map<string, JewelryState> = new Map();

  constructor() {
    blockchainBridge.registerModule('jewelry');
  }

  private getOrCreate(entityId: string): JewelryState {
  ...
  activate(entityId: string, intensity = 50): JewelryState {
    ...
    blockchainBridge.validateAIOutput('jewelry', { entityId, level: state.level });
    logger.info({ entityId, module: 'jewelry', intensity }, '[Jewelry] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'jewelry' }, '[Jewelry] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): JewelryState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): JewelryState | null { return this.states.get(entityId) ?? null; }
