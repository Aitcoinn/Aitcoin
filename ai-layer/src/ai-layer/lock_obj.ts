...
export class LockObj {
  private states: Map<string, LockObjState> = new Map();

  constructor() {
    blockchainBridge.registerModule('lock_obj');
  }

  private getOrCreate(entityId: string): LockObjState {
  ...
  activate(entityId: string, intensity = 50): LockObjState {
    ...
    blockchainBridge.validateAIOutput('lock_obj', { entityId, level: state.level });
    logger.info({ entityId, module: 'lock_obj', intensity }, '[LockObj] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'lock_obj' }, '[LockObj] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LockObjState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LockObjState | null { return this.states.get(entityId) ?? null; }
...
export const lockObj = new LockObj();
