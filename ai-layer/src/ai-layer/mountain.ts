...
export class Mountain {
  private states: Map<string, MountainState> = new Map();

  constructor() {
    blockchainBridge.registerModule('mountain');
  }

  private getOrCreate(entityId: string): MountainState {
  ...
  activate(entityId: string, intensity = 50): MountainState {
    ...
    blockchainBridge.validateAIOutput('mountain', { entityId, level: state.level });
    logger.info({ entityId, module: 'mountain', intensity }, '[Mountain] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'mountain' }, '[Mountain] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MountainState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MountainState | null { return this.states.get(entityId) ?? null; }
...
export const mountain = new Mountain();
