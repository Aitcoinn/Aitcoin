...
export class Plan {
  ...
  private getOrCreate(entityId: string): PlanState {
  ...
  activate(entityId: string, intensity = 50): PlanState {
    ...
    logger.info({ entityId, module: 'plan', intensity }, '[Plan] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'plan' }, '[Plan] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PlanState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'plan' }, '[Plan] Updated');
    return state;
  }

  getState(entityId: string): PlanState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'plan' }, '[Plan] Reset');
  }
