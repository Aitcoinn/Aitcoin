...
export class EngineerJob {
  private states: Map<string, EngineerJobState> = new Map();

  constructor() {
    blockchainBridge.registerModule('engineer_job');
  }

  private getOrCreate(entityId: string): EngineerJobState {
  ...
  activate(entityId: string, intensity = 50): EngineerJobState {
    ...
    blockchainBridge.validateAIOutput('engineer_job', { entityId, level: state.level });
    logger.info({ entityId, module: 'engineer_job', intensity }, '[EngineerJob] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'engineer_job' }, '[EngineerJob] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EngineerJobState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): EngineerJobState | null { return this.states.get(entityId) ?? null; }
...
export const engineerJob = new EngineerJob();
