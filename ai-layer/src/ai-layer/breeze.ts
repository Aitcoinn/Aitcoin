...
export class Breeze {
  private states: Map<string, BreezeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('breeze');
  }

  private getOrCreate(entityId: string): BreezeState {
  ...
  activate(entityId: string, intensity = 50): BreezeState {
    ...
    blockchainBridge.validateAIOutput('breeze', { entityId, level: state.level });
    logger.info({ entityId, module: 'breeze', intensity }, '[Breeze] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'breeze' }, '[Breeze] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BreezeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BreezeState | null { return this.states.get(entityId) ?? null; }
...
export const breeze = new Breeze();
