...
export class LampPost {
  private states: Map<string, LampPostState> = new Map();

  constructor() {
    blockchainBridge.registerModule('lamp_post');
  }

  private getOrCreate(entityId: string): LampPostState {
  ...
  activate(entityId: string, intensity = 50): LampPostState {
    ...
    blockchainBridge.validateAIOutput('lamp_post', { entityId, level: state.level });
    logger.info({ entityId, module: 'lamp_post', intensity }, '[LampPost] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'lamp_post' }, '[LampPost] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LampPostState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LampPostState | null { return this.states.get(entityId) ?? null; }
...
export const lampPost = new LampPost();
