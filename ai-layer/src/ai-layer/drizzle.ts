...
export class Drizzle {
  private states: Map<string, DrizzleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('drizzle');
  }

  private getOrCreate(entityId: string): DrizzleState {
  ...
  activate(entityId: string, intensity = 50): DrizzleState {
    ...
    blockchainBridge.validateAIOutput('drizzle', { entityId, level: state.level });
    logger.info({ entityId, module: 'drizzle', intensity }, '[Drizzle] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'drizzle' }, '[Drizzle] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DrizzleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DrizzleState | null { return this.states.get(entityId) ?? null; }
