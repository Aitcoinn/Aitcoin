...
export class Reason {
  private states: Map<string, ReasonState> = new Map();

  private getOrCreate(entityId: string): ReasonState {
  ...
  activate(entityId: string, intensity = 50): ReasonState {
    ...
    logger.info({ entityId, module: 'reason', intensity }, '[Reason] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'reason' }, '[Reason] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ReasonState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'reason' }, '[Reason] Updated');
    return state;
  }

  getState(entityId: string): ReasonState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'reason' }, '[Reason] Reset');
  }
