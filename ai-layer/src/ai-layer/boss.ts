...
export class Boss {
  ...
  private getOrCreate(entityId: string): BossState {
  ...
  activate(entityId: string, intensity = 50): BossState {
    ...
    logger.info({ entityId, module: 'boss', intensity }, '[Boss] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'boss' }, '[Boss] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BossState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'boss' }, '[Boss] Updated');
    return state;
  }

  getState(entityId: string): BossState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'boss' }, '[Boss] Reset');
  }
