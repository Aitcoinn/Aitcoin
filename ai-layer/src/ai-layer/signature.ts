...
export class Signature {
  private states: Map<string, SignatureState> = new Map();

  constructor() {
    blockchainBridge.registerModule('signature');
  }

  private getOrCreate(entityId: string): SignatureState {
  ...
  activate(entityId: string, intensity = 50): SignatureState {
    ...
    blockchainBridge.validateAIOutput('signature', { entityId, level: state.level });
    logger.info({ entityId, module: 'signature', intensity }, '[Signature] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'signature' }, '[Signature] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SignatureState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SignatureState | null { return this.states.get(entityId) ?? null; }
