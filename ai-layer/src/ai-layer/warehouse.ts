...
export class Warehouse {
  private states: Map<string, WarehouseState> = new Map();

  constructor() {
    blockchainBridge.registerModule('warehouse');
  }

  private getOrCreate(entityId: string): WarehouseState {
  ...
  activate(entityId: string, intensity = 50): WarehouseState {
    ...
    blockchainBridge.validateAIOutput('warehouse', { entityId, level: state.level });
    logger.info({ entityId, module: 'warehouse', intensity }, '[Warehouse] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'warehouse' }, '[Warehouse] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WarehouseState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WarehouseState | null { return this.states.get(entityId) ?? null; }
