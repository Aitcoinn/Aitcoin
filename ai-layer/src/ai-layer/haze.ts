...
export class Haze {
  private states: Map<string, HazeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('haze');
  }

  private getOrCreate(entityId: string): HazeState {
  ...
  activate(entityId: string, intensity = 50): HazeState {
    ...
    blockchainBridge.validateAIOutput('haze', { entityId, level: state.level });
    logger.info({ entityId, module: 'haze', intensity }, '[Haze] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'haze' }, '[Haze] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HazeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HazeState | null { return this.states.get(entityId) ?? null; }
