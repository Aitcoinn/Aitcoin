...
export class RunwaySys {
  private states: Map<string, RunwaySysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('runway_sys');
  }

  private getOrCreate(entityId: string): RunwaySysState {
  ...
  activate(entityId: string, intensity = 50): RunwaySysState {
    ...
    blockchainBridge.validateAIOutput('runway_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'runway_sys', intensity }, '[RunwaySys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'runway_sys' }, '[RunwaySys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RunwaySysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RunwaySysState | null { return this.states.get(entityId) ?? null; }
...
export const runwaySys = new RunwaySys();
export default runwaySys;
