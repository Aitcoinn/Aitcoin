...
export class Collision {
  private states: Map<string, CollisionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('collision');
  }

  private getOrCreate(entityId: string): CollisionState {
  ...
  activate(entityId: string, intensity = 50): CollisionState {
    ...
    blockchainBridge.validateAIOutput('collision', { entityId, level: state.level });
    logger.info({ entityId, module: 'collision', intensity }, '[Collision] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'collision' }, '[Collision] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CollisionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CollisionState | null { return this.states.get(entityId) ?? null; }
