...
export class Chair {
  private states: Map<string, ChairState> = new Map();

  constructor() {
    blockchainBridge.registerModule('chair');
  }

  private getOrCreate(entityId: string): ChairState {
  ...
  activate(entityId: string, intensity = 50): ChairState {
    ...
    blockchainBridge.validateAIOutput('chair', { entityId, level: state.level });
    logger.info({ entityId, module: 'chair', intensity }, '[Chair] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'chair' }, '[Chair] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ChairState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ChairState | null { return this.states.get(entityId) ?? null; }
