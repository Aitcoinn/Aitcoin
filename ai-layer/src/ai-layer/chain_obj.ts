...
export class ChainObj {
  private states: Map<string, ChainObjState> = new Map();

  constructor() {
    blockchainBridge.registerModule('chain_obj');
  }

  private getOrCreate(entityId: string): ChainObjState {
  ...
  activate(entityId: string, intensity = 50): ChainObjState {
    ...
    blockchainBridge.validateAIOutput('chain_obj', { entityId, level: state.level });
    logger.info({ entityId, module: 'chain_obj', intensity }, '[ChainObj] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'chain_obj' }, '[ChainObj] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ChainObjState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ChainObjState | null { return this.states.get(entityId) ?? null; }
...
export const chainObj = new ChainObj();
export default chainObj;
