...
export class Glasses {
  private states: Map<string, GlassesState> = new Map();

  constructor() {
    blockchainBridge.registerModule('glasses');
  }

  private getOrCreate(entityId: string): GlassesState {
  ...
  activate(entityId: string, intensity = 50): GlassesState {
    ...
    blockchainBridge.validateAIOutput('glasses', { entityId, level: state.level });
    logger.info({ entityId, module: 'glasses', intensity }, '[Glasses] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'glasses' }, '[Glasses] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): GlassesState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): GlassesState | null { return this.states.get(entityId) ?? null; }
...
export const glasses = new Glasses();
