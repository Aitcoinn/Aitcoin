...
export class Prince {
  private states: Map<string, PrinceState> = new Map();

  private getOrCreate(entityId: string): PrinceState {
  ...
  activate(entityId: string, intensity = 50): PrinceState {
    ...
    logger.info({ entityId, module: 'prince', intensity }, '[Prince] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'prince' }, '[Prince] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PrinceState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'prince' }, '[Prince] Updated');
    return state;
  }

  getState(entityId: string): PrinceState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'prince' }, '[Prince] Reset');
  }
