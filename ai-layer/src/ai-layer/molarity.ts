...
export class Molarity {
  private states: Map<string, MolarityState> = new Map();

  constructor() {
    blockchainBridge.registerModule('molarity');
  }

  private getOrCreate(entityId: string): MolarityState {
  ...
  activate(entityId: string, intensity = 50): MolarityState {
    ...
    blockchainBridge.validateAIOutput('molarity', { entityId, level: state.level });
    logger.info({ entityId, module: 'molarity', intensity }, '[Molarity] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'molarity' }, '[Molarity] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MolarityState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MolarityState | null { return this.states.get(entityId) ?? null; }
...
export const molarity = new Molarity();
