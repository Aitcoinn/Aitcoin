...
export class Allow {
  private states: Map<string, AllowState> = new Map();

  constructor() {
    blockchainBridge.registerModule('allow');
  }

  private getOrCreate(entityId: string): AllowState {
  ...
  activate(entityId: string, intensity = 50): AllowState {
    ...
    blockchainBridge.validateAIOutput('allow', { entityId, level: state.level });
    logger.info({ entityId, module: 'allow', intensity }, '[Allow] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'allow' }, '[Allow] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AllowState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AllowState | null { return this.states.get(entityId) ?? null; }
