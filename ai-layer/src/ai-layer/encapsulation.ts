...
export class Encapsulation {
  private states: Map<string, EncapsulationState> = new Map();

  constructor() {
    blockchainBridge.registerModule('encapsulation');
  }

  private getOrCreate(entityId: string): EncapsulationState {
  ...
  activate(entityId: string, intensity = 50): EncapsulationState {
    ...
    blockchainBridge.validateAIOutput('encapsulation', { entityId, level: state.level });
    logger.info({ entityId, module: 'encapsulation', intensity }, '[Encapsulation] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'encapsulation' }, '[Encapsulation] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EncapsulationState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): EncapsulationState | null { return this.states.get(entityId) ?? null; }
...
export const encapsulation = new Encapsulation();
