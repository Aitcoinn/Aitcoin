...
export class Hurricane {
  private states: Map<string, HurricaneState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hurricane');
  }

  private getOrCreate(entityId: string): HurricaneState {
  ...
  activate(entityId: string, intensity = 50): HurricaneState {
    ...
    blockchainBridge.validateAIOutput('hurricane', { entityId, level: state.level });
    logger.info({ entityId, module: 'hurricane', intensity }, '[Hurricane] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hurricane' }, '[Hurricane] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HurricaneState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HurricaneState | null { return this.states.get(entityId) ?? null; }
...
export const hurricane = new Hurricane();
