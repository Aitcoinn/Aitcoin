...
export class Mist {
  private states: Map<string, MistState> = new Map();

  constructor() {
    blockchainBridge.registerModule('mist');
  }

  private getOrCreate(entityId: string): MistState {
  ...
  activate(entityId: string, intensity = 50): MistState {
    ...
    blockchainBridge.validateAIOutput('mist', { entityId, level: state.level });
    logger.info({ entityId, module: 'mist', intensity }, '[Mist] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'mist' }, '[Mist] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MistState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MistState | null { return this.states.get(entityId) ?? null; }
