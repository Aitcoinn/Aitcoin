...
export class ForLoop {
  private states: Map<string, ForLoopState> = new Map();

  constructor() {
    blockchainBridge.registerModule('for_loop');
  }

  private getOrCreate(entityId: string): ForLoopState {
  ...
  activate(entityId: string, intensity = 50): ForLoopState {
    ...
    blockchainBridge.validateAIOutput('for_loop', { entityId, level: state.level });
    logger.info({ entityId, module: 'for_loop', intensity }, '[ForLoop] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'for_loop' }, '[ForLoop] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ForLoopState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ForLoopState | null { return this.states.get(entityId) ?? null; }
