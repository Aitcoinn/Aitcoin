...
export class Delete {
  private states: Map<string, DeleteState> = new Map();

  constructor() {
    blockchainBridge.registerModule('delete');
  }

  private getOrCreate(entityId: string): DeleteState {
  ...
  activate(entityId: string, intensity = 50): DeleteState {
    ...
    blockchainBridge.validateAIOutput('delete', { entityId, level: state.level });
    logger.info({ entityId, module: 'delete', intensity }, '[Delete] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'delete' }, '[Delete] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DeleteState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DeleteState | null { return this.states.get(entityId) ?? null; }
