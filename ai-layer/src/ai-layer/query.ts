...
export class Query {
  private states: Map<string, QueryState> = new Map();

  constructor() {
    blockchainBridge.registerModule('query');
  }

  private getOrCreate(entityId: string): QueryState {
  ...
  activate(entityId: string, intensity = 50): QueryState {
    ...
    blockchainBridge.validateAIOutput('query', { entityId, level: state.level });
    logger.info({ entityId, module: 'query', intensity }, '[Query] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'query' }, '[Query] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): QueryState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): QueryState | null { return this.states.get(entityId) ?? null; }
