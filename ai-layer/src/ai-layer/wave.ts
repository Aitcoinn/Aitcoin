...
export class Wave {
  private states: Map<string, WaveState> = new Map();

  constructor() {
    blockchainBridge.registerModule('wave');
  }

  private getOrCreate(entityId: string): WaveState {
  ...
  activate(entityId: string, intensity = 50): WaveState {
    ...
    blockchainBridge.validateAIOutput('wave', { entityId, level: state.level });
    logger.info({ entityId, module: 'wave', intensity }, '[Wave] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'wave' }, '[Wave] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WaveState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WaveState | null { return this.states.get(entityId) ?? null; }
