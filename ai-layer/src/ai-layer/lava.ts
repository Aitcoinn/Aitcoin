...
export class Lava {
  private states: Map<string, LavaState> = new Map();

  constructor() {
    blockchainBridge.registerModule('lava');
  }

  private getOrCreate(entityId: string): LavaState {
  ...
  activate(entityId: string, intensity = 50): LavaState {
    ...
    blockchainBridge.validateAIOutput('lava', { entityId, level: state.level });
    logger.info({ entityId, module: 'lava', intensity }, '[Lava] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'lava' }, '[Lava] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LavaState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LavaState | null { return this.states.get(entityId) ?? null; }
