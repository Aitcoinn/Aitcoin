...
export class Think {
  private states: Map<string, ThinkState> = new Map();

  constructor() {
    blockchainBridge.registerModule('think');
  }

  private getOrCreate(entityId: string): ThinkState {
  ...
  activate(entityId: string, intensity = 50): ThinkState {
    ...
    blockchainBridge.validateAIOutput('think', { entityId, level: state.level });
    logger.info({ entityId, module: 'think', intensity }, '[Think] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'think' }, '[Think] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ThinkState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ThinkState | null { return this.states.get(entityId) ?? null; }
