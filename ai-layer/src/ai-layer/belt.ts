...
export class Belt {
  private states: Map<string, BeltState> = new Map();

  constructor() {
    blockchainBridge.registerModule('belt');
  }

  private getOrCreate(entityId: string): BeltState {
  ...
  activate(entityId: string, intensity = 50): BeltState {
    ...
    blockchainBridge.validateAIOutput('belt', { entityId, level: state.level });
    logger.info({ entityId, module: 'belt', intensity }, '[Belt] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'belt' }, '[Belt] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BeltState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BeltState | null { return this.states.get(entityId) ?? null; }
