...
export class Vector {
  private states: Map<string, VectorState> = new Map();

  constructor() {
    blockchainBridge.registerModule('vector');
  }

  private getOrCreate(entityId: string): VectorState {
  ...
  activate(entityId: string, intensity = 50): VectorState {
    ...
    blockchainBridge.validateAIOutput('vector', { entityId, level: state.level });
    logger.info({ entityId, module: 'vector', intensity }, '[Vector] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'vector' }, '[Vector] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): VectorState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): VectorState | null { return this.states.get(entityId) ?? null; }
...
export const vector = new Vector();
