...
export class Australis {
  private states: Map<string, AustralisState> = new Map();

  constructor() {
    blockchainBridge.registerModule('australis');
  }

  private getOrCreate(entityId: string): AustralisState {
  ...
  activate(entityId: string, intensity = 50): AustralisState {
    ...
    blockchainBridge.validateAIOutput('australis', { entityId, level: state.level });
    logger.info({ entityId, module: 'australis', intensity }, '[Australis] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'australis' }, '[Australis] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AustralisState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AustralisState | null { return this.states.get(entityId) ?? null; }
...
export const australis = new Australis();
