...
export class Table {
  ...
  constructor() {
    blockchainBridge.registerModule('table');
  }

  private getOrCreate(entityId: string): TableState {
  ...
  activate(entityId: string, intensity = 50): TableState {
    ...
    blockchainBridge.validateAIOutput('table', { entityId, level: state.level });
    logger.info({ entityId, module: 'table', intensity }, '[Table] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'table' }, '[Table] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TableState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TableState | null { return this.states.get(entityId) ?? null; }
