...
export class FakeExp {
  private states: Map<string, FakeExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('fake_exp');
  }

  private getOrCreate(entityId: string): FakeExpState {
  ...
  activate(entityId: string, intensity = 50): FakeExpState {
    ...
    blockchainBridge.validateAIOutput('fake_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'fake_exp', intensity }, '[FakeExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fake_exp' }, '[FakeExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FakeExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FakeExpState | null { return this.states.get(entityId) ?? null; }
...
export const fakeExp = new FakeExp();
export default fakeExp;
