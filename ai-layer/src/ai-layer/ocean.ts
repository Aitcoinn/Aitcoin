...
export class Ocean {
  private states: Map<string, OceanState> = new Map();

  constructor() {
    blockchainBridge.registerModule('ocean');
  }

  private getOrCreate(entityId: string): OceanState {
  ...
  activate(entityId: string, intensity = 50): OceanState {
    ...
    blockchainBridge.validateAIOutput('ocean', { entityId, level: state.level });
    logger.info({ entityId, module: 'ocean', intensity }, '[Ocean] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ocean' }, '[Ocean] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): OceanState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): OceanState | null { return this.states.get(entityId) ?? null; }
