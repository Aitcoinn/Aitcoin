...
export class DriverJob {
  private states: Map<string, DriverJobState> = new Map();

  constructor() {
    blockchainBridge.registerModule('driver_job');
  }

  private getOrCreate(entityId: string): DriverJobState {
  ...
  activate(entityId: string, intensity = 50): DriverJobState {
    ...
    blockchainBridge.validateAIOutput('driver_job', { entityId, level: state.level });
    logger.info({ entityId, module: 'driver_job', intensity }, '[DriverJob] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'driver_job' }, '[DriverJob] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DriverJobState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DriverJobState | null { return this.states.get(entityId) ?? null; }
...
export const driverJob = new DriverJob();
