...
export class Money {
  private states: Map<string, MoneyState> = new Map();

  constructor() {
    blockchainBridge.registerModule('money');
  }

  private getOrCreate(entityId: string): MoneyState {
  ...
  activate(entityId: string, intensity = 50): MoneyState {
    ...
    blockchainBridge.validateAIOutput('money', { entityId, level: state.level });
    logger.info({ entityId, module: 'money', intensity }, '[Money] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'money' }, '[Money] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MoneyState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MoneyState | null { return this.states.get(entityId) ?? null; }
