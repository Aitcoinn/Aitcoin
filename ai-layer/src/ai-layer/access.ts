...
export class Access {
  private states: Map<string, AccessState> = new Map();

  constructor() {
    blockchainBridge.registerModule('access');
  }

  private getOrCreate(entityId: string): AccessState {
  ...
  activate(entityId: string, intensity = 50): AccessState {
    ...
    blockchainBridge.validateAIOutput('access', { entityId, level: state.level });
    logger.info({ entityId, module: 'access', intensity }, '[Access] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'access' }, '[Access] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AccessState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AccessState | null { return this.states.get(entityId) ?? null; }
