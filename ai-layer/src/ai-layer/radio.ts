...
export class Radio {
  private states: Map<string, RadioState> = new Map();

  constructor() {
    blockchainBridge.registerModule('radio');
  }

  private getOrCreate(entityId: string): RadioState {
  ...
  activate(entityId: string, intensity = 50): RadioState {
    ...
    blockchainBridge.validateAIOutput('radio', { entityId, level: state.level });
    logger.info({ entityId, module: 'radio', intensity }, '[Radio] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'radio' }, '[Radio] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RadioState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RadioState | null { return this.states.get(entityId) ?? null; }
