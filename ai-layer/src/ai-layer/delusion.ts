...
export class Delusion {
  private states: Map<string, DelusionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('delusion');
  }

  private getOrCreate(entityId: string): DelusionState {
  ...
  activate(entityId: string, intensity = 50): DelusionState {
    ...
    blockchainBridge.validateAIOutput('delusion', { entityId, level: state.level });
    logger.info({ entityId, module: 'delusion', intensity }, '[Delusion] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'delusion' }, '[Delusion] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DelusionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DelusionState | null { return this.states.get(entityId) ?? null; }
...
export const delusion = new Delusion();
