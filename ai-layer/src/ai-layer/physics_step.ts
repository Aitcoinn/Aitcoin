...
export class PhysicsStep {
  private states: Map<string, PhysicsStepState> = new Map();

  constructor() {
    blockchainBridge.registerModule('physics_step');
  }

  private getOrCreate(entityId: string): PhysicsStepState {
  ...
  activate(entityId: string, intensity = 50): PhysicsStepState {
    ...
    blockchainBridge.validateAIOutput('physics_step', { entityId, level: state.level });
    logger.info({ entityId, module: 'physics_step', intensity }, '[PhysicsStep] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'physics_step' }, '[PhysicsStep] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PhysicsStepState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PhysicsStepState | null { return this.states.get(entityId) ?? null; }
