...
export class Overlap {
  private states: Map<string, OverlapState> = new Map();

  constructor() {
    blockchainBridge.registerModule('overlap');
  }

  private getOrCreate(entityId: string): OverlapState {
  ...
  activate(entityId: string, intensity = 50): OverlapState {
    ...
    blockchainBridge.validateAIOutput('overlap', { entityId, level: state.level });
    logger.info({ entityId, module: 'overlap', intensity }, '[Overlap] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'overlap' }, '[Overlap] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): OverlapState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): OverlapState | null { return this.states.get(entityId) ?? null; }
