...
export class Ash {
  private states: Map<string, AshState> = new Map();

  constructor() {
    blockchainBridge.registerModule('ash');
  }

  private getOrCreate(entityId: string): AshState {
  ...
  activate(entityId: string, intensity = 50): AshState {
    ...
    blockchainBridge.validateAIOutput('ash', { entityId, level: state.level });
    logger.info({ entityId, module: 'ash', intensity }, '[Ash] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ash' }, '[Ash] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AshState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AshState | null { return this.states.get(entityId) ?? null; }
