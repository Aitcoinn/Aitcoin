...
export class Cold {
  private states: Map<string, ColdState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cold');
  }

  private getOrCreate(entityId: string): ColdState {
  ...
  activate(entityId: string, intensity = 50): ColdState {
    ...
    blockchainBridge.validateAIOutput('cold', { entityId, level: state.level });
    logger.info({ entityId, module: 'cold', intensity }, '[Cold] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cold' }, '[Cold] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ColdState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ColdState | null { return this.states.get(entityId) ?? null; }
