...
export class Gloves {
  private states: Map<string, GlovesState> = new Map();

  constructor() {
    blockchainBridge.registerModule('gloves');
  }

  private getOrCreate(entityId: string): GlovesState {
  ...
  activate(entityId: string, intensity = 50): GlovesState {
    ...
    blockchainBridge.validateAIOutput('gloves', { entityId, level: state.level });
    logger.info({ entityId, module: 'gloves', intensity }, '[Gloves] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'gloves' }, '[Gloves] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): GlovesState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): GlovesState | null { return this.states.get(entityId) ?? null; }
...
export const gloves = new Gloves();
