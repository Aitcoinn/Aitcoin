...
export class Fog {
  private states: Map<string, FogState> = new Map();

  constructor() {
    blockchainBridge.registerModule('fog');
  }

  private getOrCreate(entityId: string): FogState {
  ...
  activate(entityId: string, intensity = 50): FogState {
    ...
    blockchainBridge.validateAIOutput('fog', { entityId, level: state.level });
    logger.info({ entityId, module: 'fog', intensity }, '[Fog] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fog' }, '[Fog] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FogState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FogState | null { return this.states.get(entityId) ?? null; }
