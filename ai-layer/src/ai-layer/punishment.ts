...
export class Punishment {
  private states: Map<string, PunishmentState> = new Map();

  constructor() {
    blockchainBridge.registerModule('punishment');
  }

  private getOrCreate(entityId: string): PunishmentState {
  ...
  activate(entityId: string, intensity = 50): PunishmentState {
    ...
    blockchainBridge.validateAIOutput('punishment', { entityId, level: state.level });
    logger.info({ entityId, module: 'punishment', intensity }, '[Punishment] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'punishment' }, '[Punishment] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PunishmentState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PunishmentState | null { return this.states.get(entityId) ?? null; }
...
export const punishment = new Punishment();
