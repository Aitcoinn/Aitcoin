...
export class Sleep {
  private states: Map<string, SleepState> = new Map();

  constructor() {
    blockchainBridge.registerModule('sleep');
  }

  private getOrCreate(entityId: string): SleepState {
  ...
  activate(entityId: string, intensity = 50): SleepState {
    ...
    blockchainBridge.validateAIOutput('sleep', { entityId, level: state.level });
    logger.info({ entityId, module: 'sleep', intensity }, '[Sleep] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'sleep' }, '[Sleep] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SleepState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SleepState | null { return this.states.get(entityId) ?? null; }
