...
export class Transform {
  ...
  constructor() {
    blockchainBridge.registerModule('transform');
  }

  private getOrCreate(entityId: string): TransformState {
  ...
  activate(entityId: string, intensity = 50): TransformState {
    ...
    blockchainBridge.validateAIOutput('transform', { entityId, level: state.level });
    logger.info({ entityId, module: 'transform', intensity }, '[Transform] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'transform' }, '[Transform] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TransformState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TransformState | null { return this.states.get(entityId) ?? null; }
