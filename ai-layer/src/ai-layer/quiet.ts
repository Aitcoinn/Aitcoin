...
export class Quiet {
  private states: Map<string, QuietState> = new Map();

  constructor() {
    blockchainBridge.registerModule('quiet');
  }

  private getOrCreate(entityId: string): QuietState {
  ...
  activate(entityId: string, intensity = 50): QuietState {
    ...
    blockchainBridge.validateAIOutput('quiet', { entityId, level: state.level });
    logger.info({ entityId, module: 'quiet', intensity }, '[Quiet] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'quiet' }, '[Quiet] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): QuietState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): QuietState | null { return this.states.get(entityId) ?? null; }
