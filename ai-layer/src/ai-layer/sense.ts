...
export class Sense {
  private states: Map<string, SenseState> = new Map();

  constructor() {
    blockchainBridge.registerModule('sense');
  }

  private getOrCreate(entityId: string): SenseState {
  ...
  activate(entityId: string, intensity = 50): SenseState {
    ...
    blockchainBridge.validateAIOutput('sense', { entityId, level: state.level });
    logger.info({ entityId, module: 'sense', intensity }, '[Sense] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'sense' }, '[Sense] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SenseState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SenseState | null { return this.states.get(entityId) ?? null; }
