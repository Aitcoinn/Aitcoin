...
export class Percent {
  private states: Map<string, PercentState> = new Map();

  constructor() {
    blockchainBridge.registerModule('percent');
  }

  private getOrCreate(entityId: string): PercentState {
  ...
  activate(entityId: string, intensity = 50): PercentState {
    ...
    blockchainBridge.validateAIOutput('percent', { entityId, level: state.level });
    logger.info({ entityId, module: 'percent', intensity }, '[Percent] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'percent' }, '[Percent] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PercentState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PercentState | null { return this.states.get(entityId) ?? null; }
