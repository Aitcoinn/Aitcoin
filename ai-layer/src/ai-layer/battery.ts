...
export class Battery {
  ...
  constructor() {
    blockchainBridge.registerModule('battery');
  }

  private getOrCreate(entityId: string): BatteryState {
  ...
  activate(entityId: string, intensity = 50): BatteryState {
    ...
    blockchainBridge.validateAIOutput('battery', { entityId, level: state.level });
    logger.info({ entityId, module: 'battery', intensity }, '[Battery] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'battery' }, '[Battery] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BatteryState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BatteryState | null { return this.states.get(entityId) ?? null; }
