...
export class Bicycle {
  private states: Map<string, BicycleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('bicycle');
  }

  private getOrCreate(entityId: string): BicycleState {
  ...
  activate(entityId: string, intensity = 50): BicycleState {
    ...
    blockchainBridge.validateAIOutput('bicycle', { entityId, level: state.level });
    logger.info({ entityId, module: 'bicycle', intensity }, '[Bicycle] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'bicycle' }, '[Bicycle] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BicycleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BicycleState | null { return this.states.get(entityId) ?? null; }
...
export const bicycle = new Bicycle();
