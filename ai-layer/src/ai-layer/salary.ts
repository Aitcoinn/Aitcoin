...
export class Salary {
  private states: Map<string, SalaryState> = new Map();

  constructor() {
    blockchainBridge.registerModule('salary');
  }

  private getOrCreate(entityId: string): SalaryState {
  ...
  activate(entityId: string, intensity = 50): SalaryState {
    ...
    blockchainBridge.validateAIOutput('salary', { entityId, level: state.level });
    logger.info({ entityId, module: 'salary', intensity }, '[Salary] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'salary' }, '[Salary] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SalaryState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SalaryState | null { return this.states.get(entityId) ?? null; }
...
export const salary = new Salary();
