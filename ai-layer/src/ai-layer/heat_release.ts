...
export class HeatRelease {
  private states: Map<string, HeatReleaseState> = new Map();

  constructor() {
    blockchainBridge.registerModule('heat_release');
  }

  private getOrCreate(entityId: string): HeatReleaseState {
  ...
  activate(entityId: string, intensity = 50): HeatReleaseState {
    ...
    blockchainBridge.validateAIOutput('heat_release', { entityId, level: state.level });
    logger.info({ entityId, module: 'heat_release', intensity }, '[HeatRelease] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'heat_release' }, '[HeatRelease] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HeatReleaseState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HeatReleaseState | null { return this.states.get(entityId) ?? null; }
...
export const heatRelease = new HeatRelease();
export default heatRelease;
