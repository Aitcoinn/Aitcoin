...
export class SouthPole {
  private states: Map<string, SouthPoleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('south_pole');
  }

  private getOrCreate(entityId: string): SouthPoleState {
  ...
  activate(entityId: string, intensity = 50): SouthPoleState {
    ...
    blockchainBridge.validateAIOutput('south_pole', { entityId, level: state.level });
    logger.info({ entityId, module: 'south_pole', intensity }, '[SouthPole] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'south_pole' }, '[SouthPole] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SouthPoleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SouthPoleState | null { return this.states.get(entityId) ?? null; }
...
export const southPole = new SouthPole();
