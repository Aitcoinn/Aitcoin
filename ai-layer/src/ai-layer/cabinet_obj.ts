...
/**
 * CABINET_OBJ — Module #10518
 * Cabinet object system
 * Kategori: ALAT RUMAH TANGGA
 * Blockchain: Terhubung ke AITCOIN (ATC)
 */
export interface CabinetObjState {
  entityId: string;
  active: boolean;
  level: number;
  intensity: number;
...
export class CabinetObj {
  private states: Map<string, CabinetObjState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cabinet_obj');
  }

  private getOrCreate(entityId: string): CabinetObjState {
  ...
  activate(entityId: string, intensity = 50): CabinetObjState {
    ...
    blockchainBridge.validateAIOutput('cabinet_obj', { entityId, level: state.level });
    logger.info({ entityId, module: 'cabinet_obj', intensity }, '[CabinetObj] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cabinet_obj' }, '[CabinetObj] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CabinetObjState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CabinetObjState | null { return this.states.get(entityId) ?? null; }
...
export const cabinetObj = new CabinetObj();
