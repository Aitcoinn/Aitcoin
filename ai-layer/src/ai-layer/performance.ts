...
export class Performance {
  private states: Map<string, PerformanceState> = new Map();

  constructor() {
    blockchainBridge.registerModule('performance');
  }

  private getOrCreate(entityId: string): PerformanceState {
  ...
  activate(entityId: string, intensity = 50): PerformanceState {
    ...
    blockchainBridge.validateAIOutput('performance', { entityId, level: state.level });
    logger.info({ entityId, module: 'performance', intensity }, '[Performance] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'performance' }, '[Performance] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PerformanceState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PerformanceState | null { return this.states.get(entityId) ?? null; }
