...
export class Character {
  ...
  private getOrCreate(entityId: string): CharacterState {
  ...
  activate(entityId: string, intensity = 50): CharacterState {
    ...
    logger.info({ entityId, module: 'character', intensity }, '[Character] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'character' }, '[Character] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CharacterState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'character' }, '[Character] Updated');
    return state;
  }

  getState(entityId: string): CharacterState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'character' }, '[Character] Reset');
  }
