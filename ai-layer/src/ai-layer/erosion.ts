...
export class Erosion {
  private states: Map<string, ErosionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('erosion');
  }

  private getOrCreate(entityId: string): ErosionState {
  ...
  activate(entityId: string, intensity = 50): ErosionState {
    ...
    blockchainBridge.validateAIOutput('erosion', { entityId, level: state.level });
    logger.info({ entityId, module: 'erosion', intensity }, '[Erosion] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'erosion' }, '[Erosion] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ErosionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ErosionState | null { return this.states.get(entityId) ?? null; }
...
export const erosion = new Erosion();
