...
export class Dissolve {
  private states: Map<string, DissolveState> = new Map();

  constructor() {
    blockchainBridge.registerModule('dissolve');
  }

  private getOrCreate(entityId: string): DissolveState {
  ...
  activate(entityId: string, intensity = 50): DissolveState {
    ...
    blockchainBridge.validateAIOutput('dissolve', { entityId, level: state.level });
    logger.info({ entityId, module: 'dissolve', intensity }, '[Dissolve] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'dissolve' }, '[Dissolve] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DissolveState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DissolveState | null { return this.states.get(entityId) ?? null; }
...
export const dissolve = new Dissolve();
