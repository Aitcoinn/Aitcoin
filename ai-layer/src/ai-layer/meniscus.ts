...
export class Meniscus {
  private states: Map<string, MeniscusState> = new Map();

  constructor() {
    blockchainBridge.registerModule('meniscus');
  }

  private getOrCreate(entityId: string): MeniscusState {
  ...
  activate(entityId: string, intensity = 50): MeniscusState {
    ...
    blockchainBridge.validateAIOutput('meniscus', { entityId, level: state.level });
    logger.info({ entityId, module: 'meniscus', intensity }, '[Meniscus] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'meniscus' }, '[Meniscus] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MeniscusState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MeniscusState | null { return this.states.get(entityId) ?? null; }
...
export const meniscus = new Meniscus();
