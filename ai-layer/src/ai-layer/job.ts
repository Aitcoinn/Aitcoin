...
export class Job {
  ...
  constructor() {
    blockchainBridge.registerModule('job');
  }

  private getOrCreate(entityId: string): JobState {
  ...
  activate(entityId: string, intensity = 50): JobState {
    ...
    blockchainBridge.validateAIOutput('job', { entityId, level: state.level });
    logger.info({ entityId, module: 'job', intensity }, '[Job] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'job' }, '[Job] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): JobState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): JobState | null { return this.states.get(entityId) ?? null; }
