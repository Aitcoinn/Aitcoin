...
export class Evaporate {
  private states: Map<string, EvaporateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('evaporate');
  }

  private getOrCreate(entityId: string): EvaporateState {
  ...
  activate(entityId: string, intensity = 50): EvaporateState {
    ...
    blockchainBridge.validateAIOutput('evaporate', { entityId, level: state.level });
    logger.info({ entityId, module: 'evaporate', intensity }, '[Evaporate] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'evaporate' }, '[Evaporate] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EvaporateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): EvaporateState | null { return this.states.get(entityId) ?? null; }
...
export const evaporate = new Evaporate();
