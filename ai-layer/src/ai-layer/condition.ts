...
export class Condition {
  private states: Map<string, ConditionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('condition');
  }

  private getOrCreate(entityId: string): ConditionState {
  ...
  activate(entityId: string, intensity = 50): ConditionState {
    ...
    blockchainBridge.validateAIOutput('condition', { entityId, level: state.level });
    logger.info({ entityId, module: 'condition', intensity }, '[Condition] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'condition' }, '[Condition] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ConditionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ConditionState | null { return this.states.get(entityId) ?? null; }
