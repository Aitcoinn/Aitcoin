...
export class Consciousness {
  ...
  private getOrCreate(entityId: string): ConsciousnessState {
  ...
  activate(entityId: string, intensity = 50): ConsciousnessState {
    ...
    logger.info({ entityId, module: 'consciousness', intensity }, '[Consciousness] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'consciousness' }, '[Consciousness] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ConsciousnessState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'consciousness' }, '[Consciousness] Updated');
    return state;
  }

  getState(entityId: string): ConsciousnessState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'consciousness' }, '[Consciousness] Reset');
  }
