...
export class Capillary {
  private states: Map<string, CapillaryState> = new Map();

  constructor() {
    blockchainBridge.registerModule('capillary');
  }

  private getOrCreate(entityId: string): CapillaryState {
  ...
  activate(entityId: string, intensity = 50): CapillaryState {
    ...
    blockchainBridge.validateAIOutput('capillary', { entityId, level: state.level });
    logger.info({ entityId, module: 'capillary', intensity }, '[Capillary] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'capillary' }, '[Capillary] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CapillaryState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CapillaryState | null { return this.states.get(entityId) ?? null; }
...
export const capillary = new Capillary();
