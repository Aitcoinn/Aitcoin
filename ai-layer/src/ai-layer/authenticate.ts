...
export class Authenticate {
  private states: Map<string, AuthenticateState> = new Map();

  constructor() {
    blockchainBridge.registerModule('authenticate');
  }

  private getOrCreate(entityId: string): AuthenticateState {
  ...
  activate(entityId: string, intensity = 50): AuthenticateState {
    ...
    blockchainBridge.validateAIOutput('authenticate', { entityId, level: state.level });
    logger.info({ entityId, module: 'authenticate', intensity }, '[Authenticate] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'authenticate' }, '[Authenticate] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AuthenticateState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AuthenticateState | null { return this.states.get(entityId) ?? null; }
