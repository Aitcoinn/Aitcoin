...
export class Wisdom {
  ...
  private getOrCreate(entityId: string): WisdomState {
  ...
  activate(entityId: string, intensity = 50): WisdomState {
    ...
    logger.info({ entityId, module: 'wisdom', intensity }, '[Wisdom] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'wisdom' }, '[Wisdom] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WisdomState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'wisdom' }, '[Wisdom] Updated');
    return state;
  }

  getState(entityId: string): WisdomState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'wisdom' }, '[Wisdom] Reset');
  }
...
export const wisdom = new Wisdom();
