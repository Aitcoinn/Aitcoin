...
export class Encrypt {
  private states: Map<string, EncryptState> = new Map();

  constructor() {
    blockchainBridge.registerModule('encrypt');
  }

  private getOrCreate(entityId: string): EncryptState {
  ...
  activate(entityId: string, intensity = 50): EncryptState {
    ...
    blockchainBridge.validateAIOutput('encrypt', { entityId, level: state.level });
    logger.info({ entityId, module: 'encrypt', intensity }, '[Encrypt] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'encrypt' }, '[Encrypt] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EncryptState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): EncryptState | null { return this.states.get(entityId) ?? null; }
