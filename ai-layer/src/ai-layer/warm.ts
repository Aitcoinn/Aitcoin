...
export class Warm {
  private states: Map<string, WarmState> = new Map();

  constructor() {
    blockchainBridge.registerModule('warm');
  }

  private getOrCreate(entityId: string): WarmState {
  ...
  activate(entityId: string, intensity = 50): WarmState {
    ...
    blockchainBridge.validateAIOutput('warm', { entityId, level: state.level });
    logger.info({ entityId, module: 'warm', intensity }, '[Warm] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'warm' }, '[Warm] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WarmState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WarmState | null { return this.states.get(entityId) ?? null; }
