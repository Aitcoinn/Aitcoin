...
export class Filter {
  ...
  constructor() {
    blockchainBridge.registerModule('filter');
  }

  private getOrCreate(entityId: string): FilterState {
  ...
  activate(entityId: string, intensity = 50): FilterState {
    ...
    blockchainBridge.validateAIOutput('filter', { entityId, level: state.level });
    logger.info({ entityId, module: 'filter', intensity }, '[Filter] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'filter' }, '[Filter] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FilterState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FilterState | null { return this.states.get(entityId) ?? null; }
