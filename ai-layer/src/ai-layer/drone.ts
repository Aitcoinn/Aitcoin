...
export class Drone {
  private states: Map<string, DroneState> = new Map();

  constructor() {
    blockchainBridge.registerModule('drone');
  }

  private getOrCreate(entityId: string): DroneState {
  ...
  activate(entityId: string, intensity = 50): DroneState {
    ...
    blockchainBridge.validateAIOutput('drone', { entityId, level: state.level });
    logger.info({ entityId, module: 'drone', intensity }, '[Drone] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'drone' }, '[Drone] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DroneState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DroneState | null { return this.states.get(entityId) ?? null; }
