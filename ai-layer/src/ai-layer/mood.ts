...
export class Mood {
  ...
  private getOrCreate(entityId: string): MoodState {
  ...
  activate(entityId: string, intensity = 50): MoodState {
    ...
    logger.info({ entityId, module: 'mood', intensity }, '[Mood] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'mood' }, '[Mood] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MoodState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'mood' }, '[Mood] Updated');
    return state;
  }

  getState(entityId: string): MoodState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'mood' }, '[Mood] Reset');
  }
