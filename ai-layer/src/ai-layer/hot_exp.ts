...
export class HotExp {
  private states: Map<string, HotExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hot_exp');
  }

  private getOrCreate(entityId: string): HotExpState {
  ...
  activate(entityId: string, intensity = 50): HotExpState {
    ...
    blockchainBridge.validateAIOutput('hot_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'hot_exp', intensity }, '[HotExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hot_exp' }, '[HotExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HotExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HotExpState | null { return this.states.get(entityId) ?? null; }
...
export const hotExp = new HotExp();
export default hotExp;
