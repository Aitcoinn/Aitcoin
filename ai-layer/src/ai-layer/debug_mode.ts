...
export class DebugMode {
  private states: Map<string, DebugModeState> = new Map();

  constructor() {
    blockchainBridge.registerModule('debug_mode');
  }

  private getOrCreate(entityId: string): DebugModeState {
  ...
  activate(entityId: string, intensity = 50): DebugModeState {
    ...
    blockchainBridge.validateAIOutput('debug_mode', { entityId, level: state.level });
    logger.info({ entityId, module: 'debug_mode', intensity }, '[DebugMode] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'debug_mode' }, '[DebugMode] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DebugModeState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DebugModeState | null { return this.states.get(entityId) ?? null; }
