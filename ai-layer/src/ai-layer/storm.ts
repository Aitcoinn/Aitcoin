...
export class Storm {
  private states: Map<string, StormState> = new Map();

  constructor() {
    blockchainBridge.registerModule('storm');
  }

  private getOrCreate(entityId: string): StormState {
  ...
  activate(entityId: string, intensity = 50): StormState {
    ...
    blockchainBridge.validateAIOutput('storm', { entityId, level: state.level });
    logger.info({ entityId, module: 'storm', intensity }, '[Storm] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'storm' }, '[Storm] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): StormState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): StormState | null { return this.states.get(entityId) ?? null; }
