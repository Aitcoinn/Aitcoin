...
export class Motorcycle {
  private states: Map<string, MotorcycleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('motorcycle');
  }

  private getOrCreate(entityId: string): MotorcycleState {
  ...
  activate(entityId: string, intensity = 50): MotorcycleState {
    ...
    blockchainBridge.validateAIOutput('motorcycle', { entityId, level: state.level });
    logger.info({ entityId, module: 'motorcycle', intensity }, '[Motorcycle] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'motorcycle' }, '[Motorcycle] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MotorcycleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MotorcycleState | null { return this.states.get(entityId) ?? null; }
