...
export class Toxic {
  private states: Map<string, ToxicState> = new Map();

  constructor() {
    blockchainBridge.registerModule('toxic');
  }

  private getOrCreate(entityId: string): ToxicState {
  ...
  activate(entityId: string, intensity = 50): ToxicState {
    ...
    blockchainBridge.validateAIOutput('toxic', { entityId, level: state.level });
    logger.info({ entityId, module: 'toxic', intensity }, '[Toxic] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'toxic' }, '[Toxic] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ToxicState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ToxicState | null { return this.states.get(entityId) ?? null; }
