...
export class SwitchElec {
  private states: Map<string, SwitchElecState> = new Map();

  constructor() {
    blockchainBridge.registerModule('switch_elec');
  }

  private getOrCreate(entityId: string): SwitchElecState {
  ...
  activate(entityId: string, intensity = 50): SwitchElecState {
    ...
    blockchainBridge.validateAIOutput('switch_elec', { entityId, level: state.level });
    logger.info({ entityId, module: 'switch_elec', intensity }, '[SwitchElec] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'switch_elec' }, '[SwitchElec] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SwitchElecState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SwitchElecState | null { return this.states.get(entityId) ?? null; }
...
export const switchElec = new SwitchElec();
export default switchElec;
