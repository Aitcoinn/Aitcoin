...
export class Smog {
  private states: Map<string, SmogState> = new Map();

  constructor() {
    blockchainBridge.registerModule('smog');
  }

  private getOrCreate(entityId: string): SmogState {
  ...
  activate(entityId: string, intensity = 50): SmogState {
    ...
    blockchainBridge.validateAIOutput('smog', { entityId, level: state.level });
    logger.info({ entityId, module: 'smog', intensity }, '[Smog] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'smog' }, '[Smog] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SmogState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SmogState | null { return this.states.get(entityId) ?? null; }
