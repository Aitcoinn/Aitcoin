...
export class Tax {
  private states: Map<string, TaxState> = new Map();

  constructor() {
    blockchainBridge.registerModule('tax');
  }

  private getOrCreate(entityId: string): TaxState {
  ...
  activate(entityId: string, intensity = 50): TaxState {
    ...
    blockchainBridge.validateAIOutput('tax', { entityId, level: state.level });
    logger.info({ entityId, module: 'tax', intensity }, '[Tax] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'tax' }, '[Tax] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TaxState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TaxState | null { return this.states.get(entityId) ?? null; }
