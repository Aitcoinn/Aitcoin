...
export class UnconsciousExp {
  private states: Map<string, UnconsciousExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('unconscious_exp');
  }

  private getOrCreate(entityId: string): UnconsciousExpState {
  ...
  activate(entityId: string, intensity = 50): UnconsciousExpState {
    ...
    blockchainBridge.validateAIOutput('unconscious_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'unconscious_exp', intensity }, '[UnconsciousExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'unconscious_exp' }, '[UnconsciousExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): UnconsciousExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): UnconsciousExpState | null { return this.states.get(entityId) ?? null; }
...
export const unconsciousExp = new UnconsciousExp();
export default unconsciousExp;
