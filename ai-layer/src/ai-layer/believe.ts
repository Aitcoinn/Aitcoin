...
export class Believe {
  private states: Map<string, BelieveState> = new Map();

  constructor() {
    blockchainBridge.registerModule('believe');
  }

  private getOrCreate(entityId: string): BelieveState {
  ...
  activate(entityId: string, intensity = 50): BelieveState {
    ...
    blockchainBridge.validateAIOutput('believe', { entityId, level: state.level });
    logger.info({ entityId, module: 'believe', intensity }, '[Believe] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'believe' }, '[Believe] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BelieveState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BelieveState | null { return this.states.get(entityId) ?? null; }
...
export const believe = new Believe();
