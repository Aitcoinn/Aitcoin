...
export class WeightExp {
  private states: Map<string, WeightExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('weight_exp');
  }

  private getOrCreate(entityId: string): WeightExpState {
  ...
  activate(entityId: string, intensity = 50): WeightExpState {
    ...
    blockchainBridge.validateAIOutput('weight_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'weight_exp', intensity }, '[WeightExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'weight_exp' }, '[WeightExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WeightExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WeightExpState | null { return this.states.get(entityId) ?? null; }
...
export const weightExp = new WeightExp();
export default weightExp;
