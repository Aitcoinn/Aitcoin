...
export class MillSys {
  private states: Map<string, MillSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('mill_sys');
  }

  private getOrCreate(entityId: string): MillSysState {
  ...
  activate(entityId: string, intensity = 50): MillSysState {
    ...
    blockchainBridge.validateAIOutput('mill_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'mill_sys', intensity }, '[MillSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'mill_sys' }, '[MillSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MillSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MillSysState | null { return this.states.get(entityId) ?? null; }
...
export const millSys = new MillSys();
export default millSys;
