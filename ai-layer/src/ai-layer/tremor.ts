...
export class Tremor {
  private states: Map<string, TremorState> = new Map();

  constructor() {
    blockchainBridge.registerModule('tremor');
  }

  private getOrCreate(entityId: string): TremorState {
  ...
  activate(entityId: string, intensity = 50): TremorState {
    ...
    blockchainBridge.validateAIOutput('tremor', { entityId, level: state.level });
    logger.info({ entityId, module: 'tremor', intensity }, '[Tremor] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'tremor' }, '[Tremor] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TremorState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TremorState | null { return this.states.get(entityId) ?? null; }
...
export const tremor = new Tremor();
