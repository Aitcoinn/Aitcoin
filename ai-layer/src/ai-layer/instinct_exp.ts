...
export class InstinctExp {
  private states: Map<string, InstinctExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('instinct_exp');
  }

  private getOrCreate(entityId: string): InstinctExpState {
  ...
  activate(entityId: string, intensity = 50): InstinctExpState {
    ...
    blockchainBridge.validateAIOutput('instinct_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'instinct_exp', intensity }, '[InstinctExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'instinct_exp' }, '[InstinctExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): InstinctExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): InstinctExpState | null { return this.states.get(entityId) ?? null; }
...
export const instinctExp = new InstinctExp();
export default instinctExp;
