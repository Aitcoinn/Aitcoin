...
export class Idea {
  private states: Map<string, IdeaState> = new Map();

  private getOrCreate(entityId: string): IdeaState {
  ...
  activate(entityId: string, intensity = 50): IdeaState {
    ...
    logger.info({ entityId, module: 'idea', intensity }, '[Idea] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'idea' }, '[Idea] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): IdeaState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'idea' }, '[Idea] Updated');
    return state;
  }

  getState(entityId: string): IdeaState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'idea' }, '[Idea] Reset');
  }
