...
export class Decrypt {
  private states: Map<string, DecryptState> = new Map();

  constructor() {
    blockchainBridge.registerModule('decrypt');
  }

  private getOrCreate(entityId: string): DecryptState {
  ...
  activate(entityId: string, intensity = 50): DecryptState {
    ...
    blockchainBridge.validateAIOutput('decrypt', { entityId, level: state.level });
    logger.info({ entityId, module: 'decrypt', intensity }, '[Decrypt] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'decrypt' }, '[Decrypt] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DecryptState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DecryptState | null { return this.states.get(entityId) ?? null; }
