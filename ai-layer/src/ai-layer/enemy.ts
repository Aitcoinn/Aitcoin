...
export class Enemy {
  ...
  private getOrCreate(entityId: string): EnemyState {
  ...
  activate(entityId: string, intensity = 50): EnemyState {
    ...
    logger.info({ entityId, module: 'enemy', intensity }, '[Enemy] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'enemy' }, '[Enemy] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): EnemyState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'enemy' }, '[Enemy] Updated');
    return state;
  }

  getState(entityId: string): EnemyState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'enemy' }, '[Enemy] Reset');
  }
