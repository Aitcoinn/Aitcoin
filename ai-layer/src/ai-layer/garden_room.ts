...
export class GardenRoom {
  private states: Map<string, GardenRoomState> = new Map();

  constructor() {
    blockchainBridge.registerModule('garden_room');
  }

  private getOrCreate(entityId: string): GardenRoomState {
  ...
  activate(entityId: string, intensity = 50): GardenRoomState {
    ...
    blockchainBridge.validateAIOutput('garden_room', { entityId, level: state.level });
    logger.info({ entityId, module: 'garden_room', intensity }, '[GardenRoom] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'garden_room' }, '[GardenRoom] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): GardenRoomState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): GardenRoomState | null { return this.states.get(entityId) ?? null; }
...
export const gardenRoom = new GardenRoom();
