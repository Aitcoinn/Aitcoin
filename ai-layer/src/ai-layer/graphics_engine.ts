...
export class GraphicsEngine {
  private states: Map<string, GraphicsEngineState> = new Map();

  constructor() {
    blockchainBridge.registerModule('graphics_engine');
  }

  private getOrCreate(entityId: string): GraphicsEngineState {
  ...
  activate(entityId: string, intensity = 50): GraphicsEngineState {
    ...
    blockchainBridge.validateAIOutput('graphics_engine', { entityId, level: state.level });
    logger.info({ entityId, module: 'graphics_engine', intensity }, '[GraphicsEngine] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'graphics_engine' }, '[GraphicsEngine] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): GraphicsEngineState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): GraphicsEngineState | null { return this.states.get(entityId) ?? null; }
