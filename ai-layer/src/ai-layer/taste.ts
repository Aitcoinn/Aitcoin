...
export class Taste {
  private states: Map<string, TasteState> = new Map();

  constructor() {
    blockchainBridge.registerModule('taste');
  }

  private getOrCreate(entityId: string): TasteState {
  ...
  activate(entityId: string, intensity = 50): TasteState {
    ...
    blockchainBridge.validateAIOutput('taste', { entityId, level: state.level });
    logger.info({ entityId, module: 'taste', intensity }, '[Taste] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'taste' }, '[Taste] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TasteState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TasteState | null { return this.states.get(entityId) ?? null; }
