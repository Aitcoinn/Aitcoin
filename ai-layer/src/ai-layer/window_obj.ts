...
export class WindowObj {
  private states: Map<string, WindowObjState> = new Map();

  constructor() {
    blockchainBridge.registerModule('window_obj');
  }

  private getOrCreate(entityId: string): WindowObjState {
  ...
  activate(entityId: string, intensity = 50): WindowObjState {
    ...
    blockchainBridge.validateAIOutput('window_obj', { entityId, level: state.level });
    logger.info({ entityId, module: 'window_obj', intensity }, '[WindowObj] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'window_obj' }, '[WindowObj] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WindowObjState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WindowObjState | null { return this.states.get(entityId) ?? null; }
...
export const windowObj = new WindowObj();
