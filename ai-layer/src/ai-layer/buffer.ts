...
export class Buffer {
  private states: Map<string, BufferState> = new Map();

  constructor() {
    blockchainBridge.registerModule('buffer');
  }

  private getOrCreate(entityId: string): BufferState {
  ...
  activate(entityId: string, intensity = 50): BufferState {
    ...
    blockchainBridge.validateAIOutput('buffer', { entityId, level: state.level });
    logger.info({ entityId, module: 'buffer', intensity }, '[Buffer] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'buffer' }, '[Buffer] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BufferState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BufferState | null { return this.states.get(entityId) ?? null; }
