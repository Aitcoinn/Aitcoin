...
export class Premonition {
  private states: Map<string, PremonitionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('premonition');
  }

  private getOrCreate(entityId: string): PremonitionState {
  ...
  activate(entityId: string, intensity = 50): PremonitionState {
    ...
    blockchainBridge.validateAIOutput('premonition', { entityId, level: state.level });
    logger.info({ entityId, module: 'premonition', intensity }, '[Premonition] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'premonition' }, '[Premonition] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PremonitionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PremonitionState | null { return this.states.get(entityId) ?? null; }
...
export const premonition = new Premonition();
export default premonition;
