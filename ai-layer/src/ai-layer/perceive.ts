...
export class Perceive {
  private states: Map<string, PerceiveState> = new Map();

  constructor() {
    blockchainBridge.registerModule('perceive');
  }

  private getOrCreate(entityId: string): PerceiveState {
  ...
  activate(entityId: string, intensity = 50): PerceiveState {
    ...
    blockchainBridge.validateAIOutput('perceive', { entityId, level: state.level });
    logger.info({ entityId, module: 'perceive', intensity }, '[Perceive] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'perceive' }, '[Perceive] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PerceiveState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PerceiveState | null { return this.states.get(entityId) ?? null; }
...
export const perceive = new Perceive();
