...
export class Hot {
  private states: Map<string, HotState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hot');
  }

  private getOrCreate(entityId: string): HotState {
  ...
  activate(entityId: string, intensity = 50): HotState {
    ...
    blockchainBridge.validateAIOutput('hot', { entityId, level: state.level });
    logger.info({ entityId, module: 'hot', intensity }, '[Hot] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hot' }, '[Hot] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HotState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HotState | null { return this.states.get(entityId) ?? null; }
