...
export class Rain {
  private states: Map<string, RainState> = new Map();

  constructor() {
    blockchainBridge.registerModule('rain');
  }

  private getOrCreate(entityId: string): RainState {
  ...
  activate(entityId: string, intensity = 50): RainState {
    ...
    blockchainBridge.validateAIOutput('rain', { entityId, level: state.level });
    logger.info({ entityId, module: 'rain', intensity }, '[Rain] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'rain' }, '[Rain] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RainState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RainState | null { return this.states.get(entityId) ?? null; }
