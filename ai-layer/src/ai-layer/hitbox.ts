...
export class Hitbox {
  ...
  constructor() {
    blockchainBridge.registerModule('hitbox');
  }

  private getOrCreate(entityId: string): HitboxState {
  ...
  activate(entityId: string, intensity = 50): HitboxState {
    ...
    blockchainBridge.validateAIOutput('hitbox', { entityId, level: state.level });
    logger.info({ entityId, module: 'hitbox', intensity }, '[Hitbox] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hitbox' }, '[Hitbox] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HitboxState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HitboxState | null { return this.states.get(entityId) ?? null; }
...
export const hitbox = new Hitbox();
