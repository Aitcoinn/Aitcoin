...
export class ConfuseExp {
  private states: Map<string, ConfuseExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('confuse_exp');
  }

  private getOrCreate(entityId: string): ConfuseExpState {
  ...
  activate(entityId: string, intensity = 50): ConfuseExpState {
    ...
    blockchainBridge.validateAIOutput('confuse_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'confuse_exp', intensity }, '[ConfuseExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'confuse_exp' }, '[ConfuseExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ConfuseExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ConfuseExpState | null { return this.states.get(entityId) ?? null; }
...
export const confuseExp = new ConfuseExp();
export default confuseExp;
