...
export class Rigid {
  private states: Map<string, RigidState> = new Map();

  constructor() {
    blockchainBridge.registerModule('rigid');
  }

  private getOrCreate(entityId: string): RigidState {
  ...
  activate(entityId: string, intensity = 50): RigidState {
    ...
    blockchainBridge.validateAIOutput('rigid', { entityId, level: state.level });
    logger.info({ entityId, module: 'rigid', intensity }, '[Rigid] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'rigid' }, '[Rigid] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RigidState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RigidState | null { return this.states.get(entityId) ?? null; }
