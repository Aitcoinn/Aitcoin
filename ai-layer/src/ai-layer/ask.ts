...
export class Ask {
  private states: Map<string, AskState> = new Map();

  private getOrCreate(entityId: string): AskState {
  ...
  activate(entityId: string, intensity = 50): AskState {
    ...
    logger.info({ entityId, module: 'ask', intensity }, '[Ask] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ask' }, '[Ask] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AskState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ask' }, '[Ask] Updated');
    return state;
  }

  getState(entityId: string): AskState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'ask' }, '[Ask] Reset');
  }
