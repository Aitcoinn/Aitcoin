...
export class Blizzard {
  private states: Map<string, BlizzardState> = new Map();

  constructor() {
    blockchainBridge.registerModule('blizzard');
  }

  private getOrCreate(entityId: string): BlizzardState {
  ...
  activate(entityId: string, intensity = 50): BlizzardState {
    ...
    blockchainBridge.validateAIOutput('blizzard', { entityId, level: state.level });
    logger.info({ entityId, module: 'blizzard', intensity }, '[Blizzard] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'blizzard' }, '[Blizzard] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BlizzardState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BlizzardState | null { return this.states.get(entityId) ?? null; }
...
export const blizzard = new Blizzard();
