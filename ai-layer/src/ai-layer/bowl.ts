...
export class Bowl {
  private states: Map<string, BowlState> = new Map();

  constructor() {
    blockchainBridge.registerModule('bowl');
  }

  private getOrCreate(entityId: string): BowlState {
  ...
  activate(entityId: string, intensity = 50): BowlState {
    ...
    blockchainBridge.validateAIOutput('bowl', { entityId, level: state.level });
    logger.info({ entityId, module: 'bowl', intensity }, '[Bowl] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'bowl' }, '[Bowl] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BowlState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BowlState | null { return this.states.get(entityId) ?? null; }
