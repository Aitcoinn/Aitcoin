...
export class Catalyst {
  private states: Map<string, CatalystState> = new Map();

  constructor() {
    blockchainBridge.registerModule('catalyst');
  }

  private getOrCreate(entityId: string): CatalystState {
  ...
  activate(entityId: string, intensity = 50): CatalystState {
    ...
    blockchainBridge.validateAIOutput('catalyst', { entityId, level: state.level });
    logger.info({ entityId, module: 'catalyst', intensity }, '[Catalyst] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'catalyst' }, '[Catalyst] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CatalystState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CatalystState | null { return this.states.get(entityId) ?? null; }
