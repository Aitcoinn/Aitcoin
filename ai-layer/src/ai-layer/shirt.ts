...
export class Shirt {
  private states: Map<string, ShirtState> = new Map();

  constructor() {
    blockchainBridge.registerModule('shirt');
  }

  private getOrCreate(entityId: string): ShirtState {
  ...
  activate(entityId: string, intensity = 50): ShirtState {
    ...
    blockchainBridge.validateAIOutput('shirt', { entityId, level: state.level });
    logger.info({ entityId, module: 'shirt', intensity }, '[Shirt] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'shirt' }, '[Shirt] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ShirtState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ShirtState | null { return this.states.get(entityId) ?? null; }
