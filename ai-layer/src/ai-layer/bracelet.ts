...
export class Bracelet {
  private states: Map<string, BraceletState> = new Map();

  constructor() {
    blockchainBridge.registerModule('bracelet');
  }

  private getOrCreate(entityId: string): BraceletState {
  ...
  activate(entityId: string, intensity = 50): BraceletState {
    ...
    blockchainBridge.validateAIOutput('bracelet', { entityId, level: state.level });
    logger.info({ entityId, module: 'bracelet', intensity }, '[Bracelet] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'bracelet' }, '[Bracelet] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BraceletState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): BraceletState | null { return this.states.get(entityId) ?? null; }
...
export const bracelet = new Bracelet();
