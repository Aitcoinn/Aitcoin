...
export class Fear {
  ...
  private getOrCreate(entityId: string): FearState {
  ...
  activate(entityId: string, intensity = 50): FearState {
    ...
    logger.info({ entityId, module: 'fear', intensity }, '[Fear] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fear' }, '[Fear] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FearState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fear' }, '[Fear] Updated');
    return state;
  }

  getState(entityId: string): FearState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'fear' }, '[Fear] Reset');
  }
