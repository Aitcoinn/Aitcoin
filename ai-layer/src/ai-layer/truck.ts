...
export class Truck {
  private states: Map<string, TruckState> = new Map();

  constructor() {
    blockchainBridge.registerModule('truck');
  }

  private getOrCreate(entityId: string): TruckState {
  ...
  activate(entityId: string, intensity = 50): TruckState {
    ...
    blockchainBridge.validateAIOutput('truck', { entityId, level: state.level });
    logger.info({ entityId, module: 'truck', intensity }, '[Truck] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'truck' }, '[Truck] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TruckState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TruckState | null { return this.states.get(entityId) ?? null; }
