...
export class Indicator {
  ...
  constructor() {
    blockchainBridge.registerModule('indicator');
  }

  private getOrCreate(entityId: string): IndicatorState {
  ...
  activate(entityId: string, intensity = 50): IndicatorState {
    ...
    blockchainBridge.validateAIOutput('indicator', { entityId, level: state.level });
    logger.info({ entityId, module: 'indicator', intensity }, '[Indicator] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'indicator' }, '[Indicator] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): IndicatorState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): IndicatorState | null { return this.states.get(entityId) ?? null; }
