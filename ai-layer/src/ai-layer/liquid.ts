...
export class Liquid {
  private states: Map<string, LiquidState> = new Map();

  constructor() {
    blockchainBridge.registerModule('liquid');
  }

  private getOrCreate(entityId: string): LiquidState {
  ...
  activate(entityId: string, intensity = 50): LiquidState {
    ...
    blockchainBridge.validateAIOutput('liquid', { entityId, level: state.level });
    logger.info({ entityId, module: 'liquid', intensity }, '[Liquid] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'liquid' }, '[Liquid] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LiquidState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LiquidState | null { return this.states.get(entityId) ?? null; }
