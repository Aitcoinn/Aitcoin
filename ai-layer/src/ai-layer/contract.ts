...
export class Contract {
  private states: Map<string, ContractState> = new Map();

  constructor() {
    blockchainBridge.registerModule('contract');
  }

  private getOrCreate(entityId: string): ContractState {
  ...
  activate(entityId: string, intensity = 50): ContractState {
    ...
    blockchainBridge.validateAIOutput('contract', { entityId, level: state.level });
    logger.info({ entityId, module: 'contract', intensity }, '[Contract] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'contract' }, '[Contract] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ContractState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ContractState | null { return this.states.get(entityId) ?? null; }
