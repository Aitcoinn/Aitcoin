...
export class Attention {
  ...
  private getOrCreate(entityId: string): AttentionState {
  ...
  activate(entityId: string, intensity = 50): AttentionState {
    ...
    logger.info({ entityId, module: 'attention', intensity }, '[Attention] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'attention' }, '[Attention] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AttentionState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'attention' }, '[Attention] Updated');
    return state;
  }

  getState(entityId: string): AttentionState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'attention' }, '[Attention] Reset');
  }
