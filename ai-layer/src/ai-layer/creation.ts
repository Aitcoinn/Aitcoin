...
export class Creation {
  private states: Map<string, CreationState> = new Map();

  constructor() {
    blockchainBridge.registerModule('creation');
  }

  private getOrCreate(entityId: string): CreationState {
  ...
  activate(entityId: string, intensity = 50): CreationState {
    ...
    blockchainBridge.validateAIOutput('creation', { entityId, level: state.level });
    logger.info({ entityId, module: 'creation', intensity }, '[Creation] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'creation' }, '[Creation] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CreationState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CreationState | null { return this.states.get(entityId) ?? null; }
...
export const creation = new Creation();
