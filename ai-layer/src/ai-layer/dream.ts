...
export class Dream {
  ...
  private getOrCreate(entityId: string): DreamState {
  ...
  activate(entityId: string, intensity = 50): DreamState {
    ...
    logger.info({ entityId, module: 'dream', intensity }, '[Dream] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'dream' }, '[Dream] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DreamState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'dream' }, '[Dream] Updated');
    return state;
  }

  getState(entityId: string): DreamState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'dream' }, '[Dream] Reset');
  }
