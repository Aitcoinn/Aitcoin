...
export class ComfortExp {
  private states: Map<string, ComfortExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('comfort_exp');
  }

  private getOrCreate(entityId: string): ComfortExpState {
  ...
  activate(entityId: string, intensity = 50): ComfortExpState {
    ...
    blockchainBridge.validateAIOutput('comfort_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'comfort_exp', intensity }, '[ComfortExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'comfort_exp' }, '[ComfortExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ComfortExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ComfortExpState | null { return this.states.get(entityId) ?? null; }
...
export const comfortExp = new ComfortExp();
