...
export class CellarSys {
  private states: Map<string, CellarSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cellar_sys');
  }

  private getOrCreate(entityId: string): CellarSysState {
  ...
  activate(entityId: string, intensity = 50): CellarSysState {
    ...
    blockchainBridge.validateAIOutput('cellar_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'cellar_sys', intensity }, '[CellarSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cellar_sys' }, '[CellarSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CellarSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CellarSysState | null { return this.states.get(entityId) ?? null; }
...
export const cellarSys = new CellarSys();
export default cellarSys;
