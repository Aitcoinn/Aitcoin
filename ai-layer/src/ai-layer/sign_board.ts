...
export class SignBoard {
  private states: Map<string, SignBoardState> = new Map();

  constructor() {
    blockchainBridge.registerModule('sign_board');
  }

  private getOrCreate(entityId: string): SignBoardState {
  ...
  activate(entityId: string, intensity = 50): SignBoardState {
    ...
    blockchainBridge.validateAIOutput('sign_board', { entityId, level: state.level });
    logger.info({ entityId, module: 'sign_board', intensity }, '[SignBoard] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'sign_board' }, '[SignBoard] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SignBoardState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SignBoardState | null { return this.states.get(entityId) ?? null; }
...
export const signBoard = new SignBoard();
