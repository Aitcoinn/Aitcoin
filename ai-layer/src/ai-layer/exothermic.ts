...
export class Exothermic {
  private states: Map<string, ExothermicState> = new Map();

  constructor() {
    blockchainBridge.registerModule('exothermic');
  }

  private getOrCreate(entityId: string): ExothermicState {
  ...
  activate(entityId: string, intensity = 50): ExothermicState {
    ...
    blockchainBridge.validateAIOutput('exothermic', { entityId, level: state.level });
    logger.info({ entityId, module: 'exothermic', intensity }, '[Exothermic] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'exothermic' }, '[Exothermic] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ExothermicState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ExothermicState | null { return this.states.get(entityId) ?? null; }
...
export const exothermic = new Exothermic();
