...
export class Snow {
  private states: Map<string, SnowState> = new Map();

  constructor() {
    blockchainBridge.registerModule('snow');
  }

  private getOrCreate(entityId: string): SnowState {
  ...
  activate(entityId: string, intensity = 50): SnowState {
    ...
    blockchainBridge.validateAIOutput('snow', { entityId, level: state.level });
    logger.info({ entityId, module: 'snow', intensity }, '[Snow] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'snow' }, '[Snow] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SnowState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SnowState | null { return this.states.get(entityId) ?? null; }
