...
export class Debt {
  private states: Map<string, DebtState> = new Map();

  constructor() {
    blockchainBridge.registerModule('debt');
  }

  private getOrCreate(entityId: string): DebtState {
  ...
  activate(entityId: string, intensity = 50): DebtState {
    ...
    blockchainBridge.validateAIOutput('debt', { entityId, level: state.level });
    logger.info({ entityId, module: 'debt', intensity }, '[Debt] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'debt' }, '[Debt] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DebtState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DebtState | null { return this.states.get(entityId) ?? null; }
