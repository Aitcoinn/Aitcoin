...
export class Rule {
  ...
  constructor() {
    blockchainBridge.registerModule('rule');
  }

  private getOrCreate(entityId: string): RuleState {
  ...
  activate(entityId: string, intensity = 50): RuleState {
    ...
    blockchainBridge.validateAIOutput('rule', { entityId, level: state.level });
    logger.info({ entityId, module: 'rule', intensity }, '[Rule] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'rule' }, '[Rule] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RuleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RuleState | null { return this.states.get(entityId) ?? null; }
