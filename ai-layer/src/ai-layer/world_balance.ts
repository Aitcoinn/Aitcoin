...
export class WorldBalance {
  private states: Map<string, WorldBalanceState> = new Map();

  constructor() {
    blockchainBridge.registerModule('world_balance');
  }

  private getOrCreate(entityId: string): WorldBalanceState {
  ...
  activate(entityId: string, intensity = 50): WorldBalanceState {
    ...
    blockchainBridge.validateAIOutput('world_balance', { entityId, level: state.level });
    logger.info({ entityId, module: 'world_balance', intensity }, '[WorldBalance] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'world_balance' }, '[WorldBalance] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WorldBalanceState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WorldBalanceState | null { return this.states.get(entityId) ?? null; }
...
export const worldBalance = new WorldBalance();
export default worldBalance;
