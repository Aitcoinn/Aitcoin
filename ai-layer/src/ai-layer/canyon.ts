...
export class Canyon {
  private states: Map<string, CanyonState> = new Map();

  constructor() {
    blockchainBridge.registerModule('canyon');
  }

  private getOrCreate(entityId: string): CanyonState {
  ...
  activate(entityId: string, intensity = 50): CanyonState {
    ...
    blockchainBridge.validateAIOutput('canyon', { entityId, level: state.level });
    logger.info({ entityId, module: 'canyon', intensity }, '[Canyon] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'canyon' }, '[Canyon] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CanyonState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CanyonState | null { return this.states.get(entityId) ?? null; }
...
export const canyon = new Canyon();
