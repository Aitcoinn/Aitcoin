...
export class Lamp {
  private states: Map<string, LampState> = new Map();

  constructor() {
    blockchainBridge.registerModule('lamp');
  }

  private getOrCreate(entityId: string): LampState {
  ...
  activate(entityId: string, intensity = 50): LampState {
    ...
    blockchainBridge.validateAIOutput('lamp', { entityId, level: state.level });
    logger.info({ entityId, module: 'lamp', intensity }, '[Lamp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'lamp' }, '[Lamp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LampState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LampState | null { return this.states.get(entityId) ?? null; }
