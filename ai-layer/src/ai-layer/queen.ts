...
export class Queen {
  private states: Map<string, QueenState> = new Map();

  private getOrCreate(entityId: string): QueenState {
  ...
  activate(entityId: string, intensity = 50): QueenState {
    ...
    logger.info({ entityId, module: 'queen', intensity }, '[Queen] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'queen' }, '[Queen] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): QueenState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'queen' }, '[Queen] Updated');
    return state;
  }

  getState(entityId: string): QueenState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'queen' }, '[Queen] Reset');
  }
