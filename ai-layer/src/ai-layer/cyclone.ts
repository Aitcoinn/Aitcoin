...
export class Cyclone {
  private states: Map<string, CycloneState> = new Map();

  constructor() {
    blockchainBridge.registerModule('cyclone');
  }

  private getOrCreate(entityId: string): CycloneState {
  ...
  activate(entityId: string, intensity = 50): CycloneState {
    ...
    blockchainBridge.validateAIOutput('cyclone', { entityId, level: state.level });
    logger.info({ entityId, module: 'cyclone', intensity }, '[Cyclone] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'cyclone' }, '[Cyclone] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CycloneState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CycloneState | null { return this.states.get(entityId) ?? null; }
...
export const cyclone = new Cyclone();
