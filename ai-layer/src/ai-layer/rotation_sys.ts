...
export class RotationSys {
  private states: Map<string, RotationSysState> = new Map();

  constructor() {
    blockchainBridge.registerModule('rotation_sys');
  }

  private getOrCreate(entityId: string): RotationSysState {
  ...
  activate(entityId: string, intensity = 50): RotationSysState {
    ...
    blockchainBridge.validateAIOutput('rotation_sys', { entityId, level: state.level });
    logger.info({ entityId, module: 'rotation_sys', intensity }, '[RotationSys] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'rotation_sys' }, '[RotationSys] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): RotationSysState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): RotationSysState | null { return this.states.get(entityId) ?? null; }
...
export const rotationSys = new RotationSys();
export default rotationSys;
