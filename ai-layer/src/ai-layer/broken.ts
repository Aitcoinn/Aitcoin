...
export class Broken {
  private states: Map<string, BrokenState> = new Map();

  private getOrCreate(entityId: string): BrokenState {
  ...
  activate(entityId: string, intensity = 50): BrokenState {
    ...
    logger.info({ entityId, module: 'broken', intensity }, '[Broken] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'broken' }, '[Broken] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): BrokenState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'broken' }, '[Broken] Updated');
    return state;
  }

  getState(entityId: string): BrokenState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'broken' }, '[Broken] Reset');
  }
