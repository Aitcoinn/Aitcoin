...
export class FunctionCall {
  private states: Map<string, FunctionCallState> = new Map();

  constructor() {
    blockchainBridge.registerModule('function_call');
  }

  private getOrCreate(entityId: string): FunctionCallState {
  ...
  activate(entityId: string, intensity = 50): FunctionCallState {
    ...
    blockchainBridge.validateAIOutput('function_call', { entityId, level: state.level });
    logger.info({ entityId, module: 'function_call', intensity }, '[FunctionCall] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'function_call' }, '[FunctionCall] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FunctionCallState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FunctionCallState | null { return this.states.get(entityId) ?? null; }
...
export const functionCall = new FunctionCall();
