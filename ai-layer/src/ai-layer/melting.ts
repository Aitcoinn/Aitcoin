...
export class Melting {
  private states: Map<string, MeltingState> = new Map();

  constructor() {
    blockchainBridge.registerModule('melting');
  }

  private getOrCreate(entityId: string): MeltingState {
  ...
  activate(entityId: string, intensity = 50): MeltingState {
    ...
    blockchainBridge.validateAIOutput('melting', { entityId, level: state.level });
    logger.info({ entityId, module: 'melting', intensity }, '[Melting] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'melting' }, '[Melting] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MeltingState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MeltingState | null { return this.states.get(entityId) ?? null; }
...
export const melting = new Melting();
