...
export class Viscosity {
  private states: Map<string, ViscosityState> = new Map();

  constructor() {
    blockchainBridge.registerModule('viscosity');
  }

  private getOrCreate(entityId: string): ViscosityState {
  ...
  activate(entityId: string, intensity = 50): ViscosityState {
    ...
    blockchainBridge.validateAIOutput('viscosity', { entityId, level: state.level });
    logger.info({ entityId, module: 'viscosity', intensity }, '[Viscosity] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'viscosity' }, '[Viscosity] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ViscosityState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ViscosityState | null { return this.states.get(entityId) ?? null; }
...
export const viscosity = new Viscosity();
