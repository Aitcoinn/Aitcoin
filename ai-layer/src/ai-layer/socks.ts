...
export class Socks {
  private states: Map<string, SocksState> = new Map();

  constructor() {
    blockchainBridge.registerModule('socks');
  }

  private getOrCreate(entityId: string): SocksState {
  ...
  activate(entityId: string, intensity = 50): SocksState {
    ...
    blockchainBridge.validateAIOutput('socks', { entityId, level: state.level });
    logger.info({ entityId, module: 'socks', intensity }, '[Socks] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'socks' }, '[Socks] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): SocksState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): SocksState | null { return this.states.get(entityId) ?? null; }
