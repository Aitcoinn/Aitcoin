...
export class Presence {
  ...
  constructor() {
    blockchainBridge.registerModule('presence');
  }

  private getOrCreate(entityId: string): PresenceState {
  ...
  activate(entityId: string, intensity = 50): PresenceState {
    ...
    blockchainBridge.validateAIOutput('presence', { entityId, level: state.level });
    logger.info({ entityId, module: 'presence', intensity }, '[Presence] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'presence' }, '[Presence] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PresenceState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PresenceState | null { return this.states.get(entityId) ?? null; }
