...
export class TemperatureExp {
  private states: Map<string, TemperatureExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('temperature_exp');
  }

  private getOrCreate(entityId: string): TemperatureExpState {
  ...
  activate(entityId: string, intensity = 50): TemperatureExpState {
    ...
    blockchainBridge.validateAIOutput('temperature_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'temperature_exp', intensity }, '[TemperatureExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'temperature_exp' }, '[TemperatureExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TemperatureExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TemperatureExpState | null { return this.states.get(entityId) ?? null; }
...
export const temperatureExp = new TemperatureExp();
export default temperatureExp;
