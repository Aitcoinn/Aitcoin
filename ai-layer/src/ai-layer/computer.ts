...
export class Computer {
  private states: Map<string, ComputerState> = new Map();

  constructor() {
    blockchainBridge.registerModule('computer');
  }

  private getOrCreate(entityId: string): ComputerState {
  ...
  activate(entityId: string, intensity = 50): ComputerState {
    ...
    blockchainBridge.validateAIOutput('computer', { entityId, level: state.level });
    logger.info({ entityId, module: 'computer', intensity }, '[Computer] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'computer' }, '[Computer] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ComputerState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ComputerState | null { return this.states.get(entityId) ?? null; }
