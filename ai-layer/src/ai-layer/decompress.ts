...
export class Decompress {
  private states: Map<string, DecompressState> = new Map();

  constructor() {
    blockchainBridge.registerModule('decompress');
  }

  private getOrCreate(entityId: string): DecompressState {
  ...
  activate(entityId: string, intensity = 50): DecompressState {
    ...
    blockchainBridge.validateAIOutput('decompress', { entityId, level: state.level });
    logger.info({ entityId, module: 'decompress', intensity }, '[Decompress] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'decompress' }, '[Decompress] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): DecompressState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): DecompressState | null { return this.states.get(entityId) ?? null; }
...
export const decompress = new Decompress();
