...
export class LogSystem {
  private states: Map<string, LogSystemState> = new Map();

  constructor() {
    blockchainBridge.registerModule('log_system');
  }

  private getOrCreate(entityId: string): LogSystemState {
  ...
  activate(entityId: string, intensity = 50): LogSystemState {
    ...
    blockchainBridge.validateAIOutput('log_system', { entityId, level: state.level });
    logger.info({ entityId, module: 'log_system', intensity }, '[LogSystem] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'log_system' }, '[LogSystem] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): LogSystemState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): LogSystemState | null { return this.states.get(entityId) ?? null; }
