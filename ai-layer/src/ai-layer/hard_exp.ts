...
export class HardExp {
  private states: Map<string, HardExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hard_exp');
  }

  private getOrCreate(entityId: string): HardExpState {
  ...
  activate(entityId: string, intensity = 50): HardExpState {
    ...
    blockchainBridge.validateAIOutput('hard_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'hard_exp', intensity }, '[HardExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hard_exp' }, '[HardExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HardExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HardExpState | null { return this.states.get(entityId) ?? null; }
...
export const hardExp = new HardExp();
export default hardExp;
