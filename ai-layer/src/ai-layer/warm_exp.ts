...
export class WarmExp {
  private states: Map<string, WarmExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('warm_exp');
  }

  private getOrCreate(entityId: string): WarmExpState {
  ...
  activate(entityId: string, intensity = 50): WarmExpState {
    ...
    blockchainBridge.validateAIOutput('warm_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'warm_exp', intensity }, '[WarmExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'warm_exp' }, '[WarmExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): WarmExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): WarmExpState | null { return this.states.get(entityId) ?? null; }
...
export const warmExp = new WarmExp();
export default warmExp;
