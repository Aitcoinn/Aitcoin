...
export class TrueExp {
  private states: Map<string, TrueExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('true_exp');
  }

  private getOrCreate(entityId: string): TrueExpState {
  ...
  activate(entityId: string, intensity = 50): TrueExpState {
    ...
    blockchainBridge.validateAIOutput('true_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'true_exp', intensity }, '[TrueExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'true_exp' }, '[TrueExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TrueExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TrueExpState | null { return this.states.get(entityId) ?? null; }
...
export const trueExp = new TrueExp();
