...
export class Meeting {
  ...
  private getOrCreate(entityId: string): MeetingState {
  ...
  activate(entityId: string, intensity = 50): MeetingState {
    ...
    logger.info({ entityId, module: 'meeting', intensity }, '[Meeting] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'meeting' }, '[Meeting] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MeetingState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'meeting' }, '[Meeting] Updated');
    return state;
  }

  getState(entityId: string): MeetingState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'meeting' }, '[Meeting] Reset');
  }
