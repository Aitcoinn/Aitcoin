...
export class Transaction {
  ...
  constructor() {
    blockchainBridge.registerModule('transaction');
  }

  private getOrCreate(entityId: string): TransactionState {
  ...
  activate(entityId: string, intensity = 50): TransactionState {
    ...
    blockchainBridge.validateAIOutput('transaction', { entityId, level: state.level });
    logger.info({ entityId, module: 'transaction', intensity }, '[Transaction] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'transaction' }, '[Transaction] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TransactionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TransactionState | null { return this.states.get(entityId) ?? null; }
