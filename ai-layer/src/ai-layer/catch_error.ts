...
export class CatchError {
  private states: Map<string, CatchErrorState> = new Map();

  constructor() {
    blockchainBridge.registerModule('catch_error');
  }

  private getOrCreate(entityId: string): CatchErrorState {
  ...
  activate(entityId: string, intensity = 50): CatchErrorState {
    ...
    blockchainBridge.validateAIOutput('catch_error', { entityId, level: state.level });
    logger.info({ entityId, module: 'catch_error', intensity }, '[CatchError] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'catch_error' }, '[CatchError] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): CatchErrorState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): CatchErrorState | null { return this.states.get(entityId) ?? null; }
...
export const catchError = new CatchError();
