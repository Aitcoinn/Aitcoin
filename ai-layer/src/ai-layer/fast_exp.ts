...
export class FastExp {
  private states: Map<string, FastExpState> = new Map();

  constructor() {
    blockchainBridge.registerModule('fast_exp');
  }

  private getOrCreate(entityId: string): FastExpState {
  ...
  activate(entityId: string, intensity = 50): FastExpState {
    ...
    blockchainBridge.validateAIOutput('fast_exp', { entityId, level: state.level });
    logger.info({ entityId, module: 'fast_exp', intensity }, '[FastExp] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'fast_exp' }, '[FastExp] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): FastExpState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): FastExpState | null { return this.states.get(entityId) ?? null; }
...
export const fastExp = new FastExp();
