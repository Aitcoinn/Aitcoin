...
export class IfElse {
  private states: Map<string, IfElseState> = new Map();

  constructor() {
    blockchainBridge.registerModule('if_else');
  }

  private getOrCreate(entityId: string): IfElseState {
  ...
  activate(entityId: string, intensity = 50): IfElseState {
    ...
    blockchainBridge.validateAIOutput('if_else', { entityId, level: state.level });
    logger.info({ entityId, module: 'if_else', intensity }, '[IfElse] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'if_else' }, '[IfElse] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): IfElseState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): IfElseState | null { return this.states.get(entityId) ?? null; }
...
export const ifElse = new IfElse();
