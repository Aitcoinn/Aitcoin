...
export class Quaternion {
  private states: Map<string, QuaternionState> = new Map();

  constructor() {
    blockchainBridge.registerModule('quaternion');
  }

  private getOrCreate(entityId: string): QuaternionState {
  ...
  activate(entityId: string, intensity = 50): QuaternionState {
    ...
    blockchainBridge.validateAIOutput('quaternion', { entityId, level: state.level });
    logger.info({ entityId, module: 'quaternion', intensity }, '[Quaternion] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'quaternion' }, '[Quaternion] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): QuaternionState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): QuaternionState | null { return this.states.get(entityId) ?? null; }
