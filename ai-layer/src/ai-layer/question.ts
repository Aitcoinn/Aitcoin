...
export class Question {
  ...
  private getOrCreate(entityId: string): QuestionState {
  ...
  activate(entityId: string, intensity = 50): QuestionState {
    ...
    logger.info({ entityId, module: 'question', intensity }, '[Question] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'question' }, '[Question] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): QuestionState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'question' }, '[Question] Updated');
    return state;
  }

  getState(entityId: string): QuestionState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'question' }, '[Question] Reset');
  }
