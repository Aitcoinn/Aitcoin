...
export class Polar {
  private states: Map<string, PolarState> = new Map();

  constructor() {
    blockchainBridge.registerModule('polar');
  }

  private getOrCreate(entityId: string): PolarState {
  ...
  activate(entityId: string, intensity = 50): PolarState {
    ...
    blockchainBridge.validateAIOutput('polar', { entityId, level: state.level });
    logger.info({ entityId, module: 'polar', intensity }, '[Polar] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'polar' }, '[Polar] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PolarState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PolarState | null { return this.states.get(entityId) ?? null; }
