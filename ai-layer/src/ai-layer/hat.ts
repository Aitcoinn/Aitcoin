...
export class Hat {
  private states: Map<string, HatState> = new Map();

  constructor() {
    blockchainBridge.registerModule('hat');
  }

  private getOrCreate(entityId: string): HatState {
  ...
  activate(entityId: string, intensity = 50): HatState {
    ...
    blockchainBridge.validateAIOutput('hat', { entityId, level: state.level });
    logger.info({ entityId, module: 'hat', intensity }, '[Hat] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'hat' }, '[Hat] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): HatState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): HatState | null { return this.states.get(entityId) ?? null; }
