...
export class TableData {
  private states: Map<string, TableDataState> = new Map();

  constructor() {
    blockchainBridge.registerModule('table_data');
  }

  private getOrCreate(entityId: string): TableDataState {
  ...
  activate(entityId: string, intensity = 50): TableDataState {
    ...
    blockchainBridge.validateAIOutput('table_data', { entityId, level: state.level });
    logger.info({ entityId, module: 'table_data', intensity }, '[TableData] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'table_data' }, '[TableData] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): TableDataState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): TableDataState | null { return this.states.get(entityId) ?? null; }
