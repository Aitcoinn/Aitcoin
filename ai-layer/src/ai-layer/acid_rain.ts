...
export class AcidRain {
  private states: Map<string, AcidRainState> = new Map();

  constructor() {
    blockchainBridge.registerModule('acid_rain');
  }

  private getOrCreate(entityId: string): AcidRainState {
  ...
  activate(entityId: string, intensity = 50): AcidRainState {
    ...
    blockchainBridge.validateAIOutput('acid_rain', { entityId, level: state.level });
    logger.info({ entityId, module: 'acid_rain', intensity }, '[AcidRain] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'acid_rain' }, '[AcidRain] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AcidRainState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AcidRainState | null { return this.states.get(entityId) ?? null; }
...
export const acidRain = new AcidRain();
