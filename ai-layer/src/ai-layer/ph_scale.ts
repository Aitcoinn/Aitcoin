...
export class PhScale {
  private states: Map<string, PhScaleState> = new Map();

  constructor() {
    blockchainBridge.registerModule('ph_scale');
  }

  private getOrCreate(entityId: string): PhScaleState {
  ...
  activate(entityId: string, intensity = 50): PhScaleState {
    ...
    blockchainBridge.validateAIOutput('ph_scale', { entityId, level: state.level });
    logger.info({ entityId, module: 'ph_scale', intensity }, '[PhScale] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'ph_scale' }, '[PhScale] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): PhScaleState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): PhScaleState | null { return this.states.get(entityId) ?? null; }
...
export const phScale = new PhScale();
