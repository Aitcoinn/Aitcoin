...
export class Matrix {
  private states: Map<string, MatrixState> = new Map();

  constructor() {
    blockchainBridge.registerModule('matrix');
  }

  private getOrCreate(entityId: string): MatrixState {
  ...
  activate(entityId: string, intensity = 50): MatrixState {
    ...
    blockchainBridge.validateAIOutput('matrix', { entityId, level: state.level });
    logger.info({ entityId, module: 'matrix', intensity }, '[Matrix] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'matrix' }, '[Matrix] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): MatrixState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): MatrixState | null { return this.states.get(entityId) ?? null; }
