...
export class Student {
  ...
  private getOrCreate(entityId: string): StudentState {
  ...
  activate(entityId: string, intensity = 50): StudentState {
    ...
    logger.info({ entityId, module: 'student', intensity }, '[Student] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'student' }, '[Student] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): StudentState {
    const state = this.getOrCreate(entityId);
    state.data = { ...state.data, ...data };
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'student' }, '[Student] Updated');
    return state;
  }

  getState(entityId: string): StudentState | null {
  ...
  reset(entityId: string): void {
    ...
    logger.info({ entityId, module: 'student' }, '[Student] Reset');
  }
