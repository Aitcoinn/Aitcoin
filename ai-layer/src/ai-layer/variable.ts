...
export class Variable {
  ...
  constructor() {
    blockchainBridge.registerModule('variable');
  }

  private getOrCreate(entityId: string): VariableState {
  ...
  activate(entityId: string, intensity = 50): VariableState {
    ...
    blockchainBridge.validateAIOutput('variable', { entityId, level: state.level });
    logger.info({ entityId, module: 'variable', intensity }, '[Variable] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'variable' }, '[Variable] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): VariableState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): VariableState | null { return this.states.get(entityId) ?? null; }
