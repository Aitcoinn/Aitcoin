...
export class Clear {
  private states: Map<string, ClearState> = new Map();

  constructor() {
    blockchainBridge.registerModule('clear');
  }

  private getOrCreate(entityId: string): ClearState {
  ...
  activate(entityId: string, intensity = 50): ClearState {
    ...
    blockchainBridge.validateAIOutput('clear', { entityId, level: state.level });
    logger.info({ entityId, module: 'clear', intensity }, '[Clear] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'clear' }, '[Clear] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): ClearState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): ClearState | null { return this.states.get(entityId) ?? null; }
