...
export class Aftershock {
  private states: Map<string, AftershockState> = new Map();

  constructor() {
    blockchainBridge.registerModule('aftershock');
  }

  private getOrCreate(entityId: string): AftershockState {
  ...
  activate(entityId: string, intensity = 50): AftershockState {
    ...
    blockchainBridge.validateAIOutput('aftershock', { entityId, level: state.level });
    logger.info({ entityId, module: 'aftershock', intensity }, '[Aftershock] Activated');
    return state;
  }
  ...
  deactivate(entityId: string): void {
    ...
    state.updatedAt = Date.now();
    logger.info({ entityId, module: 'aftershock' }, '[Aftershock] Deactivated');
  }

  update(entityId: string, data: Record<string, unknown>): AftershockState {
    const state = this.getOrCreate(entityId);
  ...
  getState(entityId: string): AftershockState | null { return this.states.get(entityId) ?? null; }
...
export const aftershock = new Aftershock();
