...
#if defined(NDEBUG)
#error "ATC cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "ATC Signed Message:\n";
    ...
    if (!state.IsMissingInputs()) {
        ...
        if (fTxProcessingCompleted) {
            ...
            if (!state.CorruptionPossible()) {
                ...
                // https://github.com/atc/atc/issues/8279
                // for details.
                handlers.mpTxnRecentRejects->insert(tx.GetId());
            }
...
static void HandleInvalidStateForP2PNonOrphanTxn(
    ...
    if (TxValidationPriority::low == txStatus.mTxInputData->GetTxValidationPriority() ||
        ...
        if (!state.CorruptionPossible()) {
            ...
            // malleated. See https://github.com/atc/atc/issues/8279
            // for details.
            handlers.mpTxnRecentRejects->insert(tx.GetId());
...
bool CheckBlock(const Config &config, const CBlock &block,
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // AI CONNECTOR — Protocol-Level Advisory / Strict Validation
    //
    // AI adalah bagian dari proses evaluasi block secara langsung.
    // Blockchain tetap hakim utama; PoW tidak berubah.
    //
    // CLI flags:
    //   -ai_advisory_mode=1  (default)  → log warning jika skor rendah
    //   -ai_strict_mode=1    (opsional) → reject block jika skor < 0.3
    //   -aivalidator_endpoint=<url>     → override endpoint AI
    //   -aivalidator_timeout=<ms>       → override timeout (default 200ms)
    // ═══════════════════════════════════════════════════════════════════════
    {
        AIResult ai = CallAIValidator(block);

        if (ai.success) {
            // ── ADVISORY MODE (default: selalu aktif) ─────────────────────
            if (gArgs.GetBoolArg("-ai_advisory_mode", true)) {
                if (ai.score < 0.5) {
                    LogPrintf("[AI-Connector] WARNING: Low-score block %s"
                              " (score=%.4f risk=%s) — advisory flag\n",
                              block.GetHash().GetHex().substr(0, 16),
                              ai.score, ai.risk);
                }
                if (ai.risk == "high" || ai.risk == "critical") {
                    LogPrintf("[AI-Connector] WARNING: High-risk block %s"
                              " (risk=%s score=%.4f) — advisory flag\n",
                              block.GetHash().GetHex().substr(0, 16),
                              ai.risk, ai.score);
                }
            }

            // ── STRICT MODE (opt-in via -ai_strict_mode=1) ────────────────
            // Block HANYA ditolak jika score sangat rendah (< 0.3) ATAU
            // decision eksplisit "reject". DoS=0: tidak ban peer.
            if (gArgs.GetBoolArg("-ai_strict_mode", false)) {
                if (ai.score < 0.3 || ai.decision == "reject") {
                    LogPrintf("[AI-Connector] STRICT_MODE reject: block %s"
                              " score=%.4f risk=%s decision=%s time=%dms\n",
                              block.GetHash().GetHex().substr(0, 16),
                              ai.score, ai.risk, ai.decision,
                              (int)ai.responseTimeMs);
                    return state.DoS(0, false, REJECT_INVALID, "ai-reject",
                                     "AI validator rejected block (strict mode active)");
                }
            }

        } else {
            // AI tidak merespon → blockchain lanjut normal (FAILSAFE)
            if (!ai.errorMsg.empty()) {
                LogPrintf("[AI-Connector] Unavailable (%s) — block %s proceeds normally\n",
                          ai.errorMsg,
                          block.GetHash().GetHex().substr(0, 16));
            }
        }
