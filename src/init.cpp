...
std::string HelpMessage(HelpMessageMode mode, const Config& config) {
    ...
    strUsage += HelpMessageOpt(
        "-printtoconsole",
        _("Send trace/debug info to console instead of aitcoind.log file"));
    strUsage += HelpMessageOpt("-shrinkdebugfile",
                               _("Shrink aitcoind.log file on client startup "
...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE =
        "<https://github.com/atc-labs/aitcoin>";
    const std::string URL_WEBSITE = "<https://www.aitcoin.com>";
...
/** Sanity checks
 *  Ensure that ATC is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly) {
    ...
    // Make sure only a single ATC process is using the data directory.
...
bool AppInitMain(ConfigInit &config, boost::thread_group &threadGroup,
    ...
    if (gArgs.GetBoolArg("-shrinkdebugfile", default_shrinkdebugfile)) {
        // Do this first since it both loads a bunch of aitcoind.log into memory,
        // and because this needs to happen before any other aitcoind.log printing.
