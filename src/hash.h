...
/** A hasher class for ATC's 256-bit hash (double SHA-256). */
class CHash256 {
private:
    CSHA256 sha;
...
/** A hasher class for ATC's 160-bit hash (SHA-256 + RIPEMD-160). */
class CHash160 {
private:
    CSHA256 sha;
...
// ── Post-Quantum Hash Primitives (added 2024) ─────────────────────────────────
// SHA3-256 is the quantum-resistant hash primitive for AITCOIN.
// Grover's algorithm gives only sqrt speedup on hash functions — SHA3-256
// retains 128-bit post-quantum security (vs 64-bit for SHA-256 in PQ context).
#include "crypto/sha3.h"

/** Post-quantum hash using SHA3-256. Replaces double-SHA256 for PQ addresses. */
template <typename T1>
inline uint256 HashPQ(const T1 pbegin, const T1 pend) {
    static const uint8_t pblank[1] = {};
    uint256 result;
    CSHA3_256()
        .Write(pbegin == pend ? pblank : (const uint8_t*)&pbegin[0],
...
/** CHash256Q: SHA3-256 single-pass — quantum-resistant drop-in for CHash256. */
class CHash256Q {
    CSHA3_256 sha3;
public:
    static const size_t OUTPUT_SIZE = 32;
    void Finalize(uint8_t hash[OUTPUT_SIZE]) { sha3.Finalize(hash); }
    CHash256Q &Write(const uint8_t *d, size_t l) { sha3.Write(d,l); return *this; }
    CHash256Q &Reset() { sha3.Reset(); return *this; }
};

/** CHash160Q: SHA3-256 truncated to 20 bytes — quantum-resistant address hash.
 *  Replaces CHash160 (RIPEMD160(SHA256(pubkey))) for Dilithium3 key addresses. */
class CHash160Q {
    CSHA3_256 sha3;
public:
    ...
    CHash160Q &Write(const uint8_t *d, size_t l) { sha3.Write(d,l); return *this; }
    CHash160Q &Reset() { sha3.Reset(); return *this; }
};
// ─────────────────────────────────────────────────────────────────────────────
