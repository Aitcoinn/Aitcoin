...
UniValue dumpprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The atc address for the "
            "private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "(either absolute or relative to aitcoind)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/atc/atc/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by ATC %s\n", CLIENT_BUILD);
