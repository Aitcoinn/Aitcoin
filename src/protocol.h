...
/**
 * ATC protocol message types. When adding new message types, don't forget
...
enum ServiceFlags : uint64_t {
    ...
    // It is currently set by all AITCOIN nodes, and is unset by SPV clients
    ...
    // protocol request. AITCOIN does not support this but a patch set
    // called ATC XT does. See BIP 64 for details on how this is
    // implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered
    // connections. AITCOIN nodes used to support this by default, without
    ...
    NODE_XTHIN = (1 << 4),
    // NODE_IXC_CASH means the node supports ATC and the
    // associated consensus rule changes.
    // This service bit is intended to be used prior until some time after the
    // UAHF activation when the ATC network has adequately separated.
    ...
    // atc-development mailing list. Remember that service bits are just
