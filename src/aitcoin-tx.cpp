...
static int AppInitRawTx(int argc, char *argv[]) {
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
        ...
        std::string strUsage =
            strprintf(_("%s aitcoin-tx utility version"), _(PACKAGE_NAME)) +
            " " + FormatFullVersion() + "\n\n" + _("Usage:") + "\n" +
            "  aitcoin-tx [options] <hex-tx> [commands]  " +
            _("Update hex-encoded atc transaction") + "\n" +
            "  aitcoin-tx [options] -create [commands]   " +
            _("Create hex-encoded atc transaction") + "\n" + "\n";
...
static void MutateTxAddInput(CMutableTransaction &tx,
    ...
    static const unsigned int maxVout = MAX_TX_SIZE_CONSENSUS_AFTER_GENESIS / minTxOutSz; // aitcoin-tx tool can build txs with more vouts than pre Genesis nodes  would accept
...
static int CommandLineRawTx(int argc, char *argv[],
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded atc transaction
