...
void atc::bint::empty_bn_deleter::operator()(bignum_st* p) const
{
    ::BN_free(p);
}

atc::bint::bint() : value_{nullptr} {}

atc::bint::bint(const int i) : value_(BN_new(), empty_bn_deleter())
{
    // assert(value_);
    if(!value_)
        throw big_int_error();
...
atc::bint::bint(const int64_t i) : value_(BN_new(), empty_bn_deleter())
{
    // assert(value_);
    if(!value_)
        throw big_int_error();
...
atc::bint::bint(const size_t i) : value_(BN_new(), empty_bn_deleter())
{
    assert(value_);
    if(!value_)
        throw big_int_error();
...
atc::bint::bint(const std::string& n) : value_(BN_new(), empty_bn_deleter())
{
    // assert(value_);
    if(!value_)
        throw big_int_error();
...
atc::bint::bint(const bint& other) : value_(BN_new(), empty_bn_deleter())
{
...
atc::bint& atc::bint::operator=(const bint& other)
{
    bint temp{other};
    swap(temp);
    return *this;
...
void atc::bint::swap(bint& other) noexcept
{
    // assert(value_);
    using std::swap;
...
bool atc::operator<(const bint& a, const bint& b)
{
...
atc::bint& atc::bint::operator<<=(const int n)
{
    if(n <= 0)
        return *this;
...
atc::bint atc::bint::operator-() const
{
    bint rv{*this};
    rv.negate();
    return rv;
...
int atc::bint::spaceship_operator(
...
void atc::bint::mask_bits(const int n)
{
...
int atc::bint::size_bits() const { return BN_num_bits(value_.get()); }

int atc::bint::size_bytes() const { return BN_num_bytes(value_.get()); }

atc::bint::buffer_type atc::bint::to_bin() const
{
    // assert(value_);
...
std::ostream& atc::operator<<(std::ostream& os, const bint& n)
{
    if(n.value_ == nullptr)
        return os;
...
bool atc::is_negative(const bint& n)
{
    const auto s{BN_is_negative(n.value_.get())};
    return s == 1;
}

atc::bint atc::abs(const bint& n) { return is_negative(n) ? -n : n; }

std::string atc::to_string(const bint& n) // used in gdb pretty-printer
{
...
std::size_t atc::to_size_t_limited(const bint& n)
{
    // Precondition:
    // Windows/MSVC (sizeof(long) == 4 bytes)
    // n <= numeric_limit<int32_t>::max() and n>=0
...
std::vector<uint8_t> atc::bint::serialize() const
{
...
atc::bint atc::bint::deserialize(atc::span<const uint8_t> s)
{
