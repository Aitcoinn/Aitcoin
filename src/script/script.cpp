...
uint64_t CScript::GetSigOpCount(bool fAccurate, bool isGenesisEnabled, bool& sigOpCountError) const
{
    ...
    uint64_t n = 0;
    atc::instruction last_instruction{OP_INVALIDOPCODE};
...
bool IsP2SH(const atc::span<const uint8_t> script) {
...
bool IsDSNotification(atc::span<const uint8_t> script) {
...
bool IsDustReturnScript (atc::span<const uint8_t> script)
{
...
CScript &CScript::push_int64(int64_t n) {
    if (n == -1 || (n >= 1 && n <= 16)) {
        ...
        v.reserve(sizeof(n));
        atc::serialize(n, back_inserter(v));
        *this << v;
    }
    return *this;
}
...
atc::instruction_iterator CScript::begin_instructions() const
{
    return atc::instruction_iterator{atc::span<const uint8_t>{data(), size()}};
}

atc::instruction_iterator CScript::end_instructions() const
{
    return atc::instruction_iterator{
        atc::span<const uint8_t>{data() + size(), 0}};
}
...
size_t CountOp(const atc::span<const uint8_t> s, const opcodetype opcode)
{
    using namespace atc;
