...
CScriptNum::CScriptNum(atc::span<const uint8_t> span,
                       ...
                       const bool big_int)
{
    ...
    if(fRequireMinimal && !atc::IsMinimallyEncoded(span, nMaxNumSize))
    {
    ...
    {
        if(big_int)
            m_value = atc::bint::deserialize(span);
        else
            m_value = atc::deserialize<int64_t>(span.begin(), span.end());
    }
...
int CScriptNum::getint() const
{
    ...
    return std::visit(overload{[](const atc::bint& n) -> int {
...
size_t CScriptNum::to_size_t_limited() const
{
    ...
    return std::visit(overload{[](const atc::bint& n) {
                                   ...
                                   return atc::to_size_t_limited(n);
                               },
                               [](const int64_t n) {
                                   //we are using int32_t because this is minimum supported size in Windows and Linux based compiler
                                   assert(n >= 0 && n <= std::numeric_limits<int32_t>::max());
