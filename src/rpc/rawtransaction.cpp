                       ...
                       std::function<void()> httpCallback) 
{
    ...
    // Call into TxToJSON() in atc-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in atc-common, so we query them here and push the
    // data as JSON.
...
static UniValue createrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "key is the atc address, the numeric value (can be string) is "
            "the " +
            CURRENCY_UNIT +
            " amount\n"
