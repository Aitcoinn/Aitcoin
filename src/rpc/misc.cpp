...
static UniValue getinfo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total atc "
...
static UniValue validateaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nReturn information about the given atc address.\n"
            "\nArguments:\n"
...
#ifdef ENABLE_WALLET
        // Case 1: ATC address and we have full public key:
        if (pwallet) {
...
static UniValue createmultisig(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "are atc addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) atc address or hex-encoded public "
            "key\n"
            "       ,...\n"
            "     ]\n"
...
static UniValue echo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp) {
        throw std::runtime_error(
            ...
            "aitcoin-cli. There is no server-side difference.");
    }
